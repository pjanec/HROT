...
/// <summary>
/// ImGui scenario and story control panel for the Orchestrator subsystem (CGF1-S0506).
///
...
/// <see cref="DrillMaster"/> or any local service.</para>
///
/// Renders: Bootstrap banner, Node Health table, Time Control, 2PC History,
/// Status Banner, Drill Control, Checkpoint, Scenario, Replay, Stories, Archive.
/// </summary>
public sealed class ClusterScenarioPanel
{
    ...
    /// <param name="sysOpWriter">DDS writer for SysOpRequest commands (S0502).</param>
    ...
    public void Render(ClusterUiCache cache, bool disableAll)
    {
        ...
        // ── 2. Drill Control ───────────────────────────────────────────────
        RenderDrillControl(cache, disableAll);
        ...
        // ── 6. Stories ─────────────────────────────────────────────────────
        RenderStoriesSection(cache, disableAll);
    ...
    private void RenderTimeControl(ClusterUiCache cache, bool disableAll)
    {
        ...
        if (ImGui.Button(cache.IsPaused ? "Resume##OrcResume" : "Pause##OrcPause"))
            ...
            {
                RequestId     = Guid.NewGuid(),
                OperationType = cache.IsPaused ? SysOpType.ResumeTime : SysOpType.PauseTime,
    ...
    private void RenderDrillControl(ClusterUiCache cache, bool disableAll)
    {
    ...
    private void RenderReplaySection(ClusterUiCache cache, bool disableAll)
    {
        ...
        if (ImGui.BeginChild("##OrcReplay", AutoSize, ImGuiChildFlags.Borders | ImGuiChildFlags.AutoResizeY))
        {
            ...
            if (_selectedDrillIdx >= cache.AvailableDrills.Length) _selectedDrillIdx = -1;

            // Load Replay
            ImGui.Combo("Select Drill##OrcReplayId", ref _selectedDrillIdx,
                cache.AvailableDrills, cache.AvailableDrills.Length);
            ImGui.SameLine();
            if (ImGui.Button("Load Replay##OrcReplayBtn") && _selectedDrillIdx >= 0)
            {
                string drillId = cache.AvailableDrills[_selectedDrillIdx];
            ...
            if (cache.CurrentState == DSMState.RunningReplay)
            {
                float currentDrillTime = (float)cache.MasterSimTime;
                if (!_seekPending)
    ...
    private void RenderStoriesSection(ClusterUiCache cache, bool disableAll)
    {
        ...
        if (ImGui.BeginChild("##OrcStories", new Vector2(-1, 180), ImGuiChildFlags.Borders))
        {
            ...
            // Active stories list (from cache)
            var activeStories = cache.ActiveStories;
            ...
            if (_selectedStoryIdx >= cache.AvailableScenarios.Length) _selectedStoryIdx = -1;
            ...
            {
                string scenId     = cache.AvailableScenarios[_selectedStoryIdx];
    ...
    private void RenderArchiveSection(ClusterUiCache cache, bool disableAll)
    {
        ...
        if (_selectedUnarchivedIdx >= cache.UnarchivedLocalDrills.Length) _selectedUnarchivedIdx = -1;

        ImGui.Text("Unarchived Local:");
        ImGui.Combo("##UnarchivedCombo", ref _selectedUnarchivedIdx,
                    cache.UnarchivedLocalDrills, cache.UnarchivedLocalDrills.Length);
        ...
        {
            var drillName = cache.UnarchivedLocalDrills[_selectedUnarchivedIdx];
        ...
        // — Archived NAS Drills —
        if (_selectedArchiveIdx >= cache.ArchivedDrills.Length) _selectedArchiveIdx = -1;

        ImGui.Text("Archived on NAS:");
        ImGui.Combo("##ArchivedCombo", ref _selectedArchiveIdx,
                    cache.ArchivedDrills, cache.ArchivedDrills.Length);
        ...
        if (ImGui.Button("Import from NAS ◄##OrcImport")
            ...
            && _activeArchiveOpId == Guid.Empty)
        {
            var drillName = cache.ArchivedDrills[_selectedArchiveIdx];
            var requestId = Guid.NewGuid();
            _activeArchiveOpId = requestId;
            _sysOpWriter.Write(new SysOpRequest
            {
