...
public class CreationToolTests
{
    ...
    public void HandleClick_LeftClick_InitialDescriptorsContainGeoSpatialWithClickCoordinates()
    {
        ...
        var descriptors = captured[0].InitialDescriptors;
        Assert.NotNull(descriptors);
        var geoEntry = descriptors.FirstOrDefault(d => d._d == EDescriptorType.dtWorldPos);
        Assert.Equal(EDescriptorType.dtWorldPos, geoEntry._d);
        Assert.Equal(ClickY, geoEntry.WorldPos.Pos.Latitude,  precision: 3);
        Assert.Equal(ClickX, geoEntry.WorldPos.Pos.Longitude, precision: 3);
    }
    ...
    public void HandleClick_LeftClick_WithGeoTransform_UsesConvertedCoordinates()
    {
        ...
        var descriptors = captured[0].InitialDescriptors;
        var geoEntry    = descriptors.First(d => d._d == EDescriptorType.dtWorldPos);
        Assert.Equal(ExpectedLat, geoEntry.WorldPos.Pos.Latitude,  precision: 4);
        Assert.Equal(ExpectedLon, geoEntry.WorldPos.Pos.Longitude, precision: 4);
    }
    ...
    public void CreationTool_EmitsOnly_EntityMaster_And_GeoSpatial_Descriptors()
    {
        ...
        Assert.Contains(descriptors, d => d._d == EDescriptorType.dtWorldPos);
