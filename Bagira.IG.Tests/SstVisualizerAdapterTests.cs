...
/// <summary>
/// Unit tests for <see cref="SstVisualizerAdapter"/>.
///
/// Covers the non-Raylib-rendering surface of the adapter:
/// <see cref="SstVisualizerAdapter.GetPosition"/> (culling gate + coordinate extraction),
/// <see cref="SstVisualizerAdapter.GetHitRadius"/>, and
/// <see cref="SstVisualizerAdapter.GetHoverLabel"/>.
///
...
public class SstVisualizerAdapterTests
{
    ...
    public void GetHitRadius_ReturnsConstantWorldUnits()
    {
        ...
        Assert.Equal(SstVisualizerAdapterConstants.HitRadiusWorldUnits, radius);
    }

    /// <summary>
    /// Pins the calculated hit radius against camera constants to detect
    /// accidental changes to either <see cref="SstVisualizerAdapterConstants.FallbackCircleRadiusPx"/>
    ...
    public void GetHitRadius_MatchesExpectedWorldUnitsForDefaultZoom()
    {
        const float expected =
            (float)SstVisualizerAdapterConstants.FallbackCircleRadiusPx
            / IgCameraConstants.InitialZoom;

        Assert.Equal(expected, SstVisualizerAdapterConstants.HitRadiusWorldUnits, precision: 4);
    }
