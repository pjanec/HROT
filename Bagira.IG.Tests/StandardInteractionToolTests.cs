...
/// and the <see cref="SstVisualizerAdapter.GetHitRadius"/> contract without requiring a
...
public class StandardInteractionToolTests
{
    ...
    /// <summary>
    /// Hit radius from <see cref="SstVisualizerAdapterConstants.HitRadiusWorldUnits"/>.
    /// Stored as a test-local named constant so an assertion change is a one-line edit.
    /// </summary>
    private const float HitRadius = SstVisualizerAdapterConstants.HitRadiusWorldUnits;
    ...
    /// <see cref="MapCullingSystem"/> so that <see cref="SstVisualizerAdapter.GetPosition"/>
    ...
    private static EntityRenderLayer CreateLayer(EntityRepository repo, EntityQuery query, SstVisualizerAdapter adapter)
    ...
    /// <see cref="SstVisualizerAdapter.GetPosition"/> returns <c>null</c> for invisible
