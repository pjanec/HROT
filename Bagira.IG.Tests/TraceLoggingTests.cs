...
public sealed class TraceLoggingTests : IDisposable
{
    ...
    public void IngressAndRender_EmitsTraceLines()
    {
        ...
        using var geoWriter = new DdsWriter<GeoSpatial>(participant, "GeoSpatial");
        ...
        while ((DateTime.UtcNow - start).TotalMilliseconds < TimeoutMs)
        {
            ...
            if (!renderAttempted && TryGetEntity(_ig.World, networkId, out var entity))
            {
                if (!geoSent)
                {
                    geoWriter.Write(new GeoSpatial
                    {
                        EntityId = (int)networkId,
                        Pos = new GeoPosition { Latitude = 52.52, Longitude = 13.405, Altitude = 0 },
    ...
    private static void TryRenderOnce(EntityRepository world, Entity entity)
    {
        ...
        SstVisualizerAdapter.RenderTraceEntityIdOverride = entity.Index;
        ...
        {
            Raylib.CloseWindow();
            SstVisualizerAdapter.RenderTraceEntityIdOverride = null;
        }
    }
