...
namespace Bagira.SimHost.Systems
{
    ...
    ///
    /// <para>
    /// Phase 1 (InProgress, <see cref="SstStatusCode.InProgress"/>) is sent immediately
    ...
    public class SstRequestFinalizationSystem : IEcsModuleSystem
    {
        ...
        public SstRequestFinalizationSystem(
        ...
        public void Execute(ISimulationView view, float deltaTime)
        {
            ...
            foreach (var kvp in _tracked)
            {
                ...
                bool resolved   = false;
                var  finalStatus = SstStatusCode.EntityNotFound;
                ...
                if (pending.Kind == RequestKind.Create)
                {
                    if (_entityMap.TryGetEntity(networkId, out var entity))
                    {
                        if (!view.IsAlive(entity))
                        {
                            ...
                            resolved    = true;
                            finalStatus = SstStatusCode.EntityNotFound;
                        }
                              ...
                              && repo.GetLifecycleState(entity) == EntityLifecycle.Active)
                        {
                            ...
                            resolved    = true;
                            finalStatus = SstStatusCode.Success;
                        }
