...
While using DI (`IActionExecutor<T>`) successfully unlinked `ActionDispatchModule` from `Bagira.SimHost` at compile time, **the actual executors (`AimAndFireExecutor` and `JoinFormationExecutor`) were left behind in the `Bagira` domain.**
...
1. Formally extract the combat and formation executors into brand-new FDP toolkits, eliminating the Bagira domain residency for these systems once and for all.
