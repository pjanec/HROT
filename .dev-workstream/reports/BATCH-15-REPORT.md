...
| (pre-existing) | `DemoTkbSetup` sets `DoctrineState { BrainTier = 2 }` for `MilitaryAPC`, matching the value in the gutted `EntityBlueprints.cs`. The field value `2` equals `BrainTierBTree`, which means `HsmTickSystem` (which filters on `BrainTierHsm = 1`) would skip the APC at runtime. This pre-existing mismatch should be corrected in a future batch when the full doctrine registration for the APC is wired. |
