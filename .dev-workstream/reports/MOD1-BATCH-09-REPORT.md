...
- [x] `FDP.Framework.Runner` compiles with zero `Bagira.*` references.
- [x] `SubsystemOrchestrator` is in `FDP.Framework.Runner` with no hardcoded concrete type references.
- [x] `Bagira.Runner.Program` is a pure composition root: parse args → construct subsystems → inject → run.
- [x] `Bagira.Runner -x all` integration tests pass unconditionally.
...
|---|---|---|---|
| Bagira.DDS.DataModel.Tests | 0 | 16 | 0 |
| Bagira.IG.Tests | 0 | 304 | 0 |
| Bagira.IOS.Tests | 0 | 270 | 0 |
| Bagira.Runner.Tests | 0 | 99 | 0 |
| Bagira.Runner.Integration.Tests | 0 | 31 | 0 |
| Bagira.SimHost.Tests | 0 | 170 | 0 |
| Bagira.SimHost.Integration.Tests | 0 | 28 | 0 |
| Bagira.Map.Common.Tests | 0 | 28 | 0 |
| Fdp.Tests | 0 | 691 | 2 |
...
### Q2: P9T2 — Bagira-Specific Lines Removed from `SubsystemOrchestrator`

The original `Bagira.Runner.Services.SubsystemOrchestrator.cs` was ~350 lines. The FDP replacement (`FDP.Framework.Runner.SubsystemOrchestrator.cs`) is 209 lines. The Bagira stub is now 7 lines (a `// forward-compat redirect` comment only).

**Bagira-specific code removed (~140 lines):**
...
4. **Mode-string parsing and config loading** (~40 lines): `BagiraRunnerConfiguration.ParseModeString()` logic, JSON config merging. Now lives in `BagiraRunnerConfiguration` in `Bagira.Runner`, not in the orchestrator.
...
```
Select-String -Path "Bagira.Runner\Program.cs" -Pattern "Raylib|ImGui|ImGuiNET"
```
...
- `Bagira.Map.Common` (MapConfig)
- `Bagira.Runner.Configuration` (BagiraRunnerConfiguration)
- `Bagira.Runner.Services` (concrete subsystem constructors)
...
**One issue discovered:** `WaitingRoomCoordinator` in the original `Bagira.Runner` referenced `SubsystemStatusAnnounce` from `Bagira.DDS.DataModel.Runner`. Moving `WaitingRoomCoordinator` to `FDP.Framework.Runner` while keeping the DDS message type in `Bagira.DDS.DataModel` would create a `FDP → Bagira` reference (forbidden).

**Solution:** Created a duplicate `FDP.Framework.Runner.SubsystemStatusAnnounce` struct with the **same DDS topic name** (`"SubsystemStatusAnnounce"`) and identical field layout. This preserves wire compatibility — WaitingRoomCoordinator instances in `FDP` processes and `Bagira` processes exchange the same DDS samples over the same topic. The Bagira `SubsystemStatusAnnounce` is retained for backward compat with any remaining Bagira consumers.
