...
|---------|--------|-------|
| INTS-P2-006 | ✅ Complete | Added `BagiraEnvironment` shared bootstrapper with `CreateTkb()`, `CreateGeoTransform()`, `CreateParticipant(int)` |
| INTS-P2-007 | ✅ Complete | `SubsystemOrchestrator.Initialize()` now forces `SimHost` headless when `IG` subsystem is present |
| INTS-P2-008 | ✅ Complete | `IgApplication.InitializeNetwork()` now uses `BagiraEnvironment` for TKB, geo transform, participant |
| INTS-P2-009 | ✅ Complete | `SimHostApp.OnLoad()` now uses `BagiraEnvironment` for TKB, geo transform, participant |
| INTS-P2-010 | ✅ Complete | `IosSubsystem.Initialize()` now uses `BagiraEnvironment.CreateParticipant(config.DomainId)` |
...
- `dotnet test Bagira.Map.Common.Tests/Bagira.Map.Common.Tests.csproj --no-build`
- `dotnet test Bagira.Runner.Tests/Bagira.Runner.Tests.csproj --filter "FullyQualifiedName~SubsystemOrchestratorTests|FullyQualifiedName~IosSubsystemTests"`
...
- [x] `BagiraEnvironment.CreateTkb()` returns populated TKB (`Tank_M1Abrams`, `Infantry_Rifleman` templates resolvable)
- [x] `BagiraEnvironment.CreateGeoTransform()` maps local origin `(0,0,0)` to Berlin origin within tolerance
- [x] `BagiraEnvironment.CreateParticipant(10)` creates participant with `DomainId == 10`
...
- [x] Existing `Bagira.IG.Tests`, `Bagira.SimHost.Tests`, and `Bagira.IOS.Tests` regression suites pass after refactors
...
Main issue was a circular project dependency when trying to reference `Bagira.Map.Definitions` directly from `Bagira.Map.Common` for `BdcTkbCatalog`. `Bagira.Map.Definitions` already depends on `Bagira.Map.Common` (for `TkbEntityTypes`), so direct reverse-reference caused restore/build failure. I resolved this by invoking `BdcTkbCatalog.RegisterAll(TkbDatabase)` via reflection inside `BagiraEnvironment.CreateTkb()`, preserving the required behavior without introducing a compile-time cycle.
...
I chose reflection-based invocation for `BdcTkbCatalog.RegisterAll` to satisfy both constraints: (1) keep `BagiraEnvironment` in `Bagira.Map.Common`, and (2) keep `CreateTkb()` responsible for catalog population. Alternative considered: move `BagiraEnvironment` into `Bagira.Map.Definitions` or add a new intermediate abstractions project. Both would be more invasive than required for this batch.

**Q4: What edge cases did you discover that weren't mentioned in the spec?**

If `Bagira.Map.Definitions` is absent from the runtime graph and `CreateTkb()` is called, registration cannot happen. This now fails fast with a clear `InvalidOperationException` rather than returning an empty TKB silently. Also validated that orchestrator behavior still keeps `IG` graphical when global mode is non-headless and only `SimHost` is auto-forced headless.
