...
| DDS2ECS-S2T3 | [x] | `dtWorldPos` adds `SimTransform` + `GeoTransform`, no raw DTO; updated tests. |
| DDS2ECS-S2T4 | [x] | `dtWorldPos` maps to `GeoVelocity`, no raw DTO; added tests. |
...
- [x] `DescriptorMapper` no longer emits DDS DTOs for `EntityMaster`, `EntityInfo`, `WorldPos`, `WorldPos`
- [x] `WorldPos` mapping produces `SimTransform` + `GeoTransform`
- [x] `WorldPos` mapping produces `GeoVelocity`
...
Updated `Hrot.ClusterRunner/Services/SimHostSubsystem.cs` to replace `AutoCycloneTranslator<EntityMaster>` with `EntityMasterEgressTranslator` so no SimHost code path uses the auto-translator. The alternative was to leave it for a later batch, but it contradicts the "no auto-translator" rule.
