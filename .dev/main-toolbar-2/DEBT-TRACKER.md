...
| DBT-A2 | P3 | T5 (BATCH-34) | The unified File menu adds Save/Save-As/Save-Scenario entries, but the **old `Scenario` menu Save/Save-As entries** (registered by `ScenarioMenuCommands`) are left in place to avoid `ScenarioMenuCommands` churn (kept intact per T4). Removing the now-duplicate Scenario-menu Save/Save-As is deferred polish (the scenario New/Load entries stay). | post-MTB2 | open |
| DBT-A3 | P2 | T7 (BATCH-36) | **New Asset dialog interactive UI deferred.** No generic `NewAssetDialog` ImGui renderer exists (only the blueprint-only `RecipeCreateModal`, now retired). T7 wires a functional pick→create→open with a **default name** (recipe name, or `New{Kind}` for "Empty"); the interactive **name + folder picker popup** for New is deferred (same class as main-toolbar-1 DBT-2 SaveAsDialog UI). User can rename via Save-As meanwhile. | post-MTB2 | open |
