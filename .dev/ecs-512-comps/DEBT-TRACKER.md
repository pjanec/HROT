...
| D006 | BATCH-03-REVIEW | `GetRecordableMask_ReturnsBitMask512_WithRegisteredBit` test only checks mask.IsEmpty() — does not verify a recordable component's bit is set. Register an actually-recordable component and assert its bit. | P2 | BATCH-04 | OPEN |
| D007 | BATCH-03-REVIEW | `QueryWithChangeDetection` reads cold metadata (LastChangeTick) before the hot mask check. Not a correctness issue but violates hot-first principle for change-detection queries. Future optimization. | P3 | BATCH-04 | OPEN |
