...
2. `.dev/ai-debug-api/reviews/ADA-BATCH-04-REVIEW.md` + `ADA-BATCH-05-REVIEW.md` (the real-reproduce gate; the
   "assert the spec, not impl-reach" lesson).
3. **Design:** `.dev/ai-debug-api/DESIGN.md` — "MCP Server (Node.js)" section + the "API Surface
   (shared HTTP ↔ MCP spec)" table (the authoritative endpoint list).
4. **Task detail:** `.dev/ai-debug-api/TASK-DETAIL.md` — ADA-PM-T01, ADA-PM-T02, ADA-PM-T03.
...
- `.dev/ai-debug-api/reports/ADA-BATCH-06-REPORT.md` (DEV-GUIDE format): built/installed, decisions/deviations,
