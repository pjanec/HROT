...
| ADA-06-D01 | ADA-BATCH-06 | MCP tools for Groups K (traces) and L (mutation) are absent — their HTTP endpoints are not yet built. Group G (breakpoints) landed in ADA-BATCH-07, Group H (checkpoint/restore/diff) in ADA-BATCH-08, Group I (recording/replay) in ADA-BATCH-10, Group J (logs) in ADA-BATCH-11. Tools will be added in the batch that implements each group's endpoints. Documented in tools/ai-debug-mcp/README.md. | P3 | per endpoint batch | PARTIALLY RESOLVED (G+H+I+J done; K/L pending) |
