...
| ADA-08-D02 | ADA-BATCH-08 review (lead) | **Pre-existing serialization defect, surfaced via diff/list.** The entity dump path does `JsonNode.Parse(JsonSerializer.Serialize(dump, FdpJsonOptionsRegistry.DefaultRelaxed))`; when any in-scope entity has a `NaN`/`Infinity` float, the write emits the named literal but `JsonNode.Parse` rejects it → `GET /entities` (list), `GET /entities/{id}` (dump), and `POST /diff/compare` all return `ok:false` (`"'N' is an invalid start of a value"`). Confirmed live: a freshly-spawned tkbType 1001 (CivilianPedestrian) entity triggers it; after restore (clean entity 1000 only) all work. Dates to BATCH-02; only now exposed (earlier 1001 spawns checked entityCount, never listed/dumped). **Fix = NaN/Infinity-safe serialization end-to-end (sanitize or `AllowNamedFloatingPointLiterals` on write + avoid the string round-trip via `SerializeToNode`).** Hardens the core read surface. | **P2** | ADA-BATCH-09 (corrective T0) | OPEN |
