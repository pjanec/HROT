...
| DEBT-004 | P1 | BATCH-02 review | `EyesAndMuscleSubsystem.cs` still instantiates `NedReplicationModule` directly (via old pattern). Namespace updated to `Hrot.Network.Replication` but `.WithReplication()` migration not done. Must be fixed in BATCH-03. | BATCH-03 | 🔴 Open |
...
| DEBT-006 | P3 | BATCH-02 review | `HrotNodeBuilder._replicationConfigured` and `_replicationRole` internal fields are now dead code after wrapper-type refactor. Remove in BATCH-03. | BATCH-03 | 🔴 Open |
