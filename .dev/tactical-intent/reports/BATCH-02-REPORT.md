...
|------------|----------|----------------------------------------------------------------------------------------|
| TASK-TI005 | Complete | `Commander = 1 << 4` added to `DoctrineCategory`; 2/2 tests passing                  |
...
TASK-TI006:
- [x] `DoctrineCatalog.GetValidDoctrines(MilitaryApc)` contains "DefendArea"
- [x] `DoctrineCatalog.GetValidDoctrines(CivilianCar)` does NOT contain "DefendArea"

TASK-TI004:
- [x] SC-1: Valid `BehaviorId` → `AssignTacticalIntentEvent` published with correct `IntentId`; no `AssignDoctrineEvent` emitted
...
| File | Purpose |
|------|---------|
| `Hrot/Engine/Hrot.Core/MapDefinitions/Doctrine/Intents/DefendAreaIntentDto.cs` | Intent DTO with `[DoctrineContract]` for auto-discovery (TASK-TI006) |
| `Hrot/Engine/Hrot.Core.Tests/DoctrineCategoryTests.cs` | Tests for TASK-TI005 (2 tests) |
...
| File | Change Summary |
|------|---------------|
| `Hrot/Engine/Hrot.Core/MapDefinitions/Doctrine/DoctrineCategory.cs` | Added `Commander = 1 << 4` after `AllMilitary` |
| `Hrot/Engine/Hrot.Core/MapDefinitions/Doctrine/DoctrineIds.cs` | Added `DefendArea_Intent = 1000` in new Tactical Intent DTOs section |
| `Hrot/Subsystems/Hrot.CGF/Systems/MissionAdapterSystem.cs` | Removed `_doctrineRegistry` and `_entityMap` fields and constructor params; added parameterless constructor; replaced `AssignDoctrineEvent` publication with `AssignTacticalIntentEvent`; merged jsonParams extraction and event publication into single `if` block; removed unused `Fdp.Toolkit.Behavior` and `Fdp.Toolkit.Replication.Services` usings; updated XML doc |
| `Hrot/Subsystems/Hrot.CGF/CgfLogicPack.cs` | Changed `new MissionAdapterSystem(doctrineRegistry, entityMap)` to `new MissionAdapterSystem()` |
| `Hrot/Subsystems/Hrot.SimHost.Integration.Tests/Infrastructure/SimHostInstance.cs` | Changed `new MissionAdapterSystem(_doctrineRegistry, _entityMap)` to `new MissionAdapterSystem()` |
...
A second decision point was the hash-change-detection logic refactor. The original code computed `currentDefHash` from both `jsonParams` and `phase.DoctrineId` in a single flat block after the task extraction. After removing the registry lookup and merging the publication into the task-extraction `if` block, the `else` branch (no `ActiveMissionPlan`) needed a separate (simpler) hash computed from `phase.DoctrineId` alone. This correctly preserves the "skip if nothing changed" behaviour for both code paths.
...
- `DoctrineCatalog` only maps three categories (`MilitaryApc`, `Infantry`, `Insurgent`) in its `BuildMap` static method. The new `Commander` flag (TI005) is intentionally excluded — but the exclusion is implicit (the categories array just doesn't include it). A short comment in `BuildMap` explaining why `Commander` is absent would prevent future confusion.
...
- **No `ActiveMissionPlan` at all:** If an entity is in the query (has `MissionPlanQueue` + `DoctrineState`) but has never had an `ActiveMissionPlan` set, `repo.GetComponent<ActiveMissionPlan>(entity)` returns `null`. The null check `activePlan?.Plan?.Tasks != null` handles this safely, falling through to the `else` branch that still updates `adapterState` without publishing an event.
...
- [ ] `DoctrineCatalog.BuildMap()` should have a comment explaining that `Commander` is intentionally excluded from the iterated categories.
...
- [ ] A future batch (TASK-TI011) will add a `DefendAreaMapper` that registers against the `TacticalIntentMapperRegistry` and maps `"DefendArea"` intents to a concrete `AssignDoctrineEvent`.
...
TASK-TI005: Add Commander = 1 << 4 to DoctrineCategory; excluded from AllMilitary
TASK-TI006: Reserve DoctrineIds.DefendArea_Intent = 1000; add DefendAreaIntentDto
            with [DoctrineContract(AllMilitary)] for auto-discovery by DoctrineCatalog
TASK-TI004: MissionAdapterSystem now publishes AssignTacticalIntentEvent instead of
            AssignDoctrineEvent; remove _doctrineRegistry and _entityMap (unused);
