# BATCH-06: Phase 5b - Doctrine Auto-Registration Completion

**Batch Number:** BATCH-06  
**Tasks:** MPM-P5-T04, MPM-P5-T05, MPM-P5-T06, MPM-P5-T07  
**Phase:** Phase 5 - Doctrine Auto-Registration (Part B)  
**Estimated Effort:** 7-9 hours  
**Priority:** HIGH  
**Dependencies:** BATCH-05 completed (DoctrineContractAttribute, DTOs, DoctrineSchemaDiscovery all exist)
...
2. Rebuild `DoctrineCatalog` from `[DoctrineContract]` attributes instead of hardcoded string arrays
3. Replace behavior-ID string literals in `CgfNodes.cs` AI tree JSON with DTO constants
4. Create `DoctrineTestHelper` and remove magic behavior-ID strings from test files
...
- `Hrot/Engine/Hrot.Presentation/Behavior/DoctrineSchemaDiscovery.cs` - the new auto-registration utility
- `Hrot/Subsystems/Hrot.CGF/Configuration/CgfDoctrineSetup.cs` - current manual registrations
- `Hrot/Engine/Hrot.Core/MapDefinitions/Tkb/DoctrineCatalog.cs` - current hardcoded string arrays
- `Hrot/Subsystems/Hrot.CGF/Brains/CgfNodes.cs` - contains TreeName string literals (lines ~239, 333, 357, 378, 598)
- `Hrot/Engine/Hrot.Core/MapDefinitions/Doctrine/` - all new DTOs and DoctrineIds.cs
...
BATCH-05 created:
- `DoctrineCategory`, `DoctrineContractAttribute`, `DoctrineIds` in `Hrot.Core`
- 9 DTOs in `Hrot.Core/MapDefinitions/Doctrine/` (4 full + 5 marker), all with `[DoctrineContract]`
- `DoctrineSchemaDiscovery.AutoRegister(BehaviorUiRegistry, ScenarioBehaviorRemapper)` in `Hrot.Presentation`

**Important context from BATCH-05 report:**
- The 3 existing param DTOs in `Fdp.Toolkits` (`FireAtTargetParamsJsonDto`, `MoveToLocationParamsJsonDto`, `FollowRouteParamsJsonDto`) cannot reference `Hrot.Core` types. The new Hrot.Core versions in `Hrot.Core/MapDefinitions/Doctrine/` are the decorated ones.
- `DoctrineIds.cs` in Hrot.Core has internal integer constants mirroring `CgfDoctrineIds.cs` in Hrot.CGF.
- `DoctrineCatalog.cs` has civilian doctrines `"WanderCivil"` and `"PanicFlee"` that have NO `[DoctrineContract]` DTO - these must be preserved in TASK 2 (DoctrineCatalog rebuild).
...
### Task 1: Replace BehaviorUiSetup and CgfDoctrineSetup Behavior-ID Strings (MPM-P5-T04)
...
Replace `CreateRegistry()` to use `DoctrineSchemaDiscovery`. Two approaches - pick the one that compiles cleanly:

Option A (preferred): Replace the body with `DoctrineSchemaDiscovery.AutoRegister(registry, remapper)` where a fresh `ScenarioBehaviorRemapper` is created and discarded. Return the registry.

Option B: Split `DoctrineSchemaDiscovery` to have a `RegisterUi(BehaviorUiRegistry)` overload that only does the UI part.
...
1. `RegisterAll()` - each `registry.Register(id, "BehaviorId", ...)` call has a raw string. Replace the string with the corresponding DTO's `BehaviorId` constant. 
   ...
   **Check first:** Does `Hrot.CGF` project reference `Hrot.Core`? (Check `Hrot.CGF.csproj`). If yes, use `Hrot.Core.MapDefinitions.Doctrine.MoveToLocationParamsJsonDto.BehaviorId` etc. If no, use `DoctrineIds` class constants from Hrot.Core or add the project reference only if it doesn't create a cycle.

   If `Hrot.CGF` cannot reference `Hrot.Core`, the alternative is to use local string constants (e.g., `private const string BehaviorId_MoveToLocation = "MoveToLocation"`) inside `CgfDoctrineSetup`. This is not ideal but avoids a bad dependency. Document your choice in the report.

2. `CreateBehaviorRemapper()` - currently calls `remapper.Register<FireAtTargetParamsJsonDto>("FireAtTarget")` using Fdp.Toolkits DTOs. Check if `DoctrineSchemaDiscovery` can be called from `Hrot.CGF` (it lives in `Hrot.Presentation` - check if Hrot.CGF references Hrot.Presentation). If yes, replace with `DoctrineSchemaDiscovery.AutoRegister(new BehaviorUiRegistry(), remapper)` or a dedicated overload. If no, replace the string literal with a constant as above.

**Verify after Task 1:**
- No magic behavior-ID string literals (like `"FireAtTarget"`, `"MoveToLocation"`, etc.) remain in `BehaviorUiSetup.cs` or `CgfDoctrineSetup.cs`.
- `dotnet build IOS-IG-SimHost.sln` passes.

---

### Task 2: Rebuild DoctrineCatalog Using Reflection (MPM-P5-T05)
...
**IMPORTANT - Read DoctrineCatalog.cs first!**

The current file has:
- `s_civilianDoctrines = ["WanderCivil", "PanicFlee"]` - these have NO `[DoctrineContract]` DTO
- `s_militaryApcDoctrines = ["ConvoyEscort", "MoveToLocation", "FollowRoute", "FireAtTarget"]`
- `s_infantryDoctrines = ["InfantryCombat", "MoveToLocation", "JoinFormation", "FireAtTarget"]`
- `s_insurgentDoctrines = ["Ambush", "MoveToLocation", "FireAtTarget"]`
- `s_defaultDoctrines = ["MoveToLocation", "FollowRoute", "JoinFormation", "Idle", "FireAtTarget"]`

The DESIGN.md approach (DESIGN.md § 5.6) builds lists dynamically from `[DoctrineContract]` attributes, then also handles `GetValidDoctrines(tkbType)` via a type-to-category mapping.

**Two-part implementation:**

1. **Reflection-based list for military/insurgent categories:** Build the four military/insurgent doctrine lists dynamically from assembly scanning using the `DoctrineCategory` flags. This replaces `s_militaryApcDoctrines`, `s_infantryDoctrines`, `s_insurgentDoctrines`.

2. **Preserve civilian list manually:** The civilian doctrines `"WanderCivil"` and `"PanicFlee"` do not have `[DoctrineContract]` DTOs and so cannot appear in reflection results. Keep `s_civilianDoctrines` as a hardcoded list for now, since adding DTOs for those civilian-only behaviors is out of scope. The `GetValidDoctrines` switch still handles `TkbEntityTypes.CivilianPedestrian` and `TkbEntityTypes.CivilianCar` with the preserved hardcoded list.

3. **Replace s_defaultDoctrines:** The default list can be built via reflection using `DoctrineCategory.None` or a special approach, OR keep it as a hardcoded fallback - your choice. Document in report.

Implement `BuildMap()` per DESIGN.md § 5.6, then wire `GetValidDoctrines` to use the reflection-built map for military/insurgent types. The switch case for civilian types uses the preserved hardcoded list.

**Important:** `DoctrineCatalog` is in `Hrot.Core` which already contains `DoctrineContractAttribute` - so no new project references needed.

**Verify:**
- `DoctrineCatalog.GetValidDoctrines(TkbEntityTypes.MilitaryApc)` still returns list containing `"FireAtTarget"`, `"MoveToLocation"`, `"ConvoyEscort"`, `"FollowRoute"`.
- `DoctrineCatalog.GetValidDoctrines(TkbEntityTypes.CivilianPedestrian)` returns `["WanderCivil", "PanicFlee"]` unchanged.
...
These DTOs live in `Hrot.Core.MapDefinitions.Doctrine`. **Check if `Hrot.CGF` references `Hrot.Core`** (see the `.csproj`). If yes, use `using Hrot.Core.MapDefinitions.Doctrine;` and reference `MoveToLocationParamsJsonDto.BehaviorId` directly.
...
Has `registry.Register<Fdp.Toolkit.Behavior.Params.FireAtTargetParamsJsonDto>("FireAtTarget")`. This test is registering a DTO directly - can replace `"FireAtTarget"` with `Hrot.Core.MapDefinitions.Doctrine.FireAtTargetParamsJsonDto.BehaviorId`.
...
**Verify after Task 4:**
- `DoctrineTestHelper.cs` exists and compiles.
...
## Completion Status
- [ ] MPM-P5-T04: Replace BehaviorUiSetup + CgfDoctrineSetup behavior-ID strings
- [ ] MPM-P5-T05: Rebuild DoctrineCatalog using reflection
- [ ] MPM-P5-T06: Update CgfNodes.cs TreeName strings with DTO constants
- [ ] MPM-P5-T07: Create DoctrineTestHelper + update test files
...
- [ ] No magic behavior-ID strings in `CgfDoctrineSetup.cs` (replaced with constants or DTO refs)
- [ ] `DoctrineCatalog.cs` military/insurgent lists built from reflection (no hardcoded strings for those)
- [ ] `DoctrineCatalog.cs` civilian list (`WanderCivil`, `PanicFlee`) preserved correctly
- [ ] No raw TreeName behavior-ID strings in `CgfNodes.cs`
- [ ] `DoctrineTestHelper.cs` created in Hrot.Core
...
- **Civilian doctrines:** `WanderCivil` and `PanicFlee` are NOT in the `DoctrineCategory` enum flags or any DTO. They MUST be kept as a hardcoded list. Do NOT remove them.
...
- **DoctrineCatalog s_defaultDoctrines:** The design doesn't specify how to rebuild this. If it's complex, keep it as a hardcoded list (it's a fallback, not a category with a matching entity type).
- **Don't add new project references.** If a test project can't reference Hrot.Core, leave those strings and note them.
- **BehaviorUiSetup cleanup:** After using DoctrineSchemaDiscovery, remove the unused `using Fdp.Toolkit.Behavior.Params` if applicable.
