...
**Group A (IT-BHU-A1 through A4):** IT-BHU-A3 adapted to use the same blob for both behaviors (since `ResetHsmComponents` does not update `MachineId`). This is correct per spec — the spec only requires `Terminated` cleared and `Phase == Idle` before the first tick, not a different machine.
...
**Group E (IT-BHU-E1 and E2):** System order (6 systems, correct types, no `HsmDamageBridgeSystem`) verified. Full 2-frame integration run (mobility-lost → final state → `BehaviorFinishedEvent`) passes.
