...
I would like to start developing complex AI behaviors, like for example the coordination of multiple vehicle entities - a group. I am not sure how to approach it. I guess we might need a logical entity playing the role of a group leader (the Commander) where the coordination behavior will run, and have some way of controlling the behavior of the subordinate group members - giving the "orders" and receiving "reports" or some other way of communication
 - forcing subordinates to execute a concrete doctrine with concrete parameters
...
- Doctrine: BTree, HSM, Hardcoded.
- BrainBlackboard: a 128-byte unmanaged memory bus.
- **Section on "Large data doctrines" (Snippet 10 in HROT or GUIDE):** When a doctrine requires massive data, you don't pollute the BrainBlackboard. You lean into DOD/ECS and define a dedicated ECS component.
...
- The user asks about "forcing subordinates to execute a concrete doctrine with concrete parameters". This sounds exactly like what the "Mission Control" layer does.
- Snippet 34 (FDP_117): `AssignDoctrineHashEvent` and `AssignDoctrineEvent`. MissionDirectorSystem reads from `MissionPlanQueue.Phases` and delegates writes to `DoctrineIngressSystem` via `AssignDoctrineHashEvent`.
...
- Network boundaries: Brain vs Muscle tiers. CGF (Brain) runs the doctrines. If subordinates are on different nodes, we use DDS. But wait, `JoinFormationExecutor` says "if `_entityMap.TryGetEntity(p.LeaderNetworkId, out var leaderEntity)`". This implies the leader entity must be replicated to the node where the subordinate's brain is running, or they all run on the same CGF node.
...
- **Forcing subordinates to execute a concrete doctrine with concrete parameters:** You use the **Mission Control system**. The Leader's doctrine logic can emit ECS events (or DDS messages if remote) to change a subordinate's `MissionPlanQueue`. Or use `AssignDoctrineEvent` locally.
...
- *Forcing Doctrines:* Explain the `MissionPlanQueue` and `MissionDirectorSystem`. Leaders can assign missions by modifying the subordinate's `MissionPlanQueue` or emitting a `MissionControlIntent`/`MissionControlRequest` over DDS.
...
1. **Forcing Doctrines:**
   ...
   Locally, you could push to the `repo.Bus` an `AssignDoctrineHashEvent` (though typically `MissionDirectorSystem` does this by reading `MissionPlanQueue`).
...
- **The Leader:** You spawn a logical (or physical) "Commander" entity. This entity is assigned a higher-level BTree or HSM doctrine (e.g., a "PlatoonCommander_BT").  
...
### 3. Forcing Subordinates to Execute Concrete Doctrines
...
Mission Control mechanism is a bit high level with the tasks/doctrine sequence and their pro-programmed. It is intended to be used with the leader as the top level behavior control. But the group member "steering" might benefit from a more direct approach - for example direct control of the doctrines for individual members; these doctrines could be assigned bu the leader according to leader's wishes (those requests sent from the actions executed within leader's HSM or BTree), while the leader keeps monitoring the state of his group members, possibly receiving some reports from them (reports published as entity component that is senced with the leader so the leader can access it), don't you think? 
...
### 1. Direct Doctrine Assignment (Top-Down Steering)
...
Instead, from within the Leader's `[SharedAiAction]`, you can directly publish an **`AssignDoctrineHashEvent`** (or `AssignDoctrineEvent`) to the local Event Bus:
...
public static NodeStatus Action_OrderSubordinate(ref Params dto, ref BehaviorTreeState state, ref BTreeContext ctx) {
    ...
    var doctrineHash = // ... hash of the "SuppressingFire" doctrine

    // Force the subordinate to immediately switch doctrines
    ctx.World.Bus.Publish(new AssignDoctrineHashEvent {
...
The engine's `DoctrineIngressSystem` listens to this bus. It will instantly tear down the subordinate's current doctrine and spin up the new one, seamlessly preempting the high-level mission plan.
...
if (report.UnderFire) {
    // Leader dynamically calculates a new doctrine for the squad...
}
```
...
For the Leader to directly publish an `AssignDoctrineHashEvent` to a subordinate, or cleanly read its `SquadMemberReport` component without latency, **both the Leader's Brain and the Subordinate's Brain must be simulated on the same CGF (Simulation) node.**
...
- Doctrines are **per-entity black boxes** driven by:
...
There is **no native “control other entity” primitive inside a doctrine**.

So group AI must be built using:

- ECS state (shared data)
- Events (control signals)
- Doctrine reassignment (hard control)
...
```
[ Commander Doctrine ]   <-- decision maker
          ↓
[ Coordination Layer ]   <-- ECS + events (your design space)
          ↓
[ Subordinate Doctrines ] <-- execution
```
...
## A. HARD CONTROL (doctrine forcing)

> “force subordinates to execute a concrete doctrine”

Mechanism:

- `AssignDoctrineEvent` / `AssignDoctrineHashEvent`
- processed by `DoctrineIngressSystem`
...
- ECS components
- subordinates read them inside their own doctrine

✔ Pros:

- continuous control
- scalable
- no churn in doctrine system
...
# 5. When to use direct doctrine forcing
...
```csharp
repo.Bus.Publish(new AssignDoctrineEvent
{
    Entity = subordinate,
    DoctrineName = "Retreat_BT",
    JsonParams = "{ ... }"
});
```
...
### ❌ Don’t spam doctrine reassignment
...
> “Leader directly steering subordinates via doctrines + shared state + reports”
...
- **Leader doctrine as decision engine**
- **Subordinate doctrines as interpreters**

### ⚠️ Use doctrine reassignment only for:
