...
|---------|--------|-------|
| DB-MOD1-22 | ✅ Complete | `IgSymbolOverride` ID moved from `GlobalComponentIds` (119) to `HrotComponentIds` (167). ID 119 freed in GlobalComponentIds with tombstone comment. |
| DB-MOD1-24 | ✅ Complete | `KinematicTranslatorPack` and `CognitiveTranslatorPack` confirmed implemented in `Hrot.SimHost.Network`. `NodeBootstrapper.BuildTranslators` uses them. Unit tests exist in `TranslatorPackTests.cs`. See design note below regarding `SimHostApp`. |

---

## 🧪 Testing Results

**Unit Tests Passed:** 182 / 182 (Hrot.SimHost.Tests — includes `HrotComponentIds_NoDuplicates`, `HrotComponentIds_AllInApplicationRange`, all TranslatorPackTests)  
...
- [x] `HrotComponentIds_NoDuplicates` — ID 167 does not clash with any existing entry
- [x] `HrotComponentIds_AllInApplicationRange` — ID 167 is in 160–199 ✓
...
| `simHostMod.GeoEgressTranslator` | Kinematic | Module-owned `WorldPosEgressTranslator` — see note below |
...
1. `CognitiveTranslatorPack.Create()` includes `WorldPosIngressTranslator` — which would receive every `WorldPos` message published by `WorldPosEgressTranslator` in the same process. This causes the ingress to overwrite `NetworkTransform.LastPosition` via the command buffer, which in turn can reset the position shadow used by the egress for dirty detection.
...
These loops are confirmed to break the `Hrot.ClusterRunner.Integration.Tests.SpawnMovingVehicleIntegrationTests` when the packs are applied naively to `SimHostApp`.

The correct context for using both kinematic and cognitive packs together is `NodeBootstrapper.BuildTranslators()`, which is used by `Hrot.ClusterRunner`'s multi-node distributed setup where no single process subscribes to its own publications. `SimHostApp` is a **standalone** app where this separation does not apply.

The packs are correctly implemented and tested. The `SimHostApp.OnLoad` translator section uses `SimHostModule`-owned translator instances for the kinematic (WorldPosEgress) and cognitive (MissionEgress) translators, which avoids the loopback issue.
...
Two integration tests in `Hrot.ClusterRunner.Integration.Tests` were **already failing before this batch started** and are unrelated to DB-MOD1-22 or DB-MOD1-24:
...
Both tests expect `SmartEgressUtil.MarkDirty` to be called in the drag handlers (`SimHostVisualization.OnEntityMoved` and `TestHook_SimulateDrag`). These calls are missing. Note that `WorldPosEgressTranslator` uses its own shadow-state position comparison (not SmartEgressUtil) for dirty detection, so the fix needs to ensure the drag path correctly invalidates the shadow state or that the `EgressPublicationState` path is added to `WorldPosEgressTranslator`. This is a separate bug fix not in the scope of BATCH-11.
