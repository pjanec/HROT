...
if (toolEl.GetString() == "AREA_AUTHORING")
{
    ...
    var pathTool = new PointSequenceTool(points =>
    {
        ...
        foreach (var pt in points)
        {
            ...
            geoPoints.Add(new GeoPosition { Latitude = lat, Longitude = lon, Altitude = alt });
        }
...
-   **Zero Duplicate Code:** You avoid rewriting the local Raylib click-capture and elastic line rendering logic.-   **Proper Projection:** Because `IgApplication` already owns the `_geoTransform` and `_createEntityDdsWriter` dependencies, the lambda safely captures them. It correctly converts the flat screen coordinates to `GeoPosition` (Latitude/Longitude) before dispatching the payload.-   **Global Commit:** The tool stays purely visual ("Local Prediction") and the callback handles the "Global Commit" to the DDS backbone seamlessly.
...
```
// Inside Bagira.Map.Common.Replication.Utils.DescriptorMapper.MapToComponents()
foreach (var d in descriptors)
{
    // ... existing dtGeoSpatial mapping ...
...
```
// Inside Bagira.SimHost.Setup.DemoTkbSetup.RegisterAll() (or wherever your templates are defined)
...
Add integration test to Bagira.Runner.Integration.Tests as needede to verify it all works. Keep fixing until tests pass.
