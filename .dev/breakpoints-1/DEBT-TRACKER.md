...
| D-BP-04 | BATCH-49 | `GraphEditorWindow.SetBreakpointManager` added but canvas right-click handler is still a stub; `BlueprintBreakpointMenuPopulator.PopulateNodeMenu` never reached via UI interaction | P3 | Backlog | OPEN (deferred: canvas placeholder not yet implemented; ImGui context required for right-click popup; full wiring deferred to canvas implementation batch) |
