...
| CF-2 | **Preserve authored node identity** — add `OriginNodeId` provenance through lowering; key probe to the block's owning exec node (not `Statements[0]`); DebugMap + `NodeEnter` use authored id; pure data nodes get no probe | CF-1 | 🔄 In progress (Zoo) | bp on Delay/Sequence (authored id) pauses sim |
