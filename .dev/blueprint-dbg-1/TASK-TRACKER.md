...
| CF-3 | **Reconcile tests + editor gating + cleanup** — fix probe/step count tests (old→new documented); gate canvas breakpoint toggle to DebugMap-eligible nodes; remove `DiagLog`/`bp-diag.log` | CF-2 | ✅ Done (lead) | bp only settable on exec nodes; Delay pauses; data node toggle disabled |
