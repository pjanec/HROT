...
| CF-7-rev | **Auto in-memory instrumentation on demand** — on first breakpoint/watch (or session restore), set asset `CompilerMode`=Debug/Trace + Quick Reload (reuses `QuickReloadService:64`); zero overhead until debugging; generator/Release untouched | CF-4 | ⬜ TODO (Zoo) | set bp (no Compile) → sim auto-instruments → pauses |
