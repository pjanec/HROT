...
| CF-8 | **Persist & restore debug session** — `DataBreakpointManager` is durable owner (reuse `OnHotReloadCompleted` remount + `IsBroken` pending); save node + data breakpoints (DTOs) + watches to user-local gitignored file (generalize `WatchPersistence` beyond `IsWatch`); restore → bind on load; stale-but-retained per BPF-003 | CF-4, CF-7-rev | ⬜ TODO (Zoo) | close/reopen editor → breakpoints+conditions+watches restored, pause with no Compile |
