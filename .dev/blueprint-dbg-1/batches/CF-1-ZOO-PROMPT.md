...
no snapshot regeneration, do not weaken tests). Then read the full batch spec: `.dev/blueprint-dbg-1/TASK-DETAIL.md`
...
2. From the compile result, write a Markdown report to
   `.dev/blueprint-dbg-1/reports/CF1-NODE-IDENTITY-REPORT.md` (also echo via `ITestOutputHelper`) containing:
...
- The new test passes; the file `.dev/blueprint-dbg-1/reports/CF1-NODE-IDENTITY-REPORT.md` is produced and
...
Write `.dev/blueprint-dbg-1/reports/CF1-REPORT.md` with: what you added, the exact `dotnet build`/`dotnet test`
