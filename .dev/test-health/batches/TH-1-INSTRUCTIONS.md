...
- **Fast-run filter (document in the ledger + a `.dev/test-health/README` note):**
  `dotnet test <proj> --filter "Stability!=Flaky&Stability!=Environment&Stability!=Broken"` → must be **0 failed**.
- **Ledger:** create `.dev/test-health/TEST-HEALTH.md` — a table: `Test | Suite | Bucket | Reason | Resolution/Target`.
...
- [ ] `.dev/test-health/TEST-HEALTH.md` ledger complete (every failing test → bucket + reason + resolution/target).
...
- [ ] Report at `.dev/test-health/reports/TH-1-REPORT.md`: buckets + counts, what was fixed vs marked, the filter
