...
| DEC-03b | NodeEditor core | **NEW node-context-menu seam** + BTree "Add Decorator →" right-click submenu (discoverable UX). No general node context-menu provider exists in NodeEditor today (only `ICustomElementContextMenuProvider`, RendererId-keyed). | ⬜ — split out of DEC-03; the picker path works without it but right-click is the expected UX. |
