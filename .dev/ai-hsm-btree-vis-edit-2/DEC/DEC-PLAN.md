...
| DEC-03c | NodeEditor (picker UX) | Picker shows a single flat "All" list (no category grouping, no icons); Tab does nothing while hovering a node (must move to empty canvas). User-reported UX gaps; right-click menu (DEC-03b) makes this non-blocking for decorators. | ⬜ — follow-up, lower priority. |
| DEC-04 | BTree host | EB-B pill appearance: glyph + param label (`↺×3`, `⏲2s`) | ⬜ — NOTE: showcase screenshot shows pills already render glyph+param (`! / R x3 / C 5s`); re-scope/confirm whether DEC-04 is still needed before doing it. |
