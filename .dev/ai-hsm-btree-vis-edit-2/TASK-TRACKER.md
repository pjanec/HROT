...
- [~] **TASK-BT-17** *(CRITICAL guarantee)* Generator symbol-check — resolve each bound method's signature vs the tree's blackboard/context; incompatible/unresolved → stub+BTREE0002 (covers Inspector + hand-edit, not just palette). Build can NEVER break from a binding. *(next)*
...
- [ ] **TASK-HS-02** Command sink: **delete state** (`ApplyRemoveNodes` full cascade) → [details](./TASK-DETAIL.md#task-hs-02--command-sink-delete-state)
