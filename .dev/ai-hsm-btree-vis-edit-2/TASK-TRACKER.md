...
- [x] **TASK-HS-07** Showcase `.hsm.json` + Starter recipe → [details](./TASK-DETAIL.md#task-hs-07--showcase-hsm--starter-recipe) *(BATCH-HS-07, verified+committed; rich showcase + Starter; guards null per VE-DEBT-004; **also fixed a latent HSM-codegen build-break** — HsmBridgeEmitCore emitted lambda→fn-ptr (invalid C#) for any bound action/guard; verified Hrot.AI.Behaviors builds 0 errors with the showcase; Hsm.Editor.Tests 454/0; layout → REVIEW-HS)*
