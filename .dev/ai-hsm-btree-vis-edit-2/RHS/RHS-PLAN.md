...
| RHS-05 | HSM projection | Parallel-state region dividers + headers render | — | TODO — root cause localized: `HsmAssetMapper.ToModel()` creates `RegionNode`s but never attaches them to the parent parallel state's `RegionNodes` (assembly-loader path does via `ParentStateIndex`; JSON DTO has no parent ref) → `Regions` empty → `ContainerRenderer.DrawRegions` skipped. Fix = attach each region to the parent of its `InitialChild`. (NodeEditor container renderer already draws dashed dividers + headers — no NodeEditor change needed.) |
