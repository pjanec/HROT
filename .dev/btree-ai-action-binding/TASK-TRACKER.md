...
- [x] **S3-3** ~~Runtime key resolution~~ **Emit the scope-aware baked const**: the thunk keeps its compile-time `const __slotKey`; for Behavior bindings it's derived from `(assetId, variableId)` so co-bound nodes share the slot. (Reduced from "runtime key" — see key-formula resolution.) [details](./TASK-DETAIL.md#s3-3--scope-aware-baked-const) — *done BATCH-15 (`ae6d711`): scope-aware key in BOTH `EmitStatefulActionThunks` (thunk const) AND `BTreeEmitCore.EmitAction` (topology blob key) in lockstep via shared `ResolveStatefulSlotKey`; the topology site was NOT in TASK-DETAIL's Touches but is required. Byte-identity 136/0; share-slot tick proof + Node regression green; T20 unchanged.*
...
- [x] **S3-5** `ClearBehaviorEvent` detach fix (design change #2): capture previous behavior id + call `DetachStatefulSlots` on clear, not only on switch (today leaks on clear-without-successor) [details](./TASK-DETAIL.md#s3-5--clearbehaviorevent-detach) — *done (`334e27a`): clear handler captures previous behavior id + detaches its manifest slots (scope-aware via S3-4). Clear_DetachesStatefulSlots_NoLeak + Switch_StillDetachesPreviousSlots green; Behavior namespace 142/142.*
- [x] **S3-6** Fix-3 guard extension (design change #3): flag two stateful nodes in concurrent HSM regions resolving to the same Behavior/Entity slot key (same scope+type under one asset); extend `HsmValidator.CheckConcurrentStatefulSubtrees` [details](./TASK-DETAIL.md#s3-6--fix3-scope-guard) — *done (`3f7f6ca`): new `ConcurrentSharedScopeKey` rule + injected `sharedScopeKeys` resolver (mirrors `_isStatefulSubtree`); catches different subtrees sharing a scope key that S2-4 misses. Sequential use stays valid. Subtree tests 9/9, Validation namespace 34/34.*
- [x] **S3-7** Monitoring (v1-mandatory): thread `Role`/`Scope` into `StatefulSlotInfo`; `StatefulWorkingStateProjection` groups/labels by scope; live read-only inspector shows the shared slot's current values [details](./TASK-DETAIL.md#s3-7--scope-aware-monitoring) — *done (`640ea26`): `StatefulSlotInfo` carries Role/Scope bytes (baked only when non-default → Node corpus byte-identical); projection labels `[Behavior]`/`[Node]`/`[Entity]`. StatefulSlotInfo_CarriesRoleAndScope + Inspector_ShowsBehaviorScopedSlot_LiveValues green; Generators 100/100, Presentation projection 6/6, byte-identity 136/0.*
