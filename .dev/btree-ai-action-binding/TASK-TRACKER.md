...
- [ ] **S3-2** Scope-aware slot key: `ComputeStatefulSlotKey(assetId, scope, nodeVisualId, variableId)` — Node=`(asset,node)` unchanged; Behavior=`(asset,variableId)`; Entity=`(variableId)`. All compile-time. Pure fn + unit tests [details](./TASK-DETAIL.md#s3-2--scope-aware-slot-key)
