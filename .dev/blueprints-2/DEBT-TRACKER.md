...
| D-03 | P3 | FIX1-BATCH-01 | `HsmEmitter.BuildMachineMetadata` reconstructs transition ordering independently from `HsmFlattener`. A mismatch would silently produce wrong Guid-to-index mappings. Fix: pass `FlattenedData` into `BuildMachineMetadata` directly instead of re-iterating the graph. | Future | OPEN |
| D-04 | P3 | FIX1-BATCH-01 | `MachineMetadata.ActionNames` ordering (alphabetical by Ordinal) diverges from `HsmFlattener.BuildActionTable` ordering. `ActionNames` is unreliable for stable action-to-index mapping. | Future | OPEN |
