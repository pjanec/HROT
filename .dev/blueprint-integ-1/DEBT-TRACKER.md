...
| DEBT-010 | BATCH-11 | **CycloneDDS 0.2.3 cross-assembly codegen break.** `GizmoMap.Contracts@0.2.3-gc40f8c1444` emits `Fdp::Toolkit::Diagnostics::Gizmos::PipelineTarget` IDL that Hrot.IG's 0.2.3 `idlc` cannot resolve → clean build of Hrot.IG fails. Isolated by reverting only GizmoMap.Contracts to 0.2.2 (restores clean build). **Conflicts with the user's "0.2.3 everywhere" request** — needs the DDS team to fix the 0.2.3 prerelease codegen before GizmoMap.Contracts can move to 0.2.3. Earlier 'clean' builds passed on stale generated IDL. | P1→user | — | OPEN |
