...
**`.dev/blueprint-finalize/BF-REBUILDREFRESH-REPRO.ps1`** (new)
- Scripted end-to-end repro that snapshots `.g.cs` hashes, modifies a `.bp.json` Literal value, runs `dotnet build` incrementally, and asserts the `.g.cs` hash changed. Usage: `pwsh -File .dev/blueprint-finalize/BF-REBUILDREFRESH-REPRO.ps1`
...
**File:** `.dev/blueprint-finalize/BF-REBUILDREFRESH-REPRO.ps1`
