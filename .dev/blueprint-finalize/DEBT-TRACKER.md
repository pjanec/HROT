...
| DEBT-BF-02 | TASK-TRACKER Phase 8 | DESIGN-DEBT.md DD-1..DD-4: ChannelCommand→per-action generalization (partly done AN4/AN7), rare pin-collapse watch, StructEdit param grid (folds into BB1). | P2 | BB1 | OPEN |
...
| DEBT-BF-04 | BATCH-BB1A review | HSM `ExpressionTargetField` (type-filtered picker) added to transitions/global-transitions only, NOT states. REVIEW-BB1's surface is "HSM state". A state has 4 action slots (Entry/Exit/Activity/Timer) → needs per-slot binding, not one field per state. | P2 | BB1B | OPEN |
