...
**Constraints:**
- Constructor must accept `BehaviorRegistry registry` (not optional).
...
- `Inject` must deserialize `DomainMissionPlan`, call
  `_registry.TryGetId(task.BehaviorId, out int behaviorId)` for each task, use
...
- Do NOT use `BehaviorRegistry` static methods; only the injected instance.
...
2. Unit test: call `Inject` with the DOM from above; assert entity has `ActiveMissionPlan`
   ...
   `Phases[0].BehaviorId` equal to the registry-resolved ID.
...
Each site must obtain a `BehaviorRegistry` instance and pass it to the translator constructor.
`BehaviorRegistry` is already constructed in SimHost and CGF contexts via
`CgfBehaviorSetup.CreateBehaviorRegistry()` (or equivalent); for `EditorBootstrap`, the
