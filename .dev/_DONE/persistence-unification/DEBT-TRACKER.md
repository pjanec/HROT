...
| PU-D11 | BATCH-06 | **RESOLVED BATCH-09 (PU-402).** flushAction now writes JSON for BTree/HSM via the Save-All delegates + collision guard + never-throw (was C#). Safe after migrating the only two editor-owned assets. End-to-end edit→reload latency = Phase 9 / manual smoke. | P2 | PU-402 | RESOLVED |
| PU-D12 | BATCH-07 | **PU-501 path-at-creation is blocked on PU-D06 (same root cause).** Writing `.btree.json`/`.hsm.json` at creation requires setting `SourceFilePath` to the `.json`, but the unchanged debounced `flushAction` (PU-D11) writes C# source to `SourceFilePath` on the next edit → it would overwrite the JSON. Safe only after the flushAction→JSON switch (PU-401). Also: BTree/HSM have NO new-asset creation flow today (assembly-reflection projections only). **BATCH-07 shipped PU-502 (collision guard).** **UNBLOCKED by BATCH-09:** assets are now JSON + flush writes JSON, so path-at-creation (write `.json` at creation + set `SourceFilePath`) is now safe. PU-501 ready to pick up. | P2 | (PU-501) | UNBLOCKED |
