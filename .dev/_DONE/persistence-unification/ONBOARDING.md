...
**Ordering:** this thread runs **AFTER** Thread 2 (`.dev/_DONE/blueprint-finalize/ONBOARDING.md` — blueprint finalization + DEBT-MVE-003). Rebase on the latest `blueprint-integ-1` before starting; Thread 2's blueprint-runtime/registry changes land first.
...
- **Batch workflow:** `.dev/_DONE/persistence-unification/batches/BATCH-XX-INSTRUCTIONS.md` → sonnet coder → review → `.dev/_DONE/persistence-unification/reports/` → commit per batch (write `.git/PUxx_MSG.txt`, `git commit -F`). End messages with `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`. Exclude `Stride/` + `no-tests-HROT.Engine.dumpfilter`.
