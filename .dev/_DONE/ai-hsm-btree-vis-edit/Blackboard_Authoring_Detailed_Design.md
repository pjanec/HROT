...
> **Status:** Detailed design, derived from `AI_Editor_Shared_Infrastructure.md` + `BTree_Editor_NodeEditor_Host_Design.md` + `HSM_Editor_NodeEditor_Host_Design.md` + the `design-talk.md` brainstorm sessions.
> **Audience:** Implementation agent and human reviewer.
> **Drives:** Visual blackboard authoring across BTree and HSM editors. Adds a Blackboard Variables panel, recursive sub-tree aggregation, opt-in DTO aliasing, field-level synchronization, and editor-owned C# DTO emission that round-trips comments and tolerates user-introduced exotic fields read-only.
...
- **C# is the source of truth.** No sidecar files, no editor-private state files. The .cs blackboard DTO file is what the editor loads, what the editor saves, what gets committed to git, what the source generator and runtime consume.
