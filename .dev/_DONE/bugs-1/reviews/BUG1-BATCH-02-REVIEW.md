...
While checking the result based on QA feedback: *"in IOS if mission task MoveToLocation is created and committed, the IOS starts rendering the target point as expected but the vehicle does not start moving at all"*, a missed defect was found in how the `BehaviorFinished` string trigger is parsed.

This bug resides on the `SimHost` side in `MissionControlRequestSystem.cs`. Due to the missing `"BehaviorFinished"` case within its string-parser `switch` block, it falls back to the default `TimerElapsed` case with a param of `0f`. The vehicle stops instantly thinking the phase is finished, as the elapsed time 0 matches the criteria.
...
Adds a togglable Continuous Drag Mode updating WorldPos position at 10Hz during Map moves. Corrects optimistic concurrency sync failures after mission abort executions and successfully defaults BehaviorFinished trigger to all generic Mission tasks.
