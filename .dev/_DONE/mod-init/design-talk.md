...
-   The `SimHostApp`, `IgApplication`, and `CgfApplication` classes now completely own their network bootstrapping via `HrotNodeBuilder`.-   They inject their own domain-specific systems (like `BehaviorRegistry` or `RoadNetworkBlob`) directly into the generic builder output without relying on the orchestrator to glue them together.
