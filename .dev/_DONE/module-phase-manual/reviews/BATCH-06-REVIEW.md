...
### MPM-P5-T04: Replace BehaviorUiSetup + CgfDoctrineSetup behavior-ID strings - APPROVED

`BehaviorUiSetup.CreateRegistry()` now uses `DoctrineSchemaDiscovery.AutoRegister`. Option A (throwaway remapper) is clean and correct. Unused import removed.

`CgfDoctrineSetup.RegisterAll()` string literals replaced with DTO `BehaviorId` constants. The transitive Hrot.Core accessibility via Hrot.Common is verified by the successful build. `DoctrineDefinition.Name` fields left as-is per minimize-diff rule (they are display names, not registry keys). `CreateBehaviorRemapper()` now uses `DoctrineSchemaDiscovery.AutoRegister` registering all 9 DTOs (registering extra harmless ones is a no-op at remap time).

### MPM-P5-T05: Rebuild DoctrineCatalog Using Reflection - APPROVED

Military/insurgent lists correctly built via `BuildMap()` from `[DoctrineContract]` assembly scan. Civilian list (`WanderCivil`, `PanicFlee`) and default fallback preserved as hardcoded.
...
### MPM-P5-T07: Create DoctrineTestHelper + Update Test Files - APPROVED

`DoctrineTestHelper.cs` created correctly. Test files in `Hrot.Presentation.Tests` updated (project already references Hrot.Core). `Hrot.SimHost.Tests` and `Hrot.Network.NED.Tests` left unchanged (no Hrot.Core reference - correct decision per instructions). The NED test strings are network-protocol test data representing wire format values, not doctrine domain objects.
...
No new debt items. The DoctrineIds duplication noted in BATCH-05 review remains as a low-priority item. The `Hrot.SimHost.Tests` and `Hrot.Network.NED.Tests` test strings are acceptable (network protocol strings are by definition ground truth).
