...
**`.dev/_DONE/blueprint-dbg-2/batches/BPDBG-PERNODE-PROBES-INSTRUCTIONS.md`** — run it on **sonnet** (high blast radius:
...
## 3. Backlog — `.dev/_DONE/blueprint-dbg-2/DEBT-TRACKER.md` (single source of truth)
...
- **Never commit user `.bp.json` experiments** (e.g. `Count5.bp.json`) — they appear in the working tree; exclude them from every commit. `.dev/_DONE/blueprint-dbg-1/reports/CF1-NODE-IDENTITY-REPORT.md` is an unrelated pre-existing edit — leave it out too.
...
## 7. Plan/tracker docs
`.dev/_DONE/blueprint-dbg-2/`: `PLAN.md`, `TASK-TRACKER.md`, `DEBT-TRACKER.md`, `batches/`, `reports/`, `reviews/`.
