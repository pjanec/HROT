...
- **Test** read (`SubTickRestoreRegistrationTests.cs`): genuinely reproduces the crash — `Assert.Throws<InvalidOperationException>` on the OLD `SyncFrom(repo)` + `RestoreTo` path, mask-level proof that `GetSnapshotableMask(false)` excludes the NoPreview type, and positive proof the NEW `includeTransient:true` seeding restores correctly (entity alive, `NormalInt==99`, `NoSnapshotProbe.V==42`). Would fail without the fix.
