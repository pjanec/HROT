...
**Onboarding:** `.dev/.guides/DEV-GUIDE_claude.md`; `docs/blueprints/Blueprint_Subsystem_Debug_NodeGranularStepping_Addendum.md` §3.4; `.dev/_DONE/blueprint-dbg-2/reviews/BF-03-REVIEW.md`. Corrects BF-03 (`134eb197`).
...
## Constraints
Touch `BlueprintDebugSession.cs` + test files only. Do NOT change CF-6/ExecSuccessors semantics, do NOT commit any `.bp.json`, do NOT suppress diagnostics or weaken tests. Do NOT commit. Report → `.dev/_DONE/blueprint-dbg-2/reports/BF-04-REPORT.md` (the end-of-tick target logic, how the first-node temp BP is computed, the discriminating test + exact landing node asserted, which prior assertions changed + why, test counts). The lead reviews and commits.
