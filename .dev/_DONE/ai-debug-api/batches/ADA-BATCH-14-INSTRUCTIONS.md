...
2. `.dev/_DONE/ai-debug-api/reviews/ADA-BATCH-12/13-REVIEW.md` (the live gate; the false-green-verify recurring
   failure — RE-RUN `npm run verify` to a real tally; assert the real symptom, not the happy path).
3. **Design:** `.dev/_DONE/ai-debug-api/DESIGN.md` — Group F (commands/discovery + focus); the ADA-04-D02 note.
4. **Task detail:** `.dev/_DONE/ai-debug-api/TASK-DETAIL.md` — ADA-P1-T06b (in TASK-TRACKER), ADA-P9-T01.
...
- `.dev/_DONE/ai-debug-api/reports/ADA-BATCH-14-REPORT.md` (DEV-GUIDE format): built, decisions (managed-event
