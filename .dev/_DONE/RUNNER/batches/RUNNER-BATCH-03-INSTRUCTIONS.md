...
**The Goal:** Refactor all subsystems to function as libraries that can be embedded in `Runner.exe` or wrapped in thin standalone console applications (e.g. `Hrot.SimHost.Standalone`).
...
- **Primary Work Areas:** `Hrot.SimHost`, `Hrot.IG`, `Hrot.ExCon`
- **New Standalone App Projects:** `Hrot.SimHost.Standalone`, `Hrot.IG.Standalone`, `Hrot.ExCon.Standalone`
...
3. Create project `Hrot.SimHost.Standalone` with `Program.cs` utilizing CommandLineParser to run `SimHostSubsystem` manually.
...
3. Create `Hrot.IG.Standalone` wrapper.
...
4. Create `Hrot.ExCon.Standalone` wrapper.
