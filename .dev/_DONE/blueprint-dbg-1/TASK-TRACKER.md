...
| CF-3 | **Reconcile tests + editor gating + cleanup** — fix probe/step count tests (old→new documented); gate canvas breakpoint toggle to DebugMap-eligible nodes; remove `DiagLog`/`bp-diag.log` | CF-2 | ✅ Done (lead) — but PARTIAL (see CF-4 review) | bp only settable on exec nodes; Delay pauses; data node toggle disabled |
| CF-4 | **Exec-only, block-granular breakpoints** — probes keyed to exec owners only (no data fallback); node→block-probe map in DebugMap; `SetBreakpoint` translates clicked node→block probe (marker stays on clicked node); `IsNodeBreakpointable` true iff exec; tighten loosened tests | CF-3 | ✅ Done (Zoo) | bp on SetVariable pauses; Get Count toggle disabled; no silent-dead bp |
| CF-5 | **Step/Resume buttons in Blueprint Tools panel** — surface Continue / Step Over / Into / Out (enabled when paused) in the merged Blueprint Tools window, wired to session step/continue methods | C | ✅ Done (Zoo) | hit bp → press Continue/Step → sim resumes/advances one tick |
| CF-6 | **Real stepping (temp breakpoint on next node)** — compute next exec successor(s) from graph; one-shot temp breakpoints (invisible, via CF-4 translation); suppress origin/user bps during step pass; resume not single-tick; replace `_stepMode` tick-matching | CF-4, CF-5 | ✅ Done (lead) | Step moves executing-node cursor to next node, re-pauses there |
| ~~CF-7~~ | ~~Debug-instrument the dev build (generator)~~ — **SUPERSEDED** by CF-7-rev + CF-8 (debugging is editor-only → instrument in memory, don't bake into the build) | — | ❌ Superseded | — |
| CF-7-rev | **Auto in-memory instrumentation on demand** — on first breakpoint/watch (or session restore), set asset `CompilerMode`=Debug/Trace + Quick Reload (reuses `QuickReloadService:64`); zero overhead until debugging; generator/Release untouched | CF-4 | ✅ Done (lead) | set bp (no Compile) → sim auto-instruments → pauses |
| CF-8 | **Persist & restore debug session** — `DataBreakpointManager` is durable owner (reuse `OnHotReloadCompleted` remount + `IsBroken` pending); save node + data breakpoints (DTOs) + watches to user-local gitignored file (generalize `WatchPersistence` beyond `IsWatch`); restore → bind on load; stale-but-retained per BPF-003 | CF-4, CF-7-rev | ✅ Done (lead) | close/reopen editor → breakpoints+conditions+watches restored, pause with no Compile |
| CF-9 | **Debug DD addendum** (`DEBUG-DD-ADDENDUM.md`) — design of record, written FIRST; CF-6/7-rev/8 implement against it. Remaining: reconcile with deltas after those land | — | 🟡 Drafted (LEAD); reconcile after CF-6/7-rev/8 | n/a (doc; feeds NotebookLM) |
