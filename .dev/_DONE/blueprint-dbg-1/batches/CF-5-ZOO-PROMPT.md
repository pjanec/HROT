...
test to make it pass**, never regenerate snapshots). Then read the full spec: `.dev/_DONE/blueprint-dbg-1/TASK-DETAIL.md`
...
Write `.dev/_DONE/blueprint-dbg-1/reports/CF5-REPORT.md`: what changed, exact build/test command lines + results, full failing-test set by name (before/after). Do not weaken/delete tests, do not regenerate snapshots. If blocked, STOP and report. The lead reviews the **diff** (not the report) and commits.
