...
| DBT-A2 | P3 | T5 (BATCH-34) | Duplicate Scenario-menu Save/Save-As vs the new File menu. **DONE (d1ef98e4, BATCH-53)** — user approved retirement; scenario.save/saveAs removed from `ScenarioMenuCommands` (menu now New/Load/Migration History); File→Save still handles scenarios via the `shell.save` scenario branch; ShellSaveCommands still registers the commands. | BATCH-53 | ✅ |
...
| DBT-A1 | P3 | T7 (BATCH-36) | **DONE (d1ef98e4, BATCH-53)** — user approved: deleted `RecipeCreateModal` + `AssetNameFolderModal` (+ its tests). **`NewFromRecipeService` KEPT** — still live in `BlueprintNewAssetService`. | BATCH-53 | ✅ |
...
| DBT-A5 | P3 | **Save does not gray out after saving** (Changed/dirty flag). Save `IsEnabled` ignores dirtiness, and the editable-asset adapters hard-code `IsDirty=false`, so real dirty-tracking (mark dirty on canvas edits, clean on save) is needed to disable Save once saved. **USER (2026-06-12): fix IF feasible to detect asset changes for all 4 asset types (Blueprint/BTree/HSM/Scenario) — non-critical, keep in tracker.** Needs per-kind change detection wired to `IEditableAsset.IsDirty` (+ `MarkClean` on save, already called). | future | OPEN (to-fix-if-feasible) |
...
| BUG-A24 | P2 | **Feature:** main-toolbar **Compile/Reload** (Blueprint) + **global Full Rebuild** icons. | BATCH-56 | **DONE (8e2212de)** — `blueprint.compileReload` (IconKey `build/compile`=lightning, enabled for active Blueprint) + `blueprint.fullRebuild` (`build/rebuild`=refresh, always-on), toolbar sortOrder 50/51 right of debug. SilkIconProvider keys added; Hrot.Editor.AiShared builds clean. **Finding:** BTree/HSM have NO quick-reload (JSON-owned, recompiled on Full Rebuild) → Compile/Reload is Blueprint-only by design; Full Rebuild covers all kinds. Hrot.Editor full-build verify pending the generator fix. |
| OBS-A1 | — | **RESOLVED (user, rebased):** the New Empty BTree/HSM source-generator `AssetId` build break was fixed by the user's BTree compiler fixes; our commits were rebased on top. `Hrot.Editor` now builds 0/0 and `Hrot.Editor.Tests` 185/0 → **A23 + A24 EditorSubsystem wiring is now build-verified.** | — | ✅ |
| OBS-A2 | — | **CORRECTION to A24's note:** BTree/HSM **can** hot-reload in-memory like blueprints — `BTreeEmitCore.Emit`/`HsmEmitCore` are runtime-callable (the source generators only wrap them) and the ALC/Roslyn/coordinator infra is general. The only reason it's not wired is that `InMemoryRoslynCompiler`/`BlueprintRegistrarScanner`/`AiHotReloadCoordinator` (internal to `Hrot.Blueprints.Editor`) would need extracting to a shared assembly + per-kind `*QuickReloadService`. **NOT our workstream** — the user is handling BTree/HSM editing + hot-reload in the other thread (`IOS-IG-SimHost-FDP-3/.dev/ai-hsm-btree-vis-edit-2` Phase B). Toolbar Compile/Reload stays Blueprint-enabled; gate widens 1-line when their services land. | — | tracked (other thread) |
