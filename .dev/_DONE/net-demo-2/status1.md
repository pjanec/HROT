...
3. DemoScenarioSetup.cs — doctrine registry setup and entity spawning via SpawnEntityCommand
...
2. Directly activates CgfDoctrineIds.WanderMilitary\_BT on the CGF entity's DoctrineState \(bypasses ExCon but exercises the full pipeline\)
...
1. Registers a custom PatrolAndEngage\_BT \(ID 3099\) doctrine using TestHook\_DoctrineRegistry — a Selector BTree with Condition\_HasTarget → Action\_AimAndFire over Action\_Wander fallback
2. Activates the doctrine on the CGF entity
