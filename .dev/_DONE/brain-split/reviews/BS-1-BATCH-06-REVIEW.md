...
- Implemented **BS1-T022**: Updated `MissionDirectorSystem` to utilize `BehaviorFinished` mechanism instead of raw `NavState` polls. Added `[Obsolete]` annotation to `MissionTrigger.ReachedDestination` enum values to maintain DDS serialization backwards-compatibility. Updated UI to match.
...
- **TD-13**: Refactor `EntityMissionEgressTranslator` and `SimHostInstance` to use `BehaviorFinished` over `ReachedDestination`. Target: Next general Debt Burndown batch.
...
- Replaced MissionDirectorSystem ReachedDestination trigger with BehaviorFinished (BS1-T022)
