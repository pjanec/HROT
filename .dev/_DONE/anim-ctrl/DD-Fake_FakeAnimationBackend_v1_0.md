# DD-Fake — FakeAnimationBackend Implementation — Detailed Design (v1.0)

> **Status:** Detailed design for the initial `IAnimationBackend`
> implementation: a fake/mock backend that processes animation state
> deterministically without any 3D rendering. Companion to the five
> approved animation control design documents (mini design v0.3, DD-1
> through DD-5). This is an implementation choice for the Muscle
> Character node, not a contract change to the animation architecture.
...
### 7.3 Window registration

The window registers via whatever pattern the engine's existing
diagnostic windows use (see §11.4). The expected shape based on the
engine's existing patterns:
...
public sealed class FakeAnimBackendInspectorWindow : IDiagnosticWindow
{
    ...
    public void Draw(IDiagnosticContext ctx)
    {
        ...
        {
            if (_selectedEntity == default) DrawListView(ctx);
            else DrawDetailView(ctx, _selectedEntity);
        }
        ImGui.End();
    }
    // ...
...
The exact interface name (`IDiagnosticWindow`?) and registration call
need confirmation — §11.4.
...
DD-1's contract components (`AnimationChannel`, `LookAtChannel`,
`StanceIntent`, `StanceStatus`, `AnimationMontageQueue`,
`AnimationMontageQueueState`, `CharacterAnimationDefRuntime`,
`AnimationExecutorState`, `LookAtExecutorState`) need ComponentIds.
This DD adds `FakeAnimBackendState`. Future Stride backend adds
`StrideAnimBackendState`.

That's roughly 11 component IDs needed, plus headroom for future
animation components. Request: **allocate a reserved block** in
`GlobalComponentIds.cs` for animation, analogous to the 8000-8099
block allocated for animation event IDs in DD-3. A block of 20-30
IDs would comfortably cover known needs and future expansion.

Architect needs to pick which range — the engine's existing
allocation table (per project docs) has reserved blocks for FDP
Core (0-19), toolkit expansion (20-49), replication (50-79), IG
(80-109), module host (110-139), app descriptors (140-159 / 160-199),
and reserved 200-255. None of those is obviously animation; need
either a fresh allocation within a still-flexible range or
extension of one of the existing blocks.

### 11.2 Diagnostic window registration mechanism

The engine has standard diagnostic windows (ImGui-based, registered
with the WindowManager). What's the exact pattern for registering
a new one?

Specifically:
- Is there an `IDiagnosticWindow` or `IDevToolWindow` interface to
  implement?
- Where does the registration call live — startup wiring,
  module-loading hook, attribute discovery, manual call in some
  Muscle node init?
- Is there a category/menu convention so the fake window appears
  under an "Animation" submenu of the dev tools menu?

DD-Fake §7.3 sketched a plausible shape; needs confirmation against
the engine's actual conventions.

### 11.3 `RawNotifyEvent.PayloadVector` zero-from-fake convention

§5 specifies that the fake leaves `PayloadVector = 0` on footstep
events because it doesn't know the entity's world position.
`NotifyEventEmitterSystem` (DD-1 §11) fills in `WorldPosition` from
the entity's `SimTransform` when translating to the typed
`FootstepEvent` (DD-3 §3.2). This is a small implicit modification
to DD-1 §11's responsibility.

Confirm this works: the emitter system has access to the entity
reference and `SimTransform` is on the entity, so resolution is
trivial. But it does mean the emitter is no longer a pure-translate
function — it enriches. Acceptable, or should the fake somehow
emit a complete event?

(My proposal: acceptable. The enrichment is conceptually "translate
backend-internal event to engine event" which fits the emitter's
existing role of "translate RawNotifyEvent to typed event with
Target = self." Adding WorldPosition is the same pattern.)

### 11.4 Strict-oracle stance — relaxed?

DD-1 was written with the "strict oracle" framing — the fake's
behavior would be authoritative when Stride lands. Per your
clarification (Q5), the fake is a best-effort approximation, not
an oracle. This DD reflects that: §10 lists what the fake doesn't
do, §1 Principle 1 states the goal.

Confirm that's the intended stance. The implication is that
cross-backend test parity is not a goal — tests against the fake
verify AI behavior, tests against Stride verify visual behavior,
and divergences between them are expected (not bugs in either).

---

## Summary

DD-Fake specifies the initial implementation of `IAnimationBackend`:
a fake backend that processes animation state deterministically
without 3D rendering. All per-entity working state lives in a single
Tier 1 unmanaged ECS component (`FakeAnimBackendState`, ~1 KB) using
`[InlineArray]` for slot table and notify buffer, with a 64-bit
bitmask per slot tracking fired notifies. The backend class is thin
— mostly handle-table bookkeeping; the tick algorithm reads/writes
the state component. Synthetic footstep events emit at
distance-based cadence when grounded and moving. Hard-assert on
notify buffer overflow catches algorithmic bugs immediately. An
ImGui diagnostic window with list + detail views and clipboard JSON
snapshot export provides developer-facing introspection. AAR
recording integration is free because the state is a regular Tier 1
component.

Four open questions for architect review (§11): ComponentId
allocation block, diagnostic window registration mechanism,
`PayloadVector` enrichment by `NotifyEventEmitterSystem`, and
confirmation of the relaxed best-effort-approximation stance.
