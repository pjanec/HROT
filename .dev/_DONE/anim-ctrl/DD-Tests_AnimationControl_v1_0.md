# DD-Tests — Animation Control Test Strategy — Detailed Design (v1.0)

> **Status:** Detailed design for the test pyramid validating the
> animation control mechanism end to end. Companion to the approved
> animation control design documents (mini design v0.3, DD-1 through
> DD-5, DD-Fake). Designed for **autonomous AI-agent-driven
> implementation** — every test is specified with explicit assertion
> points, explicit waits with timeouts, explicit test data setup, and
> explicit non-goals.
...
```csharp
public sealed class AnimationIntegrationFixture : IDisposable
{
...
public static class PumpExtensions
{
    ...
    public static void PumpUntil(
        this AnimationIntegrationFixture fix,
        Func<bool> condition,
        ...
        string conditionName = "(unnamed condition)")
    {
        ...
        // Timeout — fail the test with diagnostic info.
        var diagnostic = fix.DumpAnimationDiagnostics();
    ...
    public static void PumpFrames(this AnimationIntegrationFixture fix,
                                   int frames,
                                   float dt = DefaultDt)
    {
...
The diagnostic dump on timeout is critical for agent-driven
debugging — the agent sees the full state of the system at the
moment the timeout fired, not just "timeout occurred."
...
### 7.1 `WriteParams<T>` — unsafe blob write helper
...
test-side intent authoring uses this helper.

### 7.2 `IssuePlayMontage` — common command helper
...
### 7.3 `DumpAnimationDiagnostics` — failure-mode diagnostic

Called by `PumpUntil` on timeout. Walks all humanoid entities,
serializes their full animation state (channel, queue, queue state,
stance, aim, fake backend state) as JSON. The same JSON format
DD-Fake §8 specifies for the diagnostic window's clipboard export
— same code, called from a different surface.

The agent sees this in the test failure message and knows exactly
what state the system was in when the test failed.
...
§5.1 proposes one `AnimationIntegrationFixture` shared across all
eight scenarios via `IClassFixture`. This minimizes bootstrap cost
but means tests can theoretically pollute each other if
`ResetWorld()` misses a component. Alternative: one fixture per
test (clean but expensive).

DD-Tests leans shared fixture with strict `ResetWorld()`. Confirm,
or prefer per-test bootstrap?

### 11.2 `CharacterAnimationDefRuntime.BakeForTest` test seam

§8 references a `BakeForTest` method exposed via
`[InternalsVisibleTo]`. The TKB translator (DD-4 §4) does the
baking in its `Inject` method. Is exposing a test-only baking entry
point acceptable, or should tests instead spin up the full TKB
translator machinery to bake?

Option A (lean): expose `BakeForTest` as internal seam. Simple.
Option B (strict): tests use a `TkbDatabase` with the DTO and
trigger the translator normally. More realistic, more setup.

DD-Tests leans Option A — tests should test the systems being
tested, not exercise the full TKB pipeline for every animation
scenario. The DD-4 translator gets its own dedicated tests.

### 11.3 `PumpUntil` location

The helper is generic enough that it could live in a shared test
utility project rather than the animation test project
specifically. Other future integration tests (replication, etc.)
will need similar helpers. Confirm where the helper class should
live.

### 11.4 Integration of fake backend as default for development

The DD-Fake assumes the fake backend is used during early
development. Should `AnimationIntegrationFixture` always use the
fake, or should it be parameterized so that when the real
Stride backend exists, the same eight scenarios can run against
it (with the strict-oracle stance abandoned, just verifying
"pipeline still functional")?

DD-Tests leans always-fake. The fake is the test oracle; cross-
backend parity is explicitly not a goal per DD-Fake §1
Principle 1. A separate `StrideBackendSmokeTest` can verify the
real backend boots and the pipeline doesn't crash without
requiring scenario-level parity.

---

## Summary

DD-Tests specifies a three-layer test pyramid for the animation
control mechanism: ~18 unit tests for `FakeAnimationBackend`
itself, ~11 system tests for individual Muscle ECS systems, and
8 networkless integration tests for the full pipeline. Each test
is explicit about its inputs, outputs, assertions, and timeouts
for autonomous AI-agent-driven implementation. The
`PumpUntil(predicate, maxFrames)` helper with mandatory frame
budget prevents test hangs; on timeout, a JSON diagnostic dump
shows the full pipeline state to enable rapid agent debugging.
Inline hand-rolled TKB data keeps tests self-contained and
focused. Stage-2 networked variants are explicitly deferred.

Four open questions in §11 for review; none are
implementation-blocking, all are test-strategy refinements.

---

*End of DD-Tests v1.0.*
