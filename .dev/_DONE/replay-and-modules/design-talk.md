...
-   **FireProcessingSystem**: Consumes `WeaponFireIntent` events to physically spawn new bullet entities into the world.-   **BehaviorIngressSystem**: Consumes `AssignBehaviorEvent` intents to mutate entity behaviors and reset Brain states.-   **CycloneNetworkIngressSystem**: Polls the network for incoming live intents.
...
-   **BehaviorIngressSystem**: Consumes `AssignBehaviorEvent` messages to update AI blackboards and behavior states.-   **FireProcessingSystem**: Consumes `WeaponFireIntent` events to spawn new bullet/projectile entities into the world.-   **PersonalRouteAuthoringSystem**: Consumes operator UI clicks (`CmdAppendPersonalWaypoint`) to add waypoints to a vehicle's route plan.
