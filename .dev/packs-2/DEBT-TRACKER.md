...
| DEBT-05 | ✅ P2 | No standalone unit tests for `SpawnEntityCommandEgressTranslator` or `DestroyEntityCommandEgressTranslator`. D005 success criteria 1 & 2 covered only by integration-level tests. Add `SpawnEntityCommandEgressTranslatorTests.cs` and `DestroyEntityCommandEgressTranslatorTests.cs` to `Hrot.Map.Common.Tests`. **Resolved BATCH-03.** | BATCH-02 review | BATCH-03 |
...
| DEBT-07 | P3 | `CycloneTranslator` nullability change: `PollIngress` null-guard added but `ApplyToEntity` and `Dispose` overrides in derived classes should be audited for null reader derefs. No immediate risk. | BATCH-03 review | Backlog |
