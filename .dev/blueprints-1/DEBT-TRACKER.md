...
| DEBT-003 | BATCH-03 | `BreakpointKey(string NodeId)` record specified in TASK-TH-008 but not implemented. Breakpoints use raw strings. No functional impact in Slice 1. | P3 | BATCH-07+ | OPEN (intentional deviation -- BreakpointId struct serves the same purpose; BreakpointKey is a design-only alias not required by any caller) |
| DEBT-004 | BATCH-03 | `IBlueprintDebugSession` event named `OnPinValueChangedEvent` instead of `OnPinValueChanged` (design name) due to C# conflict with generic method of same base name. Deliberate deviation; add comment in source. | P3 | BATCH-07+ | OPEN (comment present in IBlueprintDebugSession.cs line 186: "Named OnPinValueChangedEvent to avoid C# conflict with generic method OnPinValueChanged<T>.") |
...
| DEBT-021 | BATCH-18 review | `HandleBreakpointHit` fires `OnBreakpointListChanged` on every hit to signal hit-count change. Event was designed for structural changes (map registration, hash mismatch), not per-hit updates. Add separate `OnBreakpointHitCountChanged` event or remove the per-hit fire. | P3 | BATCH-19+ | OPEN |
| DEBT-022 | BATCH-20 review | `GetNodeHistory(Entity, int)` is a non-interface method on `BlueprintDebugSession`. Tests cast directly. Add to `IBlueprintDebugSession` if editor needs per-entity history access. | P4 | Future | OPEN |
| DEBT-023 | BATCH-27 review | `BuiltInChannelCommandCatalog` uses unqualified short action names ("MoveTo", "AimAndFire") instead of the hierarchical paths ("Locomotion/MoveTo", "Weapon/AimAndFire") from the design doc. This is correct for the current validator (matches ActionId strings in JSON assets), but diverges from the design intent. Add a comment to the catalog file explaining the naming convention. | P3 | BATCH-29+ | OPEN (comment added to BuiltInChannelCommandCatalog.cs in BPF-011/BATCH-06) |
