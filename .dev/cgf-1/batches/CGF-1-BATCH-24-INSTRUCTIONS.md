...
**Tasks:** **Part A — CGF1-S0310** (E2E DSM test script suite) → **Part B — Hrot.ClusterRunner orchestration node identity** (`-m all` / combined modes)  
...
5. `Hrot.ClusterRunner/Program.cs` — `RunnerOptions.NodeId`  
6. `Hrot.SimHost/SimHostApp.cs`, `Hrot.IG/IgApplication.cs`, `Hrot.ClusterRunner/Services/IosSubsystem.cs` — behaviour when `SubsystemConfig.NodeId == 0`
...
## Part B — Hrot.ClusterRunner: distinct `nodeId` per hosted subsystem

### Problem

`Hrot.ClusterRunner` can host **Orchestrator, SimHost, IG, IOS** (and **CGF** in comma-separated mode) in **one process**. Each participates in the orchestration plane (`ClusterSlave`, heartbeats, 2PC). **Duplicate `nodeId` values** cause ambiguous roster entries, wrong `NodeOp` fan-out targets, and hard-to-debug DSM stalls.
...
2. **`--node-id` = 0 (legacy / default)**  
   ...
   - **Known risk:** **SimHost** uses **`SimHostNetworkConstants.LocalNodeId` (1)** and **IG** uses **`IgNetworkConstants.LocalNodeId` (1)** for **`ClusterSlave`** when override is zero — **both register as node 1** in `-m all`. **IOS** uses a different fallback (**500**).  
   - **Fix direction (pick one, document in the report):** e.g. align IG orchestration id with **`IgNetworkConstants.InstanceId` (300)** / `_effectiveInstanceId` for `DdsOrchestrationTransport` + `ClusterSlave`, **or** teach `ResolveNodeId` to synthesize a non-zero base when **multiple** orchestration-capable subsystems are active. Preserve **standalone** `-m simhost` / `-m ig` behaviour unless product agrees to change defaults.
...
- [ ] `Hrot.ClusterRunner -m all` with **default** CLI: **no two subsystems** share the same orchestration `nodeId` (verify logs, `ClusterMaster` roster, or test hooks).  
...
- **IG handler registration tests** — BATCH-23 report: `TestHook_ClusterSlave` null in headless without DDS; add harness **or** DEBT row with lead sign-off.
...
- [RunMode](../../../Hrot.ClusterRunner/Configuration/RunMode.cs) — `All` composition (CGF not in `All` today; document if Part B touches this)  
