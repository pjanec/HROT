# CGF-1-BATCH-01: Orchestration schema, DrillMaster bootstrap, centralized ID allocator
...
You are implementing the first vertical slice of the CGF-1 control plane: **wire-level DDS contracts**, a **minimal `DrillMaster`** that owns `SystemStateTopic`, and **moving `DdsIdAllocatorServer` out of SimHost** into the orchestrator with a **transition fallback** for standalone SimHost. Complete all three tasks in order; do not stop mid-batch for approval to run tests or fix failures—run the full solution test suite and fix root causes before submitting the report.

### Required reading (in order)

1. **Workflow:** [.dev/.guides/DEV-GUIDE.md](../../.guides/DEV-GUIDE.md) — batch workflow, report expectations  
2. **Workstream onboarding:** [.dev/cgf-1/CGF-1-ONBOARDING.md](../CGF-1-ONBOARDING.md) — scope, folder map, FDP/Bagira boundary  
...
| Area | Path |
|------|------|
| DDS schemas (existing pattern) | `Bagira.DDS.DataModel/` (see e.g. `SimDescriptors.cs` for `[DdsTopic]`, `[DdsQos]`, `[DdsKey]` usage) |
| New orchestration messages | `Bagira.DDS.DataModel/Orchestration/OrchestrationMessages.cs` (per task detail) |
| ID allocator (server/client) | `FDP/ModuleHost/ModuleHost.Network.Cyclone/Services/DdsIdAllocatorServer.cs`, `DdsIdAllocator.cs` |
| SimHost app wiring (migration) | `Bagira.SimHost/SimHostApp.cs` |
| Runner / subsystem entry | `Bagira.Runner/` (`Bagira.Runner.csproj` is the executable host today) |
...
Use narrower projects while iterating if helpful (e.g. `Bagira.DDS.DataModel.Tests/Bagira.DDS.DataModel.Tests.csproj`), but **before the report** run **`dotnet test IOS-IG-SimHost.sln`** and ensure green.
...
This batch establishes the **DDS vocabulary** and **orchestrator process** that later phases (DrillSlave, 2PC, persistence) depend on. **Do not duplicate** field lists or QoS tables here—use the design doc and task detail as the single source of truth.
...
- [CGF1-S0102 — Bagira.Orchestrator bootstrapping](../CGF-1-TASK-DETAIL.md#cgf1-s0102--bagiraorchestrator-bootstrapping)  
...
- [§3.2 Stage 1.2 — Bagira.Orchestrator bootstrapping](../CGF-1-DESIGN.md#32-stage-12--bagiraorchestrator-bootstrapping)  
...
- All orchestration topics and enums exist under `Bagira.BDC.SSTD.Orchestration` with correct attributes (per design §3.1).  
- `DrillMaster` runs in a new `Bagira.Orchestrator` library, is hostable from `Bagira.Runner` (`--mode orchestrator` or equivalent config), and publishes initial `SystemStateTopic` as specified in task detail.  
...
**Scope:** Implement `OrchestrationMessages.cs` and **OrchestrationSchemaTests** in `Bagira.DDS.DataModel.Tests` exactly as specified in the task detail success conditions (reflection over `Bagira.BDC.SSTD.Orchestration`, enum values, `NodeHeartbeat` key, `SystemStateTopic` QoS).

**Non-goals:** No `DrillMaster` or runtime DDS wiring in this task.

---

### Task 2 — CGF1-S0102 (~8–10 h)

**Task definition:** [CGF-1-TASK-DETAIL.md § CGF1-S0102](../CGF-1-TASK-DETAIL.md#cgf1-s0102--bagiraorchestrator-bootstrapping)  
**Design:** [CGF-1-DESIGN.md §3.2](../CGF-1-DESIGN.md#32-stage-12--bagiraorchestrator-bootstrapping)

**Scope:**

- New projects: `Bagira.Orchestrator` (library, `net8.0`) and `Bagira.Orchestrator.Standalone` (executable host), registered in `IOS-IG-SimHost.sln`. Project references must align with the task detail (DataModel, Fdp.Kernel, Cyclone/module host as needed—**no** `Bagira.*` inside `FDP/`).  
- `DrillMaster`: subscribe `NodeHeartbeat`, publish `SystemStateTopic` on startup, `Tick()` for heartbeat-driven maintenance (roster pruning threshold per task detail / design).  
- Skeleton types: `DistributedTransaction`, `NodeRoster` as described in task detail (no full 2PC yet).  
- `Bagira.Runner`: activate orchestrator subsystem via **`--mode orchestrator`** and/or configuration—match the pattern used for other modes in that project.

**Tests:** Meet **all** success conditions in the task detail, including `DrillMasterBootstrapTests.OrchestratorPublishesStandbyOnStartup` (wall-clock bound, DDS reader asserts `CurrentState` and `TransactionEpoch`). Prefer a dedicated `Bagira.Orchestrator.Tests` project if integration tests need a clean host boundary; add it to the solution if you create it.

**Note:** If the repo has no prior `*.Standalone` exe besides `Bagira.Runner`, implement `Bagira.Orchestrator.Standalone` as a thin `Program.cs` that boots the same orchestrator path the Runner uses, with clean shutdown on Ctrl+C (task detail).
...
- Host `DdsIdAllocatorServer` inside `DrillMaster` (orchestrator), with `ProcessRequests()` (or equivalent) called from the orchestrator loop so allocations work under load.  
...
**Tests:** `DdsIdAllocatorMigrationTests.SimHostReceivesIdFromOrchestratorServer` per task detail; `Bagira.SimHost.Integration.Tests` unchanged pass.
...
- **Boundary:** No `Bagira.*` references under `FDP/`; DSM/orchestration wire types stay in `Bagira.DDS.DataModel`.  
...
**CGF-1-BATCH-02** is expected to cover **CGF1-S0104** (DrillSlave foundation) and **CGF1-S0105** (health / bootstrap recovery), pending lead review of BATCH-01.
