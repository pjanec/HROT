...
Part A matches the BATCH-03 review: **`ClusterConfiguration.LoadFrom`** fails fast when the file exists but read/JSON fails; **`DrillMasterBootstrapTests`** use **`TransitionState`** with **`LoadingLive`** payload; ImGui adds **CPU %** / **RAM (MB)** and **ACK latency** column; **`NodeHealthProfile`** and **`IngestHeartbeats`** propagate heartbeat metrics; **Standalone** references removed from **CGF-1-DESIGN** / **CGF-1-TASK-DETAIL**; **S0105** broadcast limitation is documented in task detail. **CGF1-S0201** delivers **`TransitionPlanner`** with BFS, **`TransitionStep`** / **`OperationStep`**, **`PlanTrajectory`** (int + JSON payload), eight **`TransitionPlannerTests`**, and **`DrillMaster.ProcessSysOpRequests`** integration (planner on **`TransitionState`**, **`Failure`** on unreachable path). **CGF-1-DESIGN §4.1** adjacency text still lists **`RunningEdit → LoadingLive`** while the **code** (correctly) omits that edge to match normative trajectories — already flagged for doc fix.

---

## Issues found

### Issue 1: `DrillMaster._currentDsmState` never advances (P2)

**File:** `Bagira.Orchestrator/DrillMaster.cs`  
**Problem:** `_currentDsmState` stays **`Standby`** after construction. Every **`PlanTrajectory`** uses this as **`current`**, not the last published **`SystemStateTopic`** or last completed transition. Sequential **`TransitionState`** requests from a cluster that has moved (e.g. after future 2PC) will be **planned from the wrong source state**.  
...
| **DrillMaster bootstrap** | **Aligned** with task detail op type/payload for reject/accept phases. |
...
```
feat(cgf-1): BATCH-04 debt closure + TransitionPlanner + DrillMaster wiring (CGF1-S0201)
...
- DrillMaster: plan TransitionState requests; Failure on unreachable path; CPU/RAM in roster.
- OrchestratorSubsystem ImGui: 6-col health + ACK latency column.
- TransitionPlannerTests (8); DrillMaster tests use TransitionState/LoadingLive.
