...
| Area | Verdict |
|------|---------|
| **Part B — `ResolveNodeId` + IG DrillSlave id** | **Met in source** — offsets for `Orchestrator` / `CGF` / `CI` / unknown are pairwise distinct; IG uses `_effectiveInstanceId` for orchestration when override is 0; `SubsystemOrchestratorTests` cover the regression. |
...
| **CI / tests** | **`Bagira.Runner.Tests` is not green** — `ParseMode_ComboAllThree_EqualsAllFlag` fails against current `RunMode.All` (`Orchestrator \| SimHost \| IG \| IOS`). This is a **real regression** for default `dotnet test` on that project, not an acceptable “pre-existing” excuse while `All` is documented to include Orchestrator. |
| **CLI vs code — CGF** | **Critical discovery:** [`BagiraRunnerConfiguration.ParseModeString`](../../../Bagira.Runner/Configuration/BagiraRunnerConfiguration.cs) has **no** `cgf` token (neither standalone nor comma-separated). [`Program.cs`](../../../Bagira.Runner/Program.cs) can add `CgfSubsystem` when `RunMode.CGF` is set, but **the Runner CLI cannot select CGF today**. Part B tests that use mock subsystems named `"CGF"` are valid; **claims about `orchestrator,cgf` from the CLI are not backed by configuration parsing**. |
...
**`IgApplication` DrillSlave node** — Uses `_effectiveInstanceId` (300 when override is 0), eliminating SimHost/IG both registering as `1` in `-m all`. Comment in source documents intent.

**Gaps / risks**

1. **CGF not launchable via CLI** (see above) — node-id offset for `"CGF"` is **unreachable** through `BagiraRunnerConfiguration` until `cgf` is added to mode parsing.  
...
- `SysopActionHandler` + `AssertEntityCountActionHandler` + `AddMovingTagActionHandler`; `MovingEntitySystem` as `ComponentSystem`; four JSON scripts under `TestScripts/`; `DsmE2eScriptTests` with separate DDS domain IDs; `AfterInitialize` on `HeadlessTestExecutor`; `HandleSysOpRequest` / async enqueue pattern documented honestly on `DrillMaster` (poll `SysOpStatus` separately — correct).

**Deviations / omissions**

1. **`MovingTestTag` placement** — TASK-DETAIL asks for the tag in the **same file** as `MovingEntitySystem`; implementation places it in [`OrchestratorActionHandlers.cs`](../../../Bagira.Runner/Testing/OrchestratorActionHandlers.cs). Pragmatic for compilation, but **normative doc and code diverge** — fix TASK-DETAIL or move the struct per lead policy.  
...
3. **`TakeCheckpoint` vs TASK text “TakeSnapshot”** — Code uses `SysOpType.TakeCheckpoint` (value 4), consistent with [`OrchestrationMessages.cs`](../../../Bagira.DDS.DataModel/Orchestration/OrchestrationMessages.cs). TASK wording is stale; not a code bug.
...
- `DrillMaster.HandleSysOpRequestAsync` XML **explicitly** states completion is post-enqueue only — no false promise.
...
**Subject:**  
`CGF-1-BATCH-24: S0310 E2E scripts + Runner nodeId offsets + IG DrillSlave identity`
...
DrillSlave when node override is 0. SubsystemOrchestratorTests extended.
...
AfterInitialize/SavedResults/ApproxEquals, DrillMaster.HandleSysOpRequestAsync,
OrchestratorSubsystem.TestHook_DrillMaster, DsmE2eScriptTests.
...
Batch **approved** for **merge with debt** tracked under **CGF-1-BATCH-25** (P1: green `Bagira.Runner.Tests`; CGF CLI parsing; handler fail-loud; E2E CI or explicit waiver). **Phase 3 / S0310** marked complete in tracker **with** those operational caveats documented in DEBT-TRACKER.
