...
A standalone handler for `ClusterOpType.ReplaySeek` was absent in `ClusterMaster.ProcessSingleClusterOpRequest`. When a bare `ClusterOpRequest { OperationType = ClusterOpType.ReplaySeek }` was dispatched it fell through all `if/else` branches with `capturedTrajectory = null`, so no `NodeReplaySeek` fan-out ever occurred. Added an `else if (req.OperationType == ClusterOpType.ReplaySeek)` block that immediately fans out a `NodeOpCommand { Operation = NodeOpType.NodeReplaySeek, PayloadJson = req.PayloadJson }` to all active nodes. The test in `ClusterMasterFanOutTests.cs` confirms the fan-out after transitioning to `RunningReplay`.
...
Two schema tests (`ClusterOpType_StepTime_Is14`, `ClusterOpType_SetTimeScale_Is15`) added to `OrchestrationSchemaTests.cs` pin the wire values.

### Task 2 (S0503-B): `TimeControlRequested` event in `ClusterMaster`

Added `public event Action<ClusterOpType, string>? TimeControlRequested` to `ClusterMaster`. An early-return interceptor at the top of `ProcessSingleClusterOpRequest` fires the event and returns immediately for `PauseTime`, `ResumeTime`, `StepTime`, and `SetTimeScale` — bypassing transaction creation and 2PC history entirely. Two tests in `ClusterMasterTimeControlTests.cs` verify the event fires exactly once on `PauseTime` and that `TransactionHistory` remains empty.
...
- Added `Update(float dt)` method that decrements the timer and fires `ClusterOpType.ReplaySeek` once the timer expires.
...
- Updated `RenderReplaySection` to accept `(ClusterState, bool disableAll, bool isPaused, float currentDrillTime)`: passive tracking sets `_seekSliderValue = currentDrillTime` when not pending; slider drags arm the debounce instead of writing immediately.
...
Replaced four `InputText` fields (`_loadScenarioId`, `_replayExerciseId`, `_injectScenarioId`, `_injectStoryId`) with combo-box index state (`_selectedLoadScenarioIdx`, `_selectedExerciseIdx`, `_selectedStoryIdx`) backed by `_availableScenarios`, `_availableDrills`, and `_availableStories` arrays.

Updated:
- `RenderScenarioSection`: `ImGui.Combo` + refresh button + load buttons guarded by `_selectedLoadScenarioIdx >= 0`.
- `RenderReplaySection`: `ImGui.Combo` + refresh + Load Replay guarded by `_selectedExerciseIdx >= 0`.
...
3. **DDS volatile history for `InjectStory` test:** Writing two `ClusterOpRequest` messages in rapid succession to a topic with default KeepLast(1) history caused only one to be visible to the reader. Fixed by reading sequentially with a 150ms delay between writes.
...
| 3 | Task 0 described as a "test gap" (code already exists) | Had to add the standalone `ClusterOpType.ReplaySeek` handler in `ProcessSingleClusterOpRequest` alongside the test | No standalone handler existed; the only `NodeReplaySeek` fan-out path was inside `capturedTrajectory` which required a `TransitionState` request. |
...
Hrot.NED.Tests:
  ...
  (+2 new: ClusterOpType_StepTime_Is14, ClusterOpType_SetTimeScale_Is15)
...
**Hardest part:** Identifying that `ClusterOpType.ReplaySeek` lacked a standalone handler entirely (Task 0). The instruction said "find the handler" implying it existed, but inspection of `ProcessSingleClusterOpRequest` showed no branch for it. After confirming through grep, added the handler alongside the test.
