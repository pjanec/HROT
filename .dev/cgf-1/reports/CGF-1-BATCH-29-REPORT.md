...
- [x] B: DrillMaster publishes inventory (`NasBasePath` property, `_inventoryWriter`, 5s tick throttle, `PublishAssetInventory()`, Dispose)
- [x] C: ClusterUiCache (8 readers, `Update()`, `Dispose()`, `GetNodeLastSeenMs()`, `ReachableTargets`, `ActiveStories`, `ActiveTransaction`)
- [x] D: OrchestratorScenarioPanel → ClusterScenarioPanel refactor (new constructor, new `Render(ClusterUiCache, bool)` signature, all DrillMaster reads replaced, zero _drillMaster dependency)
...
- `Bagira.DDS.DataModel.Tests`: 45 → **47** (+2: `AssetInventoryTopic_IsRegisteredInIdl`, `AssetInventoryTopicQos_IsTransientLocalKeepLast1`)
- `Bagira.Orchestrator.Tests`:  60 → **60** (unchanged, all still green)
- `Bagira.Runner.Tests`:        161 → **177** (+16: 5 `ClusterUiCacheTests`, 11 `ClusterScenarioPanelTests`)
...
1. **AssetInventoryTopic published by DrillMaster** ✅  
   `DrillMaster.Tick()` calls `PublishAssetInventory()` every 5 seconds (guarded by `_lastInventoryScan` timestamp). The `AssetInventoryTopic` struct is schema-pinned in `OrchestrationSchemaTests`.
...
6. **No regression in E2E DSM test suite** ✅  
   All 177 `Bagira.Runner.Tests` pass including all DsmE2eScriptTests.
...
| `SysOpStatusCode.Completed` | `OrchestrationStatusCode.Success` (== 0) | 
...
#### 2. Transaction ID Mismatch in DrainSysOpStatus

`NodeOpCommand.TransactionId` ≠ `SysOpStatus.RequestId` in production. DrillMaster generates a new `Guid` for each 2PC transaction's `TransactionId`, while `SysOpStatus.RequestId` echoes the original `SysOpRequest.RequestId`. The cache's `_inFlight` is keyed by `NodeOpCommand.TransactionId`. 

**Resolution:** `DrainSysOpStatus()` tries an exact match first (works in unit tests where IDs are identical). On miss, it applies a fallback: mark all in-flight transactions as completed/aborted and clear `_inFlight`. This is documented as a known limitation — the cache is an optimistic UI model, not a transactional log.

#### 3. NodeHealthProfile vs NodeHeartbeat

`DrillMaster.NodeRoster.ActiveNodes` returns `IReadOnlyDictionary<int, NodeHealthProfile>` (with `LastHeartbeatUtcSeconds`), while `ClusterUiCache.ActiveNodes` stores raw `NodeHeartbeat` DDS structs (with `WallTicksUtc`). 
...
2. **SysOpStatus.RequestId / TransactionId ambiguity**: As noted above, the two IDs are decoupled. A future protocol improvement: include `TransactionId` in `SysOpStatus` to enable unambiguous correlation.
...
1. **ReachableTargets in ClusterUiCache**: Added a `DrillMasterPlanner` field to `ClusterUiCache` so `ReachableTargets` can be computed from `CurrentState` using `BagiraStateGraph.GetNeighbors()`. This avoids `_drillMaster.GetReachableTargets()` in the panel while preserving the correct DSM graph logic.

2. **Active Stories via ManageStory sniffer**: `Process2PcNetworkTraffic()` sniffs `StartStory` and `StopStory`/`ForgetStory` `NodeOpCommand` samples to maintain `_activeStories`. This is an optimistic model — story additions/removals may lag by one `Update()` cycle compared to `DrillMaster.ActiveStories`.
...
5. **`Completed` added to DistributedTransaction**: Required by ClusterUiCache's `DrainSysOpStatus()` to distinguish "completed successfully" vs "still in flight" for CQRS-observed transactions. The existing `IsAborted` flag is unchanged (used by DrillMaster local logic).

### ActiveStories / Missing ClusterUiCache Fields Strategy

| Missing Field | Strategy |
|---|---|
| `ActiveStories` | Maintained in `ClusterUiCache._activeStories` via `NodeOpCommand` sniffer (StartStory/StopStory operations). Exposed as `IReadOnlySet<Guid>`. No DrillMaster fallback. |
| `ActiveTransaction` | Computed property: first entry of `TxHistory` when `HasInFlightTransaction`, else null. |
| `ReachableTargets` | Computed from `CurrentState` using embedded `DrillMasterPlanner(_planner)`. Updated in `DrainSystemState()` when state changes. |
...
The exact pattern found throughout the codebase (e.g., `TimePulseIngressTranslator.cs`, `DrillMaster.IngestHeartbeats()`):
...
1. **`OrchestratorScenarioPanel.cs` / `OrchestratorScenarioPanelTests.cs` not deleted**: The old class and its test file still exist. `OrchestratorSubsystem` no longer references them; the class is dead code. Deletion blocked by the `OrchestratorScenarioPanelTests` which test `DrillMaster`-specific behavior (GetReachableTargets, HandleSysOpRequest) that cannot be ported to ClusterScenarioPanel tests. These tests remain valid Orchestrator-behavior tests and should be migrated to `Bagira.Orchestrator.Tests` or removed in the next cleanup batch.

2. **SysOpStatus/TransactionId correlation**: Add `TransactionId` to `SysOpStatus` wire format for unambiguous cache correlation.
...
- The `ClusterUiCache` `_inFlight` clearing fallback in `DrainSysOpStatus()` may prematurely close transactions if multiple SysOps complete rapidly. This is acceptable for the UI use-case but not for transactional correctness.
