...
|---------|--------|--------|-------|
| `Bagira.SimHost.Integration.Tests` | 30 | 0 | Includes new `DrillSlaveHeartbeatTests` + existing migration/lifecycle |
| `Bagira.Orchestrator.Tests` | 1 | 0 | `OrchestratorPublishesStandbyOnStartup` — domain 15, exactly-one assertion |
| `Bagira.DDS.DataModel.Tests` | 43 | 0 | `AllTopicStructsHaveDdsTopicAttribute` now reflection-based |
| `Bagira.SimHost.Tests` | 360 | 0 | No regressions |
| `Bagira.IG.Tests` | 429 | 0 | No regressions |
...
| Row | What was done |
|-----|---------------|
| P2 Performance — `ProcessRequests` dual call | Removed redundant `_idAllocatorServer?.ProcessRequests()` from `DrillMaster.Tick()`. Background thread loop is the sole caller. |
...
| P3 Testing — `OrchestrationSchemaTests` reflection | `AllTopicStructsHaveDdsTopicAttribute` now reflects over all public, non-codegen structs in `Bagira.BDC.SSTD.Orchestration` (filters `*View`, `*_Native`, `*KeyHolder`). |
...
| P3 Hygiene — `DrillMaster._profiles` | Removed dead `Dictionary<int, NodeHealthProfile> _profiles` and its write site in `IngestHeartbeats`. `NodeRoster` is the sole source of truth. |
...
- `DrillMaster` (domain 16) + `SimHostApp` (nodeId=1, domain 16) + `CgfApplication` (nodeId=400, domain 16) run in-process.
- Both tick for up to 2 s wall-clock (16 ms sleep per iteration ≈ 60 Hz).  
- After 2 s, `DrillMaster.NodeRoster.ActiveNodes` contains both nodeId=1 and nodeId=400.
- Both have `LocalDsmState == Standby`.
- **Result: PASSED** (part of `Bagira.SimHost.Integration.Tests` run above).

### `IDsmHandler` FDP boundary audit

- `IDsmHandler` is declared in `Bagira.Common.Orchestration.IDsmHandler` (assembly `Bagira.Common`).
- `Bagira.Common.csproj` references only `Bagira.DDS.DataModel` and `Fdp.Kernel` — no reverse Bagira→FDP.
...
### `Bagira.Common` vs `Bagira.Runner`

Placed `IDsmHandler` in a new minimal `Bagira.Common` project (not in `Bagira.Runner`) because `Bagira.Runner` references `Bagira.SimHost`, `Bagira.IG`, `Bagira.IOS` — putting the interface there would require those projects to reference `Bagira.Runner`, creating circular dependencies.

### `DrillSlave` constructor overloads in `Bagira.SimHost`

A no-arg `DrillSlave()` constructor is preserved so existing tests that exercise only handler registration (e.g. `RecordReplayIntegrationTests`) continue to work without a live DDS participant. The DDS-active constructor `DrillSlave(DdsParticipant, int, string)` is used in production via `NodeBootstrapper.BuildOrchestration(…, participant, subsystemName)`.

### `EcsRecordReplayController : IDsmHandler` stub methods

Added `CanHandle`, `PrepareAsync`, `Commit`, `Abort` stubs (returns `false` / no-op / null task). Full 2PC dispatch wiring requires `DsmStateChangedEvent` and `FdpEventBus` integration, which is scheduled for CGF1-S0202 in BATCH-03.

### IgApplication DrillSlave exception guard
...
- Domain 15: `Bagira.Orchestrator.Tests`  
- Domain 16: `Bagira.SimHost.Integration.Tests` — CGF/DrillSlave tests  
...
```
feat(cgf-1): S0104 DrillSlave foundation + BATCH-01 debt clearance (CGF-1-BATCH-02)

Part A — BATCH-01 debt (7 items):
- DrillMaster.Tick: removed redundant ProcessRequests() call (P2 Perf)
...
- DrillMaster._profiles: removed dead dictionary (P3 Hygiene)
- CGF-1-ONBOARDING.md: DDS test isolation contributor note

Part B — CGF1-S0104:
- Add Bagira.Common (IDsmHandler with CanHandle/PrepareAsync/Commit/Abort)
- Add Bagira.CGF library + Bagira.CGF.Standalone (CgfApplication, CGF DrillSlave)
- Full DrillSlave implementation in Bagira.SimHost (heartbeat + command queue)
- DrillSlave added to Bagira.IG, Bagira.IOS, Bagira.CGF
- Wire SimHost DrillSlave via NodeBootstrapper.BuildOrchestration in SimHostApp.OnLoad
- Wire IG DrillSlave in IgApplication.InitializeNetwork / Update / Shutdown
- Wire IOS DrillSlave in IosSubsystem.Initialize / Update / Shutdown
- Register CGF via new CgfSubsystem + RunMode.CGF in Bagira.Runner
- EcsRecordReplayController: stub IDsmHandler (CanHandle=false; S0202 wires full 2PC)
- Integration test: DrillSlaveHeartbeatTests.OrchestratorReceivesHeartbeatsFromBothNodes
