...
CGF-1-BATCH-21 closes all Part A tech-debt rows (ManageStory 2PC, StoryLoadDsmHandler
fail-loud, DESIGN §5.8 note, test rename) and advances Phase 4 generalization
(G0401–G0404 partial).

**Part A — Tech debt (complete):**
- A.1: `DrillMaster.ManageStory` 2PC — `ActiveStories` mutation deferred to ACK
  collection; 2 new `DrillMasterStoryTests`.
- A.2: `Bagira.SimHost` `StoryLoadDsmHandler` — invalid payloads always set a
...
- G0402 ✅ — Generic `DrillSlave`, `DdsOrchestrationTransport`, `BagiraHandlerAdapter`; 4 tests pass.
- G0403 ✅ — Toolkit `TransitionPlanner` (BFS/int), `BagiraStateGraph`, `DrillMasterPlanner`; 31/31 orchestrator tests pass + 2 new toolkit BFS tests.
...
Tests: `Bagira.Orchestrator.Tests` 31/31 ✅; `FDP.Toolkit.Orchestration.Tests` 11/11 ✅.
...
### A.1 — `DrillMaster.ManageStory` 2PC (P2)

**Problem:** `FanOutNodeOp` for `StartStory`/`StopStory` immediately advanced
`ActiveStories` and emitted `SysOpStatus.InProgress` w/ `CompletedSteps == totalSteps`
with no `NodeOpStatus` round-trip.

**Solution:** A `_pendingManageStoryTasks` dictionary (keyed by `transactionId`) now
tracks a `PendingStoryTask` record containing the expected set of `RemainingNodeIds` and
the intended mutation lambda. On each tick, `DrillMaster` drains `NodeOpStatus` messages:
...
`SysOpStatus.Completed` is emitted. The policy is **participating-only is not required** —
...
A `if (_gateway != null)` guard was also added in the `ManageStory` foreach loop over
`PrefetchScenario` steps so that unit tests (no gateway) do not throw before `StartStory`
fan-out.

**Tests added:**
- `DrillMasterStoryTests.StartStory_ActiveStoriesUpdated_AfterNodeAck_NotBefore` — verifies `ActiveStories` is empty before ACK and populated after.
- `DrillMasterStoryTests.StartStory_NonParticipatingAck_CountsTowardCompletion` — verifies a non-participating node ACK still satisfies the set.

**Files changed:**
- `Bagira.Orchestrator/DrillMaster.cs` — `_pendingManageStoryTasks`, `PendingStoryTask` record, `if (_gateway != null)` guard, ACK drain in `Tick`
- `Bagira.Orchestrator.Tests/DrillMasterStoryTests.cs` — 2 new tests

---

### A.2 — `Bagira.SimHost` `StoryLoadDsmHandler`: always ACK or fail loud (P2)

**Problem:** Invalid `StartStory`/`StopStory` payloads could leave
`_pendingTransactionId = Guid.Empty`, causing `CommitStartStory` / `CommitStopStory` to
...
**Solution:**
- `PrepareAsync(StartStory)` and `PrepareAsync(StopStory)` always set
  `_pendingTransactionId` (even for bad payloads). Invalid payloads set
  `_pendingIsParticipating = false`.
- `CommitStartStory` / `CommitStopStory`: if `_pendingIsParticipating = true` and
...
- `StoryLoadDsmHandler_InvalidStartStoryPayload_AcksNonParticipating`
- `StoryLoadDsmHandler_InvalidStopStoryPayload_AcksNonParticipating`
...
**Files changed:**
- `Bagira.SimHost/Modules/Orchestration/Handlers/StoryLoadDsmHandler.cs` — invalid-payload paths, null-repo guard throws
- `Bagira.SimHost.Tests/StoryLoadDsmHandlerTests.cs` — 5 new tests

---

### A.3 — DESIGN + TASK-DETAIL hygiene (P3)

**Design note added:** `CGF-1-DESIGN.md` §5.8 *ManageStory 2PC — MVP Implementation Note*
...
- `Bagira.SimHost.Integration.Tests/RecordReplayIntegrationTests.cs` — test renamed
...
| Row | Description |
|-----|-------------|
| P2 Architecture | `DrillMaster.ManageStory`: no `NodeOpStatus` round-trip → 2PC implemented |
| P2 Correctness | `StoryLoadDsmHandler`: missing ACK on invalid payload / null repo → fixed |
| P3 Documentation | `CGF-1-DESIGN.md` ManageStory/story ACK MVP delta note missing → §5.8 added |
| P3 Hygiene | `NodeBootstrapper_BrainRole_RegistersEcsRecordReplayController` stale name → renamed |
| P3 Testing | No test for `ManageStory → NodeOpStatus` aggregation or malformed payload ACK → covered |
...
**New project:** `FDP/Toolkits/FDP.Toolkit.Orchestration/` — toolkit-pure (no `Bagira.*`
references). Contains:
...
| `TkDsmStateChangedEvent.cs` | `record TkDsmStateChangedEvent(int FromStateId, int ToStateId, string SubsystemName)` |
...
- 5 Bagira handler files (`Bagira.CGF/Modules/Orchestration/Handlers/StoryLoadDsmHandler.cs`
  and 4 `Bagira.SimHost` handlers) — `IDsmHandler` now ambiguous between
  `Bagira.Common.Orchestration.IDsmHandler` and `FDP.Toolkit.Orchestration.IDsmHandler`
  → all base-class declarations qualified as `Bagira.Common.Orchestration.IDsmHandler`.
...
- `Bagira.CGF/Modules/Orchestration/Handlers/StoryLoadDsmHandler.cs` — qualified `IDsmHandler`
- `Bagira.SimHost/Modules/Orchestration/Handlers/{Checkpoint,PrefetchFiles,ReplayLoad,StoryLoad}DsmHandler.cs` — qualified `IDsmHandler`

---

### G0402 — Generic `DrillSlave` + `DdsOrchestrationTransport` ✅

**Toolkit `DrillSlave`** (`FDP/Toolkits/FDP.Toolkit.Orchestration/DrillSlave.cs`):
- Uses `IOrchestrationTransport`, `ITickableDsmHandler` (handler list), publishes
  `TkDsmStateChangedEvent` via `FdpEventBus` on `CommitState`.
- `CommitStateOperationId = 2` (integer constant — toolkit cannot reference
  `NodeOpType` from `Bagira.DDS.DataModel`).
...
- Constructors: `public DrillSlave(IOrchestrationTransport, int nodeId, string subsystem, FdpEventBus?)` + `internal DrillSlave(FdpEventBus? = null)` (test-only).
...
**`DdsOrchestrationTransport`** (`Bagira.Common/Orchestration/DdsOrchestrationTransport.cs`):
...
- `StatusWriter` property exposed for backward compatibility (legacy Bagira handlers still
  need raw `DdsWriter<NodeOpStatus>` until G0404/G0405 migration).

**`BagiraHandlerAdapter`** (`Bagira.Common/Orchestration/BagiraHandlerAdapter.cs`):
- Wraps `Bagira.Common.Orchestration.IDsmHandler` as `FDP.Toolkit.Orchestration.ITickableDsmHandler`.
- `CanHandle(int) → (NodeOpType)int` conversion; `DrainDeferredAcks()` forwards to inner
  if inner implements `Bagira.Common.Orchestration.ITickableDsmHandler`.
...
**Project dependency added:** `Bagira.Common.csproj` now references `FDP.Toolkit.Orchestration`.
`FDP.Toolkit.Orchestration.Tests.csproj` references `Bagira.Common` (for DDS transport integration test).

**Note:** The 4 original Bagira `DrillSlave` copies (SimHost, CGF, IG, IOS) are
**intentionally not deleted** in this batch — deletion is G0406 and requires all handlers
to be migrated first.

**Tests (DrillSlaveTests.cs):**
- `DrillSlave_DispatchesPrepareAsyncAndCommit_SynchronousHandler` — sync handler dispatched immediately.
- `DrillSlave_DeferCommit_WhenPrepareAsyncIsAsync` — async prepare defers Commit to next Tick.
- `DrillSlave_DeduplicatesTransactions` — second PrepareAsync for same TransactionId is a no-op.
- `DdsTransport_DeliversCommand_ToDrillSlave` — end-to-end DDS round-trip on domain 17.

**Files changed:**
- `FDP/Toolkits/FDP.Toolkit.Orchestration/DrillSlave.cs` — new
- `Bagira.Common/Orchestration/DdsOrchestrationTransport.cs` — new
- `Bagira.Common/Orchestration/BagiraHandlerAdapter.cs` — new
- `FDP/Toolkits/FDP.Toolkit.Orchestration.Tests/DrillSlaveTests.cs` — new (4 tests)
- `FDP/Toolkits/FDP.Toolkit.Orchestration/FDP.Toolkit.Orchestration.csproj` — `InternalsVisibleTo`
- `FDP/Toolkits/FDP.Toolkit.Orchestration.Tests/FDP.Toolkit.Orchestration.Tests.csproj` — `Bagira.Common` reference
- `Bagira.Common/Bagira.Common.csproj` — `FDP.Toolkit.Orchestration` reference
...
- Pure BFS on `ITransitionGraph`; integer state IDs only; zero Bagira dependencies.
...
**`BagiraStateGraph`** (`Bagira.Orchestrator/BagiraStateGraph.cs`):
- `public static ITransitionGraph Build()` — constructs the canonical Bagira DSM edge set via `TransitionGraphBuilder`, matching the prior hardcoded adjacency dictionary in the old `TransitionPlanner`.
- Edges: `Standby → {LoadingEdit, LoadingLive, LoadingReplay}`; full edit/live/replay/dry-run cycles.

**`Bagira.Orchestrator/TransitionPlanner.cs` → `DrillMasterPlanner`:**
- Class renamed from `TransitionPlanner` to `DrillMasterPlanner`.
- Old hardcoded `Adjacency` dictionary removed.
- New constructor: `DrillMasterPlanner(ITransitionGraph graph)` — creates toolkit `TransitionPlanner` internally.
- `CalculateShortestPath(DSMState, DSMState)` delegates to toolkit BFS with `(int)` casts, converts result list and catches toolkit exceptions to re-throw with DSMState enum names in the message (so test assertions on "Degraded", "RunningLive" etc. still hold).
- `PlanTrajectory`, `PlanManageStory`, step classes (`ISysOpStep`, `TransitionStep`, `OperationStep`) unchanged.

**`DrillMaster.cs` updated:** `_planner` field changed to `DrillMasterPlanner`,
initialized with `new DrillMasterPlanner(BagiraStateGraph.Build())`.

**`TransitionPlannerTests.cs` updated:**
- `_planner` → `DrillMasterPlanner(BagiraStateGraph.Build())`.
- `_tkPlanner` field added: `new FDP.Toolkit.Orchestration.TransitionPlanner(BagiraStateGraph.Build())`.
...
- New test: `TkPlanner_StandbyToRunningLive_BfsPathPreserved` — verifies toolkit BFS produces same int-path as prior Bagira planner for Standby→RunningLive.
- `RunningDryRunToRunningReplay_Produces_SixSteps` updated to call `_planner.CalculateShortestPath(DSMState, DSMState)`.
...
- `Bagira.Orchestrator/BagiraStateGraph.cs` — new
- `Bagira.Orchestrator/TransitionPlanner.cs` — class renamed `DrillMasterPlanner`, BFS delegated to toolkit
- `Bagira.Orchestrator/DrillMaster.cs` — `_planner` type + initialization updated
- `Bagira.Orchestrator.Tests/TransitionPlannerTests.cs` — `DrillMasterPlanner`, `_tkPlanner`, 2 new tests
...
**`LocalDiskStorageProvider`** (`Bagira.Common/Orchestration/LocalDiskStorageProvider.cs`):
...
**Files changed:**
- `Bagira.Common/Orchestration/LocalDiskStorageProvider.cs` — new
...
| G0406 (delete 4 old Bagira `DrillSlave` copies, full CI validation) | BATCH-22 | G0404+G0405 complete |
...
| Project | Result |
|---------|--------|
| `Bagira.Orchestrator.Tests` | 31 / 31 ✅ |
| `FDP.Toolkit.Orchestration.Tests` | 11 / 11 ✅ (3 G0401 + 4 G0402 + 2 G0403 BFS + 2 G0404) |

*`Bagira.SimHost.Tests`, `Bagira.SimHost.Integration.Tests`, `Bagira.IG.Tests` etc. are
