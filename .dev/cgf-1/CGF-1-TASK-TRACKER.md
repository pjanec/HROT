...
**Active batch:** [CGF-1-BATCH-18](batches/CGF-1-BATCH-18-INSTRUCTIONS.md).  
**Last reviewed:** [CGF-1-BATCH-17](reviews/CGF-1-BATCH-17-REVIEW.md) — CONDITIONALLY APPROVED (2026-04-10).  
...
> **Progress (Phase 1):** 5 / 6 tasks done. **Phase 2:** 5 / 5 complete. **Phase 3:** 8 / 10 done (S0301–S0307, S0309); S0305 **orchestrator freeze + `ReplayLoadDsmHandler` branch code + unit tests** → [CGF-1-BATCH-17](batches/CGF-1-BATCH-17-INSTRUCTIONS.md) ✅; **S0305 residual** (SimHost **`DrillSlave` `PrepareLive` dispatch**, CGF branch visibility, **`PrepareAsync` await**, **`FullBranchPipelineTests`**) → [CGF-1-BATCH-18](batches/CGF-1-BATCH-18-INSTRUCTIONS.md) ([BATCH-17 review](reviews/CGF-1-BATCH-17-REVIEW.md#critical-gap---preparelive-never-reaches-replayloaddsmhandler-on-simhost)); S0205 residual closed in BATCH-10; subprocess CI Opportunistic.
...
- [ ] **CGF1-S0106** Orchestrator ImGui Scenario & Story Controls [details](./CGF-1-TASK-DETAIL.md#cgf1-s0106--orchestrator-imgui-scenario--story-controls)
...
- [x] **CGF1-S0305** Live-from-Replay Temporal Interlock [details](./CGF-1-TASK-DETAIL.md#cgf1-s0305--live-from-replay-temporal-interlock) — orchestrator **`ReplayMasterModule`** + **`ReplayLoadDsmHandler`** branch implementation + tests in [CGF-1-BATCH-17](batches/CGF-1-BATCH-17-INSTRUCTIONS.md) Part B ✅; **production `DrillSlave` path still routes `PrepareLive` to `LiveLoadDsmHandler` first** (branch dead) + CGF branch **`PrepareLive`** vs **`ScenarioLoadDsmHandler`** + **`FullBranchPipelineTests`** → [CGF-1-BATCH-18](batches/CGF-1-BATCH-18-INSTRUCTIONS.md) ([review](reviews/CGF-1-BATCH-17-REVIEW.md))
