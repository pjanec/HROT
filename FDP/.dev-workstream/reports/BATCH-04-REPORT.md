...
The behavior test count grew from **15 → 25**: 10 additions (1 ordering integration test, 2 strengthened existing test assertions with added invariants, 3 BTree tests, 2 HSM tests, 4 behavior ingress tests including the end-to-end preemption chain).

---

## Task Completion Checklist

- [x] **Corrective 0a** — Three weak existing tests fixed: `Arbitration_ClearsStaleChannel` now asserts `BehaviorInstanceId == 0`; `Dispatcher_CallsOnEnter_OnFirstTick` now asserts `Status == Running` and `DispatchedInstanceId == ActionInstanceId`; `Dispatcher_SkipsNullExecutor_Gracefully` now asserts bookkeeping ran (`ActionInstanceId == DispatchedInstanceId`). All pass.
...
- [x] **BCS-P1-T7** — `BehaviorRegistry` + `BehaviorIngressSystem` + `AssignBehaviorEvent` implemented; 4 tests pass including the end-to-end preemption chain (`BehaviorIngress_StaleSetsNewInstanceId_ArbitrationClearsOldAction`).
- [x] **`InputSystemGroup`** added to `StandardSystemGroups.cs`; `BehaviorIngressSystem` runs in it.
...
`IAIContext` declares nine members: `DeltaTime`, `Time`, `FrameCount`, `RequestRaycast`, `GetRaycastResult`, `RequestPath`, `GetPathResult`, `GetFloatParam`, and `GetIntParam`. Of these, only `GetFloatParam` and `GetIntParam` need to return per-entity data — they index into the behavior parameter arrays carried in `BrainBlackboard`. The raycast/path members are side-effecting query stubs with no ECS counterpart yet; they return `default` / `-1`.
...
- `float _deltaTime`, `float[] _floatParams`, `int[] _intParams` — tick inputs from the behavior definition.
...
The pattern in `BehaviorIngressSystem.OnUpdate` is:
...
The pattern matches the established idiom in `BehaviorIngressSystemTests.cs` where the test reads back via `*(FleeBlackboard*)bbPtr->Memory` and verifies the written float.
...
- [ ] `BehaviorIngressSystem` does not validate that `JsonParams` is well-formed before passing it to `ParseParams`. A malformed string will throw inside the delegate with no per-entity error context. Consider wrapping in a try/catch with entity ID logging in debug builds.
...
| `FDP/Toolkits/FDP.Toolkit.Behavior/BehaviorRegistry.cs` | Startup-time registry: behavior name hash → `BehaviorDefinition` |
| `FDP/Toolkits/FDP.Toolkit.Behavior/Events/AssignBehaviorEvent.cs` | Managed event class carrying behavior assignment payload |
...
| `FDP/Toolkits/FDP.Toolkit.Behavior/Systems/BehaviorIngressSystem.cs` | Consumes `AssignBehaviorEvent`, updates `BehaviorState`, parses blackboard |
...
| `FDP/Toolkits/FDP.Toolkit.Behavior.Tests/BehaviorIngressSystemTests.cs` | 4 behavior ingress tests including end-to-end preemption chain |
