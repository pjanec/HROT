...
namespace ModuleHost.Core.Tests.Integration
{
    public class TimeIntegrationTests
    {
        ...
        public void Initialize_DefaultConfig_CreatesStandaloneController()
        {
            ...
            // NOTE: In previous version, kernel created a default controller. 
            // Now, we MUST inject it. Tests that relied on default must now provide one.
            var bus = new FdpEventBus();
            var controller = TimeControllerFactory.Create(bus, new TimeControllerConfig { Role = TimeRole.Standalone });
