...
///   <item>IG stub delivers a <see cref="MapClickEvent"/>; ExCon emits
...
public class FullStackWorkflowTests
{
    ...
    private sealed class WorkflowFixture : IDisposable
    {
        ...
        /// the ExCon.  Returns the new entity.
        /// </summary>
    ...
    public async Task FullWorkflow_PlacementToMissionCommit_CompletesSuccessfully()
    {
        ...
        f.ClickQueue.Enqueue(new MapClickEvent
        {
            ...
            Position             = new GeoPoint { Latitude = 50.0, Longitude = 14.4 }
        });
...
// ──────────────────────────────────────────────────────────────────────────────
// ExCon.9.4 — Conflict detection workflow tests
// ──────────────────────────────────────────────────────────────────────────────

/// <summary>
/// Scenario 5: Optimistic-lock conflict detection.
///
/// Validates that two concurrent ExCon clients editing the same mission correctly
...
/// mirroring two separate ExCon operator stations connected to the same DDS
/// domain.</para>
/// </summary>
[Collection("Integration")]
