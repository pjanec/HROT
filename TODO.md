...
[ISSUES] collected during time sync development - can be obsolete, needs revising
...
- **Tick-domain mixups:** code mixed raw ticks (`_getTick()`), synced ticks (`SyncedWallTicks`), and PLL-adjusted `_virtualWallTicks`, causing incorrect comparisons and huge deltas.
- **Hard-snap domain bug:** `_lastUpdateRawTicks` was assigned a synced value (`currentAbsTicks`/`SyncedWallTicks`) instead of raw `_getTick()`, inflating `rawDelta` after a hard snap.
- **Barrier check uses wrong domain:** `SlaveSyncController.UpdateBarrierPending` compared `barrier` against `_virtualWallTicks` (PLL-adjusted) instead of `SyncedWallTicks`, risking missed barriers on multi-machine setups.
...
- **`SeedState` / pulse flooding:** `MasterSyncController.SeedState` set `_totalWallTicks` but didn't reset `_lastPulseTicks`, allowing immediate zero-timestamp pulses.
- **Barrier advancing sim-time:** `UpdateBarrierPending` previously advanced sim-time while waiting for barrier, causing unexpected time progression during pause.
- **PLL drift vs tests:** PLL warm-up altered `_virtualWallTicks` so tests using virtual ticks as barrier became flaky; tests needed to use SyncedWallTicks/ticks.
...
- **TimePulseDescriptor field name:** tests/spec used `FrameNumber` but actual struct exposes `SequenceId`.
- **`CreateMasterSlave` helper bug:** helper captured `ref` parameters in lambdas (not allowed in C# closures); helper removed/fixed.
- **Assertion overload mismatch:** xUnit `Assert.Equal<T>` lacks string-message overload; tests had to be rewritten.
- **Test vacuity / drain tests:** some drain tests passed vacuously because they asserted on empty read buffers without swapping/publishing, not verifying actual drain behavior.
