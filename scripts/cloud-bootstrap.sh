...
install_ddsmonitor() {
    ...
    local tools_dir="$DOTNET_ROOT/tools"

    if "$DOTNET_ROOT/dotnet" tool list --global 2>/dev/null | grep -qi "$pkg"; then
        log "ddsmonitor already installed; skipping. (DDSMON_FORCE=1 to update.)"
        [ "${DDSMON_FORCE:-0}" = 1 ] && "$DOTNET_ROOT/dotnet" tool update --global "$pkg" >/dev/null 2>&1 || true
    else
        log "Installing ddsmonitor ($pkg) as a global tool ..."
        if "$DOTNET_ROOT/dotnet" tool install --global "$pkg" >/dev/null 2>&1; then
            log "ddsmonitor installed."
        else
            log "NOTE: ddsmonitor install failed (network policy must allow nuget.org). Continuing."
            return 0
        fi
    fi

    # Global-tools dir on PATH for interactive/login shells.
    export PATH="$tools_dir:$PATH"
    persist_env "export PATH=\"$tools_dir:\$PATH\""

    # ...and a /usr/local/bin wrapper for Claude's NON-interactive Bash shells (same
    # reason as the dotnet wrapper above: they source neither ~/.bashrc nor ~/.profile).
    # The tool's native apphost needs DOTNET_ROOT to locate the runtime under $HOME/.dotnet.
    if [ -d /usr/local/bin ] && [ ! -e /usr/local/bin/ddsmonitor ] && [ -w /usr/local/bin ]; then
        cat > /usr/local/bin/ddsmonitor <<WRAP
#!/bin/sh
# Generated by scripts/cloud-bootstrap.sh — runs the CycloneDDS ddsmonitor tool.
export DOTNET_ROOT="$DOTNET_ROOT"
exec "$tools_dir/ddsmonitor" "\$@"
WRAP
        chmod +x /usr/local/bin/ddsmonitor
        log "Wrapper installed: /usr/local/bin/ddsmonitor"
    fi
}
