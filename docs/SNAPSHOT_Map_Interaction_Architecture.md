...
| — | ⚠ **CORRECTED `2026-09-10`:** no Stride file *constructs* `SharedOrbatPanel`, ⛔ **but that is not "no Stride orbat"** — mode 1 hosts the REAL editor and `RegisterWindows` registers every editor panel *(`DESIGN_Stride_Node_Modes.md:415`, design-claimed, production path unmeasured)*; mode 2 is a **Muscle** node whose target surface is SimHost's, and SimHost has no orbat either. ⇒ the real gap is that the RULE is stated nowhere. ⚠⚠ **AND THE RULE IS NOT KEYED ON BRAIN ROLE** *(user, `2026-09-10`: "orbat needed also on excon node")* — 📐 `DESIGN_Node_Roles_And_Policies.md:152` calls ExCon *"operator console … not an ECS node"*, yet it carries the **richest** orbat in the repo *(the 434-line fork, `UXI-25`)*. ⇒ the predicate is **"the node has an operator-facing force view"**, satisfied by brain-role ECS nodes *(editor · CGF · Stride mode 1)* **and** by the non-ECS operator console; the counter-example is **Muscle/SimHost**, which legitimately has none | ✅ **CLOSED `2026-09-10`** — the rule now has an owner: 📄 [`DESIGN_Node_Roles_And_Policies.md`](DESIGN_Node_Roles_And_Policies.md) **§5a**, with the per-host as-is table. ⛔ Do not restate it here |
