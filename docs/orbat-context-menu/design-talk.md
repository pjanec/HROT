...
**Mechanism:** `MapCommandRequest` using `CommandType.CMD_SET_VIEW`. Similar to selection, the IOS can publish a `MapCommandRequest`. You would read the entity's current `GeoSpatial` coordinates from the IOS `DerRepo` and pass them in the `CommandArgsJson` (e.g., `{ "lat": 45.0, "lon": 12.0, "zoom": 1000 }` as documented in the struct). The IG will consume this and move its `MapCamera`.
...
**Fix** **Bagira.SimHost/Brains/SimHostNodes.cs****:**
...
public void StartPersonalRouteAuthoring(int vehicleEntityId)
{
    ...
    string argsJson = Newtonsoft.Json.JsonConvert.SerializeObject(new {
        ...
        tkbType = Bagira.Map.Common.TkbEntityTypes.TacGraphic_Route 
    });
...
First, give the IOS a precise, well-framed command so it doesn't have to piggyback on generic authoring. In `Bagira.DDS.DataModel/MapMessages.cs`, add a new command type:
...
private async Task OrchestratePersonalRouteAsync(Guid requestId, int vehicleId, Vector2[] canvasPoints)
{
    ...
    // 1. Convert drawn points to GeoSpatial
    var waypoints = new List<Waypoint>();
    foreach (var p in canvasPoints)
    {
        var (lat, lon, alt) = _geoTransform.ToGeodetic(new Vector3(p.X, p.Y, 0f));
        waypoints.Add(new Waypoint { Position = new GeoPosition { Latitude = lat, Longitude = lon, Altitude = alt } });
    }
...
**2\. Center on Entity** The IOS must retrieve the entity's current physical location and command the IG camera to move there. It does this by querying the local `IDerRepo` for the entity's `GeoSpatial` descriptor. Once it has the `GeoPosition`, the IOS publishes a `MapCommandRequest` with `Type = CommandType.CMD_SET_VIEW`. The `CommandArgsJson` will contain the extracted coordinates (e.g., `{"lat": 45.0, "lon": 12.0, "zoom": 1000}`).
...
In **Bagira.IG/IgApplication.cs**, add the missing `case` to the command polling loop inside the `Update(float dt)` method:
