...
| ⚠ **㉗** | 🔴 **`Fdp.Toolkits.Tests` PRODUCED FOUR FAILURES IN ONE WINDOW AND HAS BEEN GREEN EVER SINCE — UNREPRODUCIBLE, CAUSE UNKNOWN.** 📐 The complete record, `2026-09-20`: **four** full-suite runs red *(three ⇒ `TransientSpawnTagRails` ×2; one ⇒ `SquadInputsP3Tests.AllReaders_ZeroAlloc_After1MillionCalls`, a zero-alloc perf assertion flaky for its own reasons)*, then **seven consecutive full runs at 2252/0** — with parallelism ON and OFF alike — plus `TransientSpawnTagRails` **2/2 isolated**. ⛔⛔ **The failure MESSAGE was never captured**, so nothing was ever diagnosed. ⛔ **My own characterisation flip-flopped three times** *("flaky" → "deterministic" → "order-dependent with an unstable order")*, and the honest verdict is the first one | ⛔⛔ **THE NAMED SUSPECT IS REFUTED, and that is the useful part.** The suspicion was that `TransientSpawnTagRails:103`'s process-global `ComponentTypeRegistry.GetId` could return a different id depending on which of the **26** `ComponentTypeRegistry.Clear()` callers ran first. 📐 **Measured: ids are NOT assignment-ordered** — `GetOrRegisterManaged` (`ComponentType.cs:119-147`) **requires** an explicit `[ComponentId]` attribute and throws without one, so `ScenarioIgnoreTag` is **always 200** whatever ran before. ⇒ **registration order cannot move an id, and this hypothesis is dead.** ⭐⭐ **No fix was applied, deliberately** — adding a `Clear()` to the class's constructor would be a cargo-cult change against an unreproduced failure, and `Clear()` is itself the cross-test hazard here. ⭐⭐ **FIXED SO THE NEXT RED DIAGNOSES ITSELF** *(`2026-09-20`)*: both rails now append a `[trap ㉗ diagnosis]` line carrying `tagId` *(expected 200)*, `registeredOnRepo`, both masks' bit, liveness and the two entity indices — and the serializer rail's `Assert.Single` became an `Assert.True` that PRINTS THE COUNT AND THE KEYS. ⛔ No production change, no behaviour change. ⚠ **And the reproduction window reopened and closed again the same day**: it failed once more in the final gate *(2/2256)*, then **10 further runs were green** and the catch loop *(with `--logger console;verbosity=detailed`)* never fired. ⇒ 🔒 **do not hunt it — wait for it**, and read the diagnosis line when it lands |
