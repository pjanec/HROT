...
current-answer: §3 — the NEXT ACTION is O4 (BTree state and params into occurrence slots), which
  is the design's own STOP-OR-GO gate. Increment A is COMPLETE; B1, B2, B3 (both halves of O3a) and
...
| ⚠ **㉗** | 🔴 **`Fdp.Toolkits.Tests` IS ORDER-SENSITIVE IN THE FULL RUN — a DIFFERENT test fails each time.** 📐 Measured `2026-09-20` over four runs: `TransientSpawnTagRails` isolated **2/2** · with the `O4` rails **8/8** · with a `ComponentTypeRegistry.Clear()` caller **23/23** · full suite run A ⇒ **`TransientSpawnTagRails` ×2 red** · full suite run B ⇒ **a DIFFERENT test red** *(`SquadInputsP3Tests.AllReaders_ZeroAlloc_After1MillionCalls`)* while `TransientSpawnTagRails` passed. ⭐ Named suspect, ⛔ **not diagnosed**: `TransientSpawnTagRails:103` reads the process-global `ComponentTypeRegistry.GetId`, and **five** files in the same assembly call `ComponentTypeRegistry.Clear()` with nothing serialising them | ⭐⭐ **do NOT read a single full-suite red here as a regression** — re-run it, and run the named class ISOLATED before believing it. ⛔ I could not reproduce it deliberately, so the mechanism is a SUSPECT. ⚠ Run B's failure is a zero-alloc perf assertion, flaky for its own reasons ⇒ **the suite is order-sensitive generally**, not "O4 broke spawning". ⭐ The real fix is an xUnit collection grouping the `Clear()` callers with the `GetId()` readers — **filed, not done** |
