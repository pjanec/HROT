...
current-answer: §3 — O4 is DONE and the STOP-OR-GO gate is GO (design §21). The NEXT ACTION is O5,
  then O6. ⛔ Read design §21.2 FIRST: it names three things O4 did not do, one of which (the
  EXTERNAL reset path, BehaviorIngressSystem:204/:235) is an open hole, not a deferral. Increment A is COMPLETE; B1, B2, B3 (both halves of O3a) and
