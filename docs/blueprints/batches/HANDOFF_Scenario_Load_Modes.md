...
current-answer: dispatch pointer for HN-029 — two host-agnostic scenario-load endpoints
  ...
  design: cites Q54 § Scenario load + mgmt-1 §12.
...
📄 **[`Architect_Question_54` § Scenario load](../Architect_Question_54_Cluster_Mcp_Contract.md)** *(Q54-3,
RESOLVED)* — the two endpoints, the host-agnostic `TransitionStateIntent` seam, the live/edit/preview table, the
sequence diagram, and the built-vs-gap table. ⛔ **The state machine is NOT yours to design** — it is owned by
...
## 1. ⭐⭐ THE PREMISE — measured, do not re-derive *(Q54 § Scenario load)*
...
⭐⭐ **Fold the as-built into [`Architect_Question_54` § Scenario load](../Architect_Question_54_Cluster_Mcp_Contract.md)**
