...
| **⑤** | §5: *"extend `SaveWatches`/`LoadWatches`"* | ⛔ Those route to the `[Obsolete]` `WatchPersistence` and are breakpoint-only ⇒ `DebugSessionPersistence` extended instead, **as the dispatch directed**. 🔴 And it has **no production caller** — §7 |
...
| 🔴 **nothing in the editor SAVES a pin set** *(`BP-503b`)* | 📐 **Measured: `DebugSessionPersistence.Save` has NO production caller** — only tests; the live path still uses the obsolete `SaveWatches`. ⇒ the mechanism round-trips under test and is one call away from real, but ⛔ calling it edits `EditorSubsystem` — same collision |
