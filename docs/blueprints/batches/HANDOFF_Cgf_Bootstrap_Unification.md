...
- ⭐ **Seed it from AQ62's (b) inventory + the 5 known drift instances** — create-core, `MutationInterceptor` non-null, scenario-catalog non-empty, perspective toolbar section present, center/rotate routed through the shared systems — and grow it to the full (b) list. ⚠ **Proof it has teeth: reverting any one known fix must turn it RED.**
