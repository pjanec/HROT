...
updated: 2026-08-23
current-answer: dispatch pointer for the UI lane — fix MX-009 (staged writes stop draining after a breakpoint
  hit — a real watch-feature bug) + MX-006 (PanelSnapshot ClearCaptured), then finish observability: U-obs-3
  (the gizmo peer feed, with MX-011) and U-obs-4 (the single-host smoke suite's T2 reads PanelSnapshot).
known-conflict: none.
-->
# HANDOFF — UI lane · **MX-009 (drain bug) + MX-006 + observability leftovers (`U-obs-3`/`U-obs-4`)**
...
| # | task | design | gate |
|---|---|---|---|
| **MX-009** | 🔴 **FIRST — staged writes STOP DRAINING after ANY breakpoint hit.** 📐 *(time lane, HN-122)* `ResumeAndDrainSystem` returns early on **`_staged.IsRewound`**, and **deleting a breakpoint does NOT clear it** — only `RequestContinue`/`RequestStep` do. ⇒ after any breakpoint fires, every later watch/staged write shows `pending=true` **forever**. ⛔ In `DataBreakpointManager` *(your file)*. ⭐ **The fix:** every resume path *(incl. deleting the last/only breakpoint)* must clear the rewound state so draining resumes — ⚠ confirm the intended semantics against the design *(the pause-while-stopped is correct; the STUCK-after-resume is the bug — 📌 `M-41`'s "accepted and silently discarded")*. ⭐ **Already built by the time lane, do NOT duplicate:** `POST /breakpoints/continue` + `/step` clear it explicitly, and `POST …/variable` now reports `willDrain:false` when stopped — your fix makes the **implicit** resume paths correct too | `DataBreakpointManager` · the time lane's HN-122 repro *(stage 42+step drains; arm a Lifecycle bp, fire, delete it; stage+step ×5 ⇒ pending forever)* · `M-41` | a rail: after a breakpoint fires and is deleted, a staged write drains on the next advancing tick |
...
⭐ **Your surface:** `DataBreakpointManager` *(MX-009 — your file)* · `Fdp.Diagnostics.Contracts` *(the gizmo feed
into `PanelSnapshot` + `ClearCaptured`)* · the **in-process** smoke suite *(the `EditorHarness` fixture,
`Hrot.ClusterRunner.Integration.Tests`-side / `DESIGN_Smoke_Suite.md`)*.
⛔ **Do NOT touch:** the time lane's **subprocess** harness *(`Hrot/Runner/Hrot.SystemTests`)*, `Hrot.Editor/DebugApi`
*(the `GET /panels` + `/breakpoints/continue` endpoints are the time lane's — MX-009 fixes the MANAGER, not the API)*,
the engine, the parked Stride tree. ⚠ A cross-lane edit is STOP-and-report *(`R-128`)*.
