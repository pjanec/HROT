...
| **`AS-1b`** ⚠ **RESTATED** | ⭐ **`GlobalTime` is ONE struct doing TWO jobs**, and the call site cannot tell them apart: a **per-frame reading** *(what `Update()` returns and the kernel pushes)* and a **transferable state snapshot** *(`GetCurrentState()` ⇄ `SeedState`)*. ⛔ `DeltaTime` is meaningless in the second, so all five `BuildGlobalTime(0f, 0f)` sites zero it — **and every one of them is CORRECT**: the snapshot · the unknown-mode arm · `Step()` outside `Stepping` · `UpdateBarrierPending` · the slave's `// frozen`. ⇒ ⚠⚠ **the defect is TYPING, not the zeros** — a delta predicate read through `GetCurrentState()` answers *"halted"* forever, and `IsPaused` reads plausibly on both roles | ⭐⭐ **RAIL** `GetCurrentStateHardCodesTheDeltaToZero…` | 🔴 **a trap for callers.** ⭐ **The rule, not a code change: read `GlobalTime` from the LIVE WORLD, never from the controller** |
