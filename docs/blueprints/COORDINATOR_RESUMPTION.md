...
## 3. ⭐ STATE — coordinator HEAD `7d759b04d` *(snapshot `2026-08-26`; confirm by git)*
> ✅ **UI Slice A MERGED** — CGF scenario session, ids **`CE-046..048`** *(CE-047 `MigrationAlertManager.Draw` unwired, CE-048 `LoadScenarioLive` not yet routed — both OPEN)*. Shared `IScenarioSession` in `Hrot.Editor.AiShared.Scenarios`; obligation ⑤ folded into `DESIGN_Cgf_Scenario_Session_Slice.md` §9. ⏳ T3 was 107/0 in the backend run.
>
> ✅ **BACKEND Test-Suite Reliability MERGED** — ids **`QA-001..013`**, `DEBT-AIB-030` CLOSED. ⭐⭐ The crash + rotating flakes + reds were **ONE root cause: a repository/world LEAK** *(kernel released, repositories not)* → OOM → host death. Integration suite now COMPLETES + is stable *(52 stable reds, all base-proven pre-existing — `QA-013`)*; SimHost/Fdp.Toolkits flakes gone *(2037/2037 ×3)*; T3 107/0. Clean merge, both `Hrot.Editor`+`Hrot.CGF` **build 0/0** *(Slice A + backend `Shutdown()` coexist — verified by build, not just textual merge)*. Durable record: `TESTING_Harness_And_Goldens.md` §7.
>   - 🔴🔴 **`QA-006` is a PRODUCT leak** *(SnapshotPool warmup never released ⇒ every node teardown leaks 10 repositories in the shipping runner)* — tell whoever owns runtime memory.
>   - ⚠ **BACKEND HANDBACK (coordinator follow-ups):** **`QA-013`** scope the 52 integration reds *(a programme, not a batch — 28 classes: replication/cluster-transition/recording/mission/EQS)* · **`QA-012`** needs a record/replay batch · **3 ledger rows carry a FALSE mechanism** — `Q52 §6.3`/`ST-026`/`AX-023` say ids assign in registration order; MEASURED FALSE, ids are deterministic, the mechanism is `ComponentTypeRegistry.Clear()` — correct where quoted · `EcsPatchContextTests ×2` pre-existing, belong to the `Q59`/Axis-B merge owner · `tracker-counts.py` counts only `BP-` rows *(so `CE-/QA-/MX-` are invisible — widening is a 1-line coordinator call)*.
>
...
> ⭐⭐⭐ **COORDINATION MODEL CHANGED `2026-08-26` — FRAME-DELEGATION** *(CLAUDE.md WHO-DESIGNS amendment)*: at 3-lane scale the coordinator writes a **FRAME** *(goal + fences + key decisions/leans + design basis + acceptance)*; the **session** does the inventory + authors the UML in a `DESIGN_*` doc as step 1, then builds; coordinator **verifies design+UML on return**. ⇒ the three NEXT handoffs below are **FRAME**-handoffs, not full designs.
>   - **UI/CGF** `claude/reset-working-branch-qd1qpv` — ✅ **E4 MERGED** *(`CE-052`)* — ⭐⭐⭐ **the frame-delegation model's first payoff: the session's inventory OVERTURNED my frame's premise.** Measured: `EntityInspectorPanel` is ALREADY shared on both hosts; `View`/`DerRepo`'s only consumers are condemned dead UI; so E4 was NOT an extraction — the real content was **ONE silent-default defect** *(CGF built a `DataBreakpointManager` but never wired `MutationInterceptor` ⇒ data breakpoints silently didn't fire on CGF)* + a forwarding rail + a `CE-051` fix *(the 3 E3 systems needed `[UpdateInPhase]` — a boot crash T3 caught)*. Design+UML authored by the session *(obligation ④ ✅)*; gap map line 172 corrected by me *(their item ③)*. ⇒ 🏁 **Axis-C extraction E1–E4 essentially COMPLETE** *(E5 = thin-host bootstrap, by construction not a build item)* — "cgf==editor at capability level" is largely done. ⭐ **UI lane FREE** — next candidates need a steer: deferred `CE-047/048/050`, the toolbar/menu-customization AQ, or Axis-B map/selection parity *(gap map item 4)*.
>   - **BACKEND** `claude/blueprint-macro-feature-sdmspn` — ✅ **`QA-013` triage MERGED** *(51 classified, 3 stale fixed, `QA-018` real defect, `QA-017/019..026` refiled)*. ⭐ **NEXT (FRAME): backend-owned refiled defects** — `HANDOFF_Integration_Reds_Backend_Defects.md` *(QA-017 cluster-transition · QA-024 EQS · QA-022 map/area-if-not-UI; investigation-led)*. ⛔ Fenced OFF: `QA-023` blueprint-serialization *(MCP lane owns it now)*, `QA-020` replication *(Axis-B)*, `QA-025` environmental.
>   - **MCP** `claude/mcp-authoring-commands-7l3n49` — ✅ **Blueprint-param PERSISTENCE MERGED** *(HN-124, `MX-030..036`)* — instance-blueprint params now **survive save→reload** *(the 3 engine pieces: Extract diff-emits param bytes, `BlueprintAssignmentDto` gains `byte[]? Params`+`ParamsStructureHash`, Materialization `WriteParamsRegion`; layout-guarded)*; **`Q61-C` CLOSED** *(elevated from deferred by the frame)*; the run-state-aware attach/detach + list MCP wire *(Q61-A/B)*; `QA-023` fixed. Design decision made in-lane *(§3: resolver-shape bytes, not the dead `Overrides` dict)*; round-trip rail green; catalog 98 tools. Verified Fdp.Toolkits+SimHost+Editor build 0/0 combined with E4. ⛔ OUT: same-blueprint-twice *(slot-identity change, separate)*. ⭐ **NEXT: Phase 2 SKILL-usability** *(boundaries generator etc.)* + the **doc-sufficiency exercise** *(both cycles now live — parametric blueprint attach persists)*.
>
