...
>   - **MCP** `claude/mcp-authoring-commands-7l3n49` — ✅ **MX4b MERGED** *(mission editing, `MX-025..029`)*. ⭐ **NEXT (FRAME): blueprint-param PERSISTENCE + the MCP wire** — `HANDOFF_Blueprint_Param_Persistence.md` *(AQ61; the finding: instance params don't persist by ANY path — 3 engine pieces in `BlueprintStateTranslator`/`Materialization`/`AssetignmentDto` + AQ61 A/B + fold `QA-023`; single-instance only)*. ⚠ MCP scope EXPANDS into blueprint serialization this batch — fenced off from backend. ⭐ Phase 2 SKILL-usability + the doc-sufficiency exercise *(both cycles now live)* still queued behind it.
>
