...
0c. 🔴 **NEXT — §6 HANDBACK (feature parity), user `2026-08-26`: "i need feature parity, and menus shown just for available features."** The menu MECHANISM already discharges the second half *(item exists only for a serviceable command, ruling 49)*. The remaining gap is the FEATURES. Scoping in `batches/REPORT_Cgf_Menu_Follows_Focus.md` §6:
   ...
   - ✅ **§6.2 — RESOLVED via AQ60; Slice A HANDOFF READY.** The user ruled *(2026-08-26)*: CGF ≡ editor for scenarios, code shared. 📄 `Architect_Question_60` *(§3b ruling, §4 grounded design)* · design `DESIGN_Cgf_Scenario_Session_Slice.md` *(READY-TO-BUILD, UML §4/§5)* · handoff `HANDOFF_Cgf_Scenario_Session.md`. **Slice A:** extract a shared `IScenarioSession` facade *(the scenario half of `EditorApplication` — assembly wall + editor-window-half block sharing the god-object intact; the scenario half is cleanly separable, world is a ctor param)* → instantiate in both; `File/Scenario` on both hosts *(Load-for-Edit/Load-for-Live over HN-029; New mode-branched; Save edit-mode; Checkpoint-**Save**)*. ⭐ UI/CGF lane; rule-4 re-pull.
