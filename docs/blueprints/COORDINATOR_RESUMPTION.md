...
0b. ⭐ **UXI-05 menu-follows-focus** — ✅ **HANDOFF READY** *(design written + §6 global-lean approved, `2026-08-26`)*. UXI-05 step-1 registry model *(`MenuItemNode`→`List<MenuBinding>`, `string? perspective`, draw-time resolution, editor byte-identical)* + extend `CgfEditorShellToolbar` to register the common-core `File` menu *(GLOBAL)* + `SUBSET-BY-DESIGN` menu verdict. 📄 design `DESIGN_Cgf_Menu_Follows_Focus_Slice.md` *(READY-TO-BUILD, UML §4/§5)* · handoff `HANDOFF_Cgf_Menu_Follows_Focus.md`. ⛔ **DEFERS** the four gizmo-block cleanup *(UXI-05 steps 2/3 = UXI-13 — CGF is in none of them; cross-subsystem `--mode all` gizmo-menu walk)*. ⭐ UI/CGF lane; rule-4 re-pull *(hot files)*.
