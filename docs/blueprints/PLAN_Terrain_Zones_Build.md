...
| **B5** | Introduce **terrain as a SINGLETON concept** — a GLOBAL scenario block beside `Header.TkbName`, and an ECS singleton in host memory | a scenario round-trips the terrain global (identity + asset references); the loader reads identity from it, not from a per-entity component. ⛔ **Not an ordinary entity, and not a new parallel section** — it rides the global block `TkbName` already occupies | design **§2.1e ①②** ⚠ **resolves §3-U4** |
...
| **C5** | Introduce the **terrain loader** as a `IClusterStateHandler` mirroring **`TkbLoadClusterStateHandler`** — registered **UNCONDITIONALLY on every ECS host** and ordered **before** the scenario handler | loading a scenario whose terrain global references a road network yields a populated `ZoneEnvironmentData` **without** any `Zones` section, **on a pure MuscleGround node too**. ⛔⛔ **The rail that matters: a node with NO authoring deps still loads terrain** — see §2.1e ④ | design **§2.1e**, **§2.1d** ⚠ **resolves §3-U8** |
...
| **U4** | terrain identity has nothing to compare against | 🔒 *"Terrain is **not an ordinary entity** as others, it is **by design a singleton concept** and a special one already being handled in a special way (or should be — loading various assets etc). Scenario persistence for such special singleton is not a problem."* ⇒ ⭐ **new task B5.** ⛔⛔ **My 'ordinary entity' lean is RETRACTED.** ⚠ And this does NOT reopen the retired `Zones` section: that was a **content bundle duplicating entities**; terrain is a **global fact**, in the same class as `$meta` and `Header.TkbName`, which §6a already keeps as globals |
