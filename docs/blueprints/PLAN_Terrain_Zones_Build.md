...
| **B5** | Introduce **terrain as a SINGLETON concept**, carrying the terrain identity + its asset references (road networks first), and persisted with the scenario | a scenario round-trips the terrain singleton; the loader reads terrain identity from it. ⚠ **It is NOT an ordinary entity** — ruled `2026-09-17`: terrain is singular by design and already needs special handling (asset loading), so special persistence for it is acceptable | design **§2.1d** ⚠ **resolves §3-U4** |
