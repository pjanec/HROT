...
### E3 — Place + bind params (typed filter / Promote) · PARTIAL (blocked by I4)
A compiled AiPrimitive's *name* already appears in the method picker — it's drawn from `BehaviorRegistry.GetRegisteredNames()`, which AiPrimitive hot-reload populates (`BTreePickerDrawers.cs:63-108`) — so it can be **selected**. But `ExpressionTargetField` type-safe variable filtering and "Promote to new variable" both call `IActionSchemaExporter.Lookup(fqn)`, which returns null for it, so binding degrades to an **unfiltered** variable list and Promote is a no-op (`BlackboardFieldPickerAttribute.cs:31-36`; `BTreePickerDrawers.cs:237-258`).
**Fix:** falls out of I4 (once the exporter knows the blueprint action's DTO type, the existing generic binding UI works).
