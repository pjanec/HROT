...
| finding | evidence |
|---|---|
| ⭐⭐ **Mission editing over MCP was SPECIFIED but never built.** Missions are the *proper* way a behavior attaches to an entity (as a task); edit-time and runtime are the **same** `CommitMissionAsync` path *(no difference — the cgf==editor thesis)* | ✅ **BUILT 2026-08-26 (Batch HN-123, MX-025..029)** — `GET/POST/DELETE /missions/*` registered in `DebugApiHost.cs`, 4 MCP tools; as-built + 4 deviations in `MCP_Integration.md` §"AS-BUILT — MX4b". *(was: build-state READY-TO-BUILD, gated on MX-002; no `/missions/*` route registered)* |
...
|---|---|---|---|---|
| **1 — commands** ✅ **BUILT (HN-123)** | **MX4b · mission editing** ⭐⭐⭐ | `GET /missions/{id}` · `POST /missions/{id}/task {behavior,params}` · `DELETE /missions/{id}/tasks` · `POST /missions/{id}/run {restart?}` — each **read-snapshot → modify → `CommitMissionAsync(id, plan, baseVersion)`** (OCC); run = `SendControlCommandAsync`. ⚠ **As-built:** `params` is stored **verbatim** as `BehaviorParams` (the engine reads it with plain JSON) — `ScenarioSerializer` is the discovery/MX4a side only; the shape is discovered via the already-built `GET /behaviors` (MX4a) | `MCP_Integration.md` §Group P + §UML + §"AS-BUILT — MX4b" | §3 (MX-002 — RESOLVED below) |
