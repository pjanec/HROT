...
| # | correction |
|---|---|
| **①** | ⭐⭐⭐ **P3 IS NO LONGER BLOCKED ON USER DECISIONS.** ⛔ The `P3` row above and this file's STATUS both say *"its §5 holds THREE OPEN DECISIONS that are the USER's to settle before it starts."* 📐 **That was true on `2026-09-03` and is FALSE now:** [`DESIGN_Role_Affinity_Ownership.md`](../DESIGN_Role_Affinity_Ownership.md) was updated `2026-09-10` and **all three are resolved** — ① / ①b settled `2026-09-01`; ② **user-approved `2026-09-10`** *("yes on boot warning")*; ③ **user-ruled `2026-09-10`** *(build the `IRoleShardProvider` seam now, defer only the implementation)*; and ①c's own column now reads *"nothing measurable remains."* ⇒ ⭐ **the only thing left in §5 is a per-system REVIEW, not a decision.** ⚠ What DID grow is step **3b**: §3.5 named ONE un-gated tick system and there are **SEVEN**, three of which WRITE cognitive state — the HSM tick of the day (since merged into `BrainTickSystem`) being the non-negotiable second. ⇒ **a per-system pass, not a one-line query edit.** |
| **②** | ⛔⛔ **A FOURTH `READY-TO-BUILD` DESIGN OF THIS PROGRAMME IS ABSENT FROM THIS PLAN** — 📄 [`DESIGN_Entity_Authoring_Surface.md`](../DESIGN_Entity_Authoring_Surface.md) *(`build-state: READY-TO-BUILD`, `updated: 2026-09-03`)*, the **ONE-method `RequestEntityCreation(owner, …)`** surface that **SUPERSEDES `DESIGN_Entity_Creation_Unification.md` §3.4's two-method shape**. 📐 Measured `2026-09-11`: `RequestEntityCreation` has **zero** occurrences in `.cs`, and the two superseded names survive **only in three doc comments** *(`EntityCreation.cs:55-56`, `EntityCreationPack.cs:52`)* — so it is **designed, promised in those comments, and unbuilt**. 🔴 **Searched this file, `DESIGN_Role_Affinity_Ownership.md` and `PROGRAMME_Unification_And_Harness.md` for it: ZERO references** ⇒ it was never sequenced against `P2`/`P3`. ⭐ It carries **12 acceptance criteria** including deleting `IgEntityCreationRequests` and closing defects **`G1`** *(an authored AREA never reaches the request source — `ScenarioSpawnAdapter` still publishes the raw command)* and **`G2`** *(`_ = _nameResolver`, a resolved name discarded)*. ⚠ **It is INDEPENDENT of `P2`/`P3`** *(a caller-side affordance; they are ownership and registration)*, so it can run in parallel — ⛔ but it must not stay invisible to the plan, which is how `P1`'s completion went three days unrecorded. |
