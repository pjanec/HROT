...
>
> ⛔⛔ **AND A NEW 🔴🔴 FINDING, from fixing the visual-check guide:** `VariableEditModal.Draw()` has
> **ZERO callers, production or test** ⇒ ⭐ **the edit dialog still cannot appear.** 📄 **group `A`, row
> `A6`.** ⚠ **Batch 87's `TheEditDialogIsDrawnTests` is green and cannot see it** — it constructs the
> modal itself. 📌 **Third consecutive turn of one pattern: built, exposed, never reached.**
>
> ⭐ **The visual-check guide is REWRITTEN** — 📄 [`GUIDE_Blueprint_Visual_Check.md`](GUIDE_Blueprint_Visual_Check.md):
> five wrong rows corrected, the BTree/HSM fence removed, and **part `H`** added for the new Details
> panel. ⭐⭐ **`B3`/`B8`/`D1b`/`E6b` are now REGRESSION rows** — Batch 87 fixed all four.
>
...
> ⭐⭐ **`Q41` ruled IN FULL** *(`A`/`B`/`D`, then `C1′`/`C2′`/`C3′`)*; ⭐⭐ **`Q43` ruled IN FULL** — 📄 **[`Q43`](Architect_Question_43_Blueprint_Authored_Param_Resolver.md)** — 📄 **[`Architect_Question_41_Blueprint_Driving_BTree_Params.md`](Architect_Question_41_Blueprint_Driving_BTree_Params.md)**.
>
...
| ✅ **C4** | ⭐⭐⭐ **A resolver authored AS A BLUEPRINT** — 📄 **[`Q43`](Architect_Question_43_Blueprint_Authored_Param_Resolver.md)**, ✅ **APPROVED IN FULL `2026-08-18`** *(`R-94`)* | ⭐⭐ **Two of the three hard parts already exist** — the hook *(`C2`)* and **`IrOp_MakeStruct` / `IrOp_SetMembers`**, both BUILT with live emit arms. ⭐⭐ **And so does the ENTRY-POINT KIND** — `GraphKind.Construction` is a **reserved, unconsumed slot** *(`R-93`, `Q23`)*. ⛔ **No new dispatch kind.** ⇒ the delta is **one emitter arm for `IrGraphKind.Construction`** + a **`V_ResolverPurity`** validator + the picker filter. ⚠ **Sequenced THIRD** — after `C2` and `C3` |
...
| ⭐⭐ **the Instance override half** | `BlueprintAssignmentDto.Overrides` is **designed and forward-compatible, empty in MVP** — 📄 `.dev/_DONE/blueprint-scenario/BLUEPRINT-SCENARIO-DESIGN.md` §6, deferred for ONE stated reason: *"the authoring UX ('where is a per-instance override edited?') is unsettled."* ⇒ ⭐⭐⭐ **that UX is Track C.** The two are one work item, and nobody had connected them |
...
| | |
|---|---|
| ✅ **safe to park** | `Q33-D`'s widening is **runtime-only** — the slot table is `[DataPolicy(NoScenario)]` and the scenario format is already a per-assignment list ⇒ **a later change, not a migration** |
