...
| ⭐⭐⭐ **`Q39` — merge `Variables` + `Working State` into ONE section** | 📄 **[`Architect_Question_39`](Architect_Question_39_Merge_Variables_And_Working_State.md)**. ⭐⭐ **The user asked, and the answer is their own prior ruling:** `Q32` ruling 8 — *"as the global vars and working state vars are **the same stuff**, it makes no sense to emit them differently."* ✅ **The emitter union SHIPPED** *(`IrAsset.cs:95`)*; ⛔ **the outline is the leftover** — the `C-sections` batch was solving VISIBILITY, not classification. ⭐ **`Inputs` stays separate** *(different backing type, different lifecycle, excluded from the IR union)*. 🔴🔴 **`Q39-B` is the sharp one:** the merged `[+]` must derive the kind from the asset's DISPATCH KIND — ⛔ **picking wrong creates the first asset ever with BOTH lists populated**, and the union is order-sensitive *(`AiPrimitiveLowering.cs:61` — off-by-one)*. ⚠ **Nothing asserts the `0 with both` invariant today** |
