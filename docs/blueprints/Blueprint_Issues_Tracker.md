...
- [ ] **[BP-93](Blueprint_Issues_Detail.md#bp-93)** 🔴 · `RW-L` — **The editor writes tracked assets to disk without an explicit Save.** *User: "I never saved the BTree and HSM so it is very weird they got saved automatically — this is undesired."* After a session with **no Save invoked**, `git status` showed `PlatoonHillAttack2.btree.json` and `SampleGuard.hsm.json` modified: exploratory edits **persisted anyway** (BTree gained `NewVar1`; HSM gained `halo1`, **two new `State` nodes**, `__Root` lost a child and moved `X 0→395,Y 0→130`). Both files were also **fully reformatted** — hand-authored compact JSON expanded to multi-line — so the real change hides in ~160 lines of noise and the diff is near-unreviewable. ⇒ scratch experiments silently become committed-asset changes and there is **no safe way to explore an asset**. **Investigate the trigger** (document close / perspective switch / exit / timer — `AiDocumentManager`, `saveDocument`?), settle the policy (explicit Save + dirty marker + prompt on close), and make the serializer round-trip-stable. ⚠ See **[BP-94](Blueprint_Issues_Detail.md#bp-94)** — *found after the batch-19 visual check*
- [ ] **[BP-94](Blueprint_Issues_Detail.md#bp-94)** 🔴 · `RW-L` — **A load→save round-trip changes fields the user never touched.** Independent of *when* the editor saves ([BP-93](Blueprint_Issues_Detail.md#bp-93)), *what* it writes is not value-preserving: `PlatoonHillAttack2`'s `Blackboard.Variables["state"].IsAutoManaged` went **`false` → absent/null** although that variable was never edited, and `SampleGuard`'s `Blackboard.Managed` went **`false` → `true`**. `IsAutoManaged` is load-bearing for the HillAssault2 shared-state wiring, so "false" → "unspecified" is not cosmetic. Candidates: the writer **omits default-valued properties** and the reader defaults them differently (asymmetric-default round-trip), and/or `Managed` is **inferred on save** from variable count rather than round-tripped. ⚠ The `Managed` flip may be intended (adding `halo1` is a plausible trigger) — **confirm before calling it a bug**; `IsAutoManaged` has no such excuse. **Test to add:** load every shipped `.btree.json`/`.hsm.json`/`.bp.json`, save untouched, assert deep equality — one test catches both fields and BP-93's reformatting — *found after the batch-19 visual check*
