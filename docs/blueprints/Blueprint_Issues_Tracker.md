...
- [ ] **[BP-87](Blueprint_Issues_Detail.md#bp-87)** 🔴 · `RW-H` — **The parameter/variable type dropdown offers 8 types the compiler cannot resolve.** *Found 2026-08-08 while fixing BP-84/85/86 — the user's own `SquadState1.bp.json`, authored during the visual check, **fails the build**: `BP1500: Pin type 'Vector3' does not resolve`.* The combo is filled from `BlackboardTypeHelper.DefaultKnownTypeNames` and persists the **bare display name** as `TypeId` (`GraphSignatureWindow.cs:361`), but `StaticTypeRegistry.TryResolve` only accepts an exact `TypeTable` hit or a `global::` FQN, and its alias table is just `bool/byte/short/int/long/float/double` (`StaticTypeRegistry.cs:83-89`). `Vector2/3/4` + `Quaternion` **are** registered — under their FQNs only (`:38-42`) — and `sbyte/ushort/uint/ulong` are absent under any name. ⇒ **8 of 15 offered types produce BP1500**, with no author-time warning; it lands at *build* time in another project. 📐 **Needs an architect call on scope, not on mechanism:** the vector four clearly want an alias→FQN mapping, but whether the four unsigned integer types should be legal blueprint types at all is a design question (no coercion-table entries either) — register them or remove them from the dropdown; silently offering them is the one certainly-wrong option. ⚠ Durable fix is to derive the dropdown from what the compiler can resolve so the two hand-maintained lists cannot drift again. ⚠ While this asset is present `dotnet build` of the solution fails (the blueprint compile is a build step of `Hrot.AI.Behaviors`) — *found while fixing BP-84/85/86*
...
- [ ] **[BP-92](Blueprint_Issues_Detail.md#bp-92)** 📐 · `RW-H` — **Where do Functions live, and how are they shared?** *User: "why are functions stored together in an instance blueprint json? how can they be shareable with other blueprints? should they be in separate blueprint files? How does Unreal do that?"* Today a Function graph lives **inside the owning asset's `.bp.json`** and cross-asset calls go through the peer mechanism (`CallPeer.{guid}`, BP-68) — there is no shared function-library concept. **Architect questions:** (1) is asset-private + peer-call the intended model, or should a shared function-library asset exist? (2) what does `Dispatch: Instance` mean for a functions-only asset like `SquadState`? (3) Unreal distinguishes per-Blueprint functions from **Blueprint Function Libraries** — ⚠ Claude cannot verify Unreal's semantics from this repo, so **confirm the parity target before it justifies a design**. Upstream of **[BP-88](Blueprint_Issues_Detail.md#bp-88)**, touches **[BP-75](Blueprint_Issues_Detail.md#bp-75)** — *raised in the batch-19 visual check*
