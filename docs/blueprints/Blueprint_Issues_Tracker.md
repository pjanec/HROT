...
- [ ] **[BP-86](Blueprint_Issues_Detail.md#bp-86)** 🔴 · `RW-L` — **Function input parameters render with corrupted names on the entry node.** Adding three inputs yields pins labelled **`P1?am0` / `P2?am1` / `P3?am2`** instead of `Param0/1/2`. The corruption is **positional and structured**: two bytes at **offset 1–2** are overwritten by *(row index + 1)* as an ASCII digit plus one unprintable byte, with the `am{i}` tail intact — a **buffer/encoding** defect, not a bad format string. ✅ **Four layers already eliminated:** the generator (`GraphSignatureWindow:386` = `$"Param{Count}"`), the model (`GraphSignatureEditModel.AddParameter:61-71` stores the name verbatim), the signature-window row buffer (`DrawParameterRows:338-348`), and the display-label resolver (`BlueprintGraphModel:318-332` early-returns unless the pin is named `"Value"`). ⇒ suspect `BlueprintPinModel`'s label path or the NodeEdit canvas label renderer — the row-dependent digit points at an ImGui id/label buffer (`PushID($"name_{tableId}_{i}")` sits adjacent). 🔬 **Split it in 10 seconds:** save and read `Inputs[].Name` — `Param0` ⇒ render-only (downgrade); `P1?am0` ⇒ persisted corruption that reaches the compiler as an identifier — *found in the visual check*
