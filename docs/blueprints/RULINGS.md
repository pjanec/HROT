...
| ⭐⭐⭐ **R-75** | ⭐⭐ **A WATCH'S ENTITY IS A `NetworkIdentity.Value`, NEVER AN `Entity` HANDLE** *(user requirement `2026-08-18`: watches survive scenario restart)*. ⛔ **`Entity` is a slot/generation handle, recycled on restart**; ⭐ **`NetworkIdentity` is the replication identity and scenario load ALREADY REMAPS IT** ⇒ **the planning→runtime translation exists.** ⚠ **MEASURE FIRST: does a planning-time entity carry one, preserved into the run?** | `NetworkIdentity.cs:11` · `Q40` §9 |
...
R-75 | FDP/Toolkits/Fdp.Toolkits/Replication/Components/NetworkIdentity.cs | Global ID (GUID-like or sequential unique ID)
