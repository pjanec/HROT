...
| 🔴🔴 **R-86** | ⚠⚠ **UNRULED CONFLICT — §5 vs §6 of ONE document.** ⭐ §5: *"node-owned (`IsAutoManaged`) rows never get a writable dialog, in either mode."* ⛔ §6: the planning write path **IS** a node-owned variable's `DefaultValueJson`, and the Inspector panel is *"the only LIVE surface"* for it. ⇒ ⛔⛔ **sequencing rows 58/59 would ship a dialog that refuses exactly the parameter variables**, making the Inspector panel PERMANENT — against ruling 9. ⭐⭐ **My recommendation, awaiting the user: the editor owns the variable's IDENTITY, the designer owns its VALUE** ⇒ narrow §5 to *not renamable / not deletable / no RUNNING write*, ⭐ **planning default EDITABLE**. ⚠ 🔒 passthrough stays excluded in both modes | `DESIGN_Variable_Details_And_Editing.md` §5 |
...
R-86 | docs/blueprints/DESIGN_Variable_Details_And_Editing.md | never get a writable dialog, in either mode
