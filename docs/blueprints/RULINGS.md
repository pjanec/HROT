...
| **M-3** | What still treats `WorkingState` as distinct from `Variable`? | `grep -rn "DeclarationKind.WorkingState" --include=*.cs Hrot/ \| grep -v Tests` | `2026-08-18` |
| **M-12** | Do any shipped assets declare BOTH `WorkingState` and `Variable`? *(decides whether a kind collapse can move any offset)* | census all `*.bp.json` by the `Kind` values present — ⭐ Batch 56 measured **0 of 458** | ✅ **`2026-08-18`, run by the coordinator: 541 assets scanned, ZERO declare both.** 194 `(Variable)` · 32 `(Parameter, WorkingState)` · 7 `(Parameter)` · 5 `(WorkingState)` · 303 none ⇒ ⭐⭐ **the merge is order-neutral for the whole corpus** |
