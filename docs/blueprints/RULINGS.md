...
| 🔴🔴 **M-20** | Do Approach A alias bindings SURVIVE a save/reload? | `grep -n "_aliases\[" Hrot/Subsystems/AI/Hrot.BTree.Editor/Model/BehaviorTreeAsset.cs` and `grep -rn "Alias" --include=*.cs Hrot/Subsystems/AI/Hrot.AiEditor.Persistence/` | ⛔⛔ **`2026-08-19`: NO — CONCLUSIVE, and it is a DEFECT.** The only writes to `_aliases` are **rename** *(`:409`)*, **`AddAlias`** *(`:504`, the drag-drop UI)* and **prune** *(`:536`)*; ⛔ **nothing on the LOAD path touches them**, and the persistence assembly has **no alias field at all**. ⇒ ⭐ **drag-to-alias, the type-match check, the cross-region refusal and the `↳ aliased by:` badge all evaporate on reload.** ⭐⭐⭐ **And the design SAYS it should persist** — `BTree_HSM_JSON_Persistence_Detailed_Design.md:132` lists *"**subtree sync bindings**, **alias relationships**, **conflict/unused suppressions**"* together; ⚠ **the other two were BUILT and aliases were not.** ⏭ **Batch 91** |
