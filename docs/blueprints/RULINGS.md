...
| **M-12** | Do any shipped assets declare BOTH `WorkingState` and `Variable`? *(decides whether a kind collapse can move any offset)* | census all `*.bp.json` by the `Kind` values present — ⭐ Batch 56 measured **0 of 458** | ⚠ **`2026-08-18` via `BP-244`'s record — RE-MEASURE before relying on it** |
