...
| **E8b** ⛔ **NEXT in this line** | the **`ParamResolverRef` property** — reuse + naming a curated C# resolver | ⭐ free on disk: `[JsonIgnore(WhenWritingNull)]` is the established convention on both sides, and `StructureHash` hashes no asset-level scalar ⇒ **zero asset movement, zero hash movement** | ⚠ needs a **runtime-join** check: a behaviour→blueprint reference cannot be compile-time validated *(`V_PeerReferences` works only within one compilation)* |
| **E8c** ⛔ **the EXPENSIVE one — its own design pass** | the **per-VARIABLE** half for BTree/HSM behaviours | ⭐ the carrier EXISTS — `ManagedBlackboardVariable(Name, Type, ByteOffset)`, emitted by BOTH bridges — and the resolve site has everything in scope *(after Step 2 in `__parseParams`: the DTO type, the offset, `world`, `self`, `host`)* | ⛔ but it moves a DTO field on **two** asset kinds, a 4th member on the manifest record, a Step-3 emit in **both** bridges, the scanner join — **and a runtime granularity that does not exist today** *(the curated mechanism is whole-behaviour)*. 📐 31 of 32 params variables are whole structs, so `ResolveParams<TDto>` fits |
