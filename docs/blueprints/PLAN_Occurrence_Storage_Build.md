...
| **E4** | **`O7c`** — delete `BrainHsm64`/`BrainHsm128` **and `BrainBTreeState`** | instances into slots; `F9`'s tick-system reshape | ⛔⛔ **RE-RATED AND RE-SEQUENCED `2026-09-22` — 📄 `DESIGN_Occurrence_Scoped_Storage.md` §31.5.** ~~🔴 L — 188 refs / 18 production files~~: 📐 **the 188 counted TESTS**; production is **42 lines / 12 files**. ⭐ Now **five ordered steps, BTREE FIRST**: ① delete `BrainHsm64` — ✅ **DONE `2026-09-22`**, free, because nothing in production ever attached it *(§31.11)* · ② the **BTree** root into a slot (`CE-319`) + extract the shared `BrainTickWalk` — ⭐ the slice that PROVES the model and the only one the golden test can SEE · ③ `CE-318` · ④ the HSM instance into a slot · ⑤ `HsmDebugSession` as a list |
