...
| **E8** ⛔ **HELD — the SELECTION property** *(`R-149`)* | a params REGION names its resolver | ⭐ the DATA half lands without the editor; ⛔ the PICKER half is `Q41-C2′` and the UI lane is held | ⚠ granularity: per-ASSET for a blueprint, per-VARIABLE for a behaviour blackboard — **one rule, one resolver per params REGION** *(§7.2)* |
