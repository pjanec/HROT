...
| **`P3`** ⭐⭐ **THE REAL ITEM — "UNIFY PARAMS INTO SLOTS", not "delete the component"** | ⭐ the **root** behaviour's params get a slot *(role ①)*, and ingress **SCATTERS** each state's slice into its slot at assign time *(role ③)* instead of leaving a table for occurrences to copy | 📐 **PRICED `2026-09-21`: the params role is `BehaviorParameters` — 28 production files / 60 refs** *(the component as a whole is 66 files / 176 refs; ⛔ do NOT price this off that number)*. ⚠ **The old "136 refs / 17 files" figure was never about this** — it was `BrainBTreeState` | ⛔ **open, unscheduled** |
| ~~**`P3a`**~~ ✅ **DISSOLVED `2026-09-21`** | ~~where the seed layout lives once ② has no blackboard~~ | ⭐ **the question was wrong.** Slots are **provisioned EAGERLY at assign time already** — `BehaviorIngressSystem.cs:140-165` calls `TryGetHostedOccurrenceDemand` → `ProvisionStatefulSlots`/`EnsureOccurrenceStore`, off a demand computed at **scan** time. ⇒ ingress can scatter straight into them; the parse still lands transactionally in the **stack shadow** that already exists at `:92`. ⛔ **There is no persistent seed layout to re-home** | ✅ |
| **`P4`** ⛔⛔ **NOT "DELETE" — the component STAYS, and that is FORCED** | ⭐ what remains of `BrainBlackboard` after `P3`: **the BTree action's blackboard handle**, the ingress scratch, and a replicated component | 🔴 **see the ABI finding below** | — |

#### 🔴🔴 WHY `BrainBlackboard` CANNOT BE DELETED — **and it has nothing to do with the root occurrence** *(measured `2026-09-21`)*

| 📐 the three structural holds | |
|---|---|
| ⛔⛔⛔ **THE BTREE ACTION ABI IS HARD-TYPED TO IT** | `BTreeActionRegistryFactory.cs:35,39,64` — **`ActionRegistry<BrainBlackboard, BTreeContext>`**, and **every** BTree action's `RegisterAll` must take exactly that type or it is skipped. The interpreter hands `ref bb` to every action ⇒ **every curated and generated action body is `(ref BrainBlackboard bb, …)`**. ⭐ `DEC-05-SPEC` already recorded this: *"runtime is hard typed to `BrainBlackboard`."* ⇒ **deleting the type is an ABI change across every action** |
| ⛔ **IT IS A REPLICATED COMPONENT** | `BehaviorTkbTranslator.cs:46` yields `typeof(BrainBlackboard)` as a TKB type and `:125` attaches it on receive ⇒ removing it changes the **replication surface**, not just local storage |
| ⚠ **ROLE ④, which `E5a` also never named** | `HsmHostVariableAccess` *(`E7a`)* holds **a pointer into the host's `BehaviorParameters`** — the host-variable-read surface. ⭐ Tractable *(it re-points at the root's slot)*, but it is a fourth consumer of the params region |

⇒ ⭐⭐⭐ **The user's own shape is therefore the CORRECT target and is FORCED, not a compromise** *(user, `2026-09-21`: "I do not need to retire completely if it can play useful role — not storing params, this should be unified into the occurrence slots")*: **params storage unifies into occurrence slots; the component stays** as the BTree action handle + ingress scratch + replicated state. ⛔ **"Retire `BrainBlackboard`" was the wrong goal all along** — the right one is *"`BrainBlackboard` stops being a params STORE."*
...
| ⭐ **the PARAMS UNIFICATION** *(b, restated)* | **`P3`** — **28 files / 60 refs**, no declined prerequisite, no unowned design question. ⛔ **It does NOT depend on `O8`**, and `P3a` is gone. ⚠ **`P2` should land first** so the curated path is already on the seam when the root joins it |
| ⛔ **DELETING the component** | 🔴 **not achievable without an ABI change to every BTree action** — and after `P3` there is no reason to want it |
...
| ⭐ **the STORAGE** — where params live | ⛔⛔ **RE-AIMED `2026-09-21`: this was written as "the DELETION", and deleting the component is NOT achievable** — the BTree action ABI is hard-typed to it *(`BTreeActionRegistryFactory.cs:35`)* and it is replicated *(`BehaviorTkbTranslator.cs:46`)*. ⭐ The real goal is **params storage unifies into occurrence slots** — `P3`, **28 files / 60 refs**, with **no** declined prerequisite. 📄 **The ordered path `P0`…`P4` is in *"THE PATH"* above**, and it names **two further roles this row never did** *(the ingress commit target; `HsmHostVariableAccess`'s pointer into `BehaviorParameters`)*. ⚠ The old `P3a` *(where the seed layout lives)* is **DISSOLVED** — slots are already provisioned eagerly at assign time |
