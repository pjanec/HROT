...
| **Q56-F** | Relationship to the write-path / R-52 | ✅ **AI-asset authoring writes the ASSET FILE → hot reload — the SAFE path, distinct from the live occurrence-slot value write (R-52)**. ⇒ ⛔ authoring does not touch the frozen variable-model write path |
