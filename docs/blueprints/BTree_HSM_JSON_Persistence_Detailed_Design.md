...
> **Note — root-params ceiling:** each behaviour's params region is sized to `RootParamsBytes(def)` and seated structurally by the tier allocator, whose ceiling is the largest tier's payload, **16 096 bytes** (`BlueprintBlackboard16384` — see `DESIGN_Occurrence_Scoped_Storage.md` §30.15). ⚠ **But a 100-byte cap is still enforced in five places** — `BehaviorConstants.MaxBehaviorParamByteSize`, `BehaviorParameterSizeAnalyzer` (`FDP_001`), `Stage2_Validate` (`BP1200`), `BTreeBlackboardPackHelper.MaxInlineBytes` (pinned by `InlineBudgetConstantAgreementTests`) and a runtime throw in `BehaviorRegistry` — so size against **100** until `CE-307` removes them. The interrupt registers `ExpectedThreatLevel`, `Interrupt_MobilityLost`, `Interrupt_Reserved` live in their own `BrainInterrupts` component, untouched by this move ([BehaviorConstants.cs](FDP/Toolkits/Fdp.Toolkits/Behavior/BehaviorConstants.cs)). The `HillAttackDtos.cs` "60-byte" comment is **stale**. The bin-packer must size per behaviour via `RootParamsBytes(def)`, not a fixed constant (Slice 1.5).
