...
> **Note — root-params ceiling:** each behaviour's params region is sized to `RootParamsBytes(def)` and seated structurally by the tier allocator, whose ceiling is the largest tier's payload, **16 096 bytes** (`BlueprintBlackboard16384` — see `DESIGN_Occurrence_Scoped_Storage.md` §30.15). ✅ **`CE-307` (`2026-09-22`) replaced the old 100-byte guard with that same capacity bound** — `BehaviorConstants.MaxRootParamsByteSize` is now `Tier16384PayloadSize`, and `FDP_001` refuses only a region no tier could hold. The interrupt registers `ExpectedThreatLevel`, `Interrupt_MobilityLost`, `Interrupt_Reserved` live in their own `BrainInterrupts` component, untouched by this move ([BehaviorConstants.cs](FDP/Toolkits/Fdp.Toolkits/Behavior/BehaviorConstants.cs)). The `HillAttackDtos.cs` "60-byte" comment is **stale**. The bin-packer must size per behaviour via `RootParamsBytes(def)`, not a fixed constant (Slice 1.5).
