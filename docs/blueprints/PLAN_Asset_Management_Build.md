...
| # | task | success condition | owning chapter |
|---|---|---|---|
| **B1** | ⭐⭐ **Derive** `hrot.asset.needs.*` from `RoleLoadRequirements` through the **3-row `LoadPart → AssetKind` adapter**, ⭐ **plus the one-line rule `Brain ⇒ all AI asset kinds`** — ⛔ do NOT author a parallel table, ⛔⛔ do NOT extend `RoleLoadRequirements` *(it would claim a load step nobody implements)*. ✅ **RULED — `Q72-M`, design §7.3a carries the whole map.** ⭐⭐ **The AI-kind set is a PREDICATE — *the contributor has a non-null `BaseFolder`*** — ⛔ **not a list**: `AssetRoots.AssetsRelative:197-204` **throws** for `Blackboard`/`Utility` | the token set is **computed** from the requirement table; changing the table changes the tokens with **no second edit**. ⭐⭐ **Rail `Map2D` explicitly: it gets the KNOWLEDGE BASE and NOT terrain** — ⛔ *"Map2D gets nothing"* would throw `FileNotFoundException` in `KnowledgeBaseLoadStep` on IG. ⭐ Rail the subtraction: a role with no declared consumer loses **that part**, never the node. ⭐ Rail that a rootless kind yields **no** token rather than throwing | design **§7.3**, **§7.3a**, **§6 caption**; `Q72-L` |
| **B2** | `hrot.asset.authors.*`, and 🔴🔴 **THE SUBTRACTION: a node is NEVER an automatic sync target for a kind it advertises as author** | ⛔⛔ **This is the task that stops `B` destroying authored work.** 📐 `CgfSubsystem.DefaultRole = NodeRole.Brain` ⇒ every CGF **is** a Brain host **and** an authoring host reading `AssetRoots` (`:2078`, `:2506`) ⇒ without the subtraction, the mirror lands **on the folder the author is editing**. ⭐⭐ **Rail it as a LOSS test, not a filter test:** author a file locally, sync against a NAS that has never seen it, assert **the local file still exists and is unchanged**. ⭐ Also: *"nobody authors K"* stays **legal** (the external-tool case) | design **§7.3b**, **§6**; `Q72-J`, `Q72-M` |
...
| **W6** | ⭐ **nothing — it does NOT block B** | the **mirror of `C4`**: *"refresh my authored kinds from NAS"*, explicit and user-triggered | 🔴 **THE USER** — a new user-facing operation, so ⛔ not assumed and not a task. ⚠ §7.3b makes it the only route by which an authoring host takes someone else's published assets; until then `C3`'s BEHIND arm **warns and stops there** |
