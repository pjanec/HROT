...
| 🔴🔴🔴 **`CE-116` — THE OPEN DESIGN QUESTION** | `MandatoryComponents` is **per TEMPLATE**; an override is **per ENTITY** *(`Health` on 6 of 8 hill-attack entities, `UnitSubordinate` on 4 of 8)*. ⛔ Hard ⇒ **deadlocks** non-overriding entities · Soft ⇒ every one pays the stall and a late override lands after systems ran on the default · always-send ⇒ defeats TKB-as-source. ⭐⭐⭐ **Recommended: a per-entity expected-override mask, DERIVED from the creator's step-8 set** *(not authored ⇒ nothing to drift)*; readiness becomes `template ∪ perEntity`. ⭐ Ingress already runs BEFORE promotion *(`:284` before `:308`)*, so a same-tick override needs no declaration — ⛔ **but that is an unguaranteed DDS ordering assumption that fails silently**, which is why `SimTransform` is Hard. ⚠ **3 open sub-questions: Hard-vs-Soft · extend `dtEntityMaster` or add a descriptor · mask vs short id list** |
