...
| 🔴🔴🔴 **`CE-116` — THE OPEN DESIGN QUESTION, 3rd iteration** | `MandatoryComponents` is **per TEMPLATE**; an override is **per ENTITY**. ✅ **RULED Hard** *("if scenario wants it, it must be fulfilled")*. ⛔⛔ **Two of my designs are DEAD:** a **component-id bitmask** leaked an FDP id onto the wire *(📌 `Q59` §7.4 retired `DescriptorOrdinal` from FDP for the MIRROR violation)*; **nesting overrides in the creating sample** is impossible because 📐 **`CreateGhost` is called from ≥10 ingress translators ⇒ FIRST-TOUCH creation**, no privileged sample. ⭐⭐ That fact *justifies* the readiness gate rather than making it a workaround. ⭐⭐⭐ **§13's proposal: readiness ENTIRELY IN DESCRIPTOR SPACE** — `EntityMaster` gains one `uint64 RequiredDescriptors` *(38 `EDescriptorType`s fit in 64 **bits**)*, `GhostStateTracker` gains `AppliedDescriptors`, each ingress ORs its ordinal, promotion checks `(applied & required) == required`; the creator **derives** the mask from its step-8 set via the inverse of `TargetComponentIds`. ⭐ **Works because `EntityMaster` is a promotion prerequisite BY CONSTRUCTION** — `EntityMasterIngressTranslator:140` is the only place `TkbIdentity` is stamped ⇒ no marker sample, no timeout, no ordering assumption. 🔴 **BLOCKER: `TargetComponentIds` is under-adopted 9 of 41 behind a silent empty default** ⇒ the inverse map is sparse and would silently omit a bit. ⚠ Also unmeasured: `EntityMaster` IDL extensibility · `Volatile` durability vs late joiners · whether every ingress has a place to OR |
