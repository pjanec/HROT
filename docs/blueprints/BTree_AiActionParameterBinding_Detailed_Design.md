...
> **Working drafts / demo specs:** `.dev/_DONE/btree-ai-action-binding/` (SLICE1-DESIGN, SLICE2-DESIGN, ARCHITECT-REVIEW-BRIEF). This doc is the canonical aggregation.
...
- **The root params slot**: the packed variable table of the behaviour assigned to the entity, located by `RootParamsAccess` and keyed `OccurrenceSlotKey.ComputeRootParamsKey(BehaviorState.ActiveBehaviorHash)`. Its size is `RootParamsBytes(def)` — there is no fixed cap; the allocator promotes the entity to a larger tier if the current one cannot hold it.
...
- `BlackboardBinPacker` computes sequential, C#-alignment-padded offsets, bounded by the tier payload the allocator can supply (**176 / 928 / 3 936 / 16 368 B**). The **authoritative** offset is the compiled struct layout (`Marshal.OffsetOf`); the editor bin-packer is advisory for the budget UI but must replicate C# sequential layout exactly (natural alignment capped at 8; padding before `fixed`/`[InlineArray]`).
...
### 3.4 Constraints
The aggregate of all bound master DTOs must fit the payload the allocator can reserve for the root params slot; beyond that the entity is promoted a tier, and only the top tier's payload (16 368 B) is a hard stop. Exactly **one stateful** AiPrimitive per entity in Slice 1 (lifted by Slice 2, §4).
