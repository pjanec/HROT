...
| **U-10** 🟠 | migrator **pair** + envelope 1→2 — **D2** | persistence | ⭐ **transform B49 · READER wired B54 (all 58 load from v2) · 4 corpus-invisible defects fixed.** ⭐⭐ **`BP-235` is ANSWERED — [`Q31`](Architect_Question_31_Migration_Seam_ANSWERS.md) ⇒ `A1`/`B1`/`C2`/`D1`; extract `Hrot.Blueprints.Schema`, blast radius is 2 profiles not 6.** ⭐⭐ **THE BUMP IS RELEASED (user, `2026-08-14`)** ⇒ ⏭ **B55 builds all three steps and CLOSES `U-10`. See §U-10** | U-15, U-12 |
