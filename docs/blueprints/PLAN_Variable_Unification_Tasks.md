...
| **52** ⏭ | 🔴 **the red gate** · `U-12` | ⛔ **§1 first: the suite must be green before a store flip is verified.** Then the rails **and the store flip**, once nothing reads the old views. ⚖️ **kept alone: three rail restatements + a store flip is its own revert story** |
