...
| **53** ⏭ | ⭐⭐ **`U-12`'s STORE FLIP** — the three lists become serialization-only projections | 🔴🔴 **`persistence-shape.txt` must not move.** ⛔ **No clean mid-flip stop** |
| **54** 🆕 | ⭐ **`U-10`'s WIRING** — write + read v2, bump `$meta.schemaVersion` | ⚠⚠ **Re-sequenced Batch 49, measured.** Only after `U-12` does the on-disk shape mirror an in-memory shape that exists, ⇒ the migrator becomes **a thin mapping** rather than a three-lists ⇄ one-array conversion into a shape nothing consumes. 🔴 **`BP-235` (the netstandard2.0 wall) and `ClusterRunner --mode migrate` are both live here** |
