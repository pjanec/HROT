...
| **51** ⏭ | `U-11` **editor bucket** | ⭐ **~50 refs / 8 files.** ⛔ **`U-12` cannot delete the views until a grep says nothing reads them** |
| **52** | `U-12` | the rails **and the store flip**, once nothing reads the old views. ⚖️ **kept alone: three rail restatements + a store flip is its own revert story** |
| **53** 🆕 | ⭐ **`U-10`'s WIRING** — write + read v2, bump `$meta.schemaVersion` | ⚠⚠ **Re-sequenced Batch 49, measured.** Only after `U-12` does the on-disk shape mirror an in-memory shape that exists, ⇒ the migrator becomes **a thin mapping** rather than a three-lists ⇄ one-array conversion into a shape nothing consumes. 🔴 **`BP-235` (the netstandard2.0 wall) and `ClusterRunner --mode migrate` are both live here** |
