...
| ✅ **`VehiclePresets.GetPreset` now stamps its own `Class`** *(FIXED `2026-08-28`, user challenge)* | 📐 Found while writing the rails: `GetPreset(Tank).Class == PersonalCar` — it filled 13 kinematic fields and left the 14th, the one saying **what the struct IS**, at its default. ⭐⭐ **Measured: 11 callers, and THREE had each independently written the missing assignment** *(`VehicleCommandSystem:75` "Set class field" · the CarKinem example's `ScenarioManager:144` "Ensure class is set" · the translator)* ⇒ **a duplicated workaround for a function's own omission** *(ruling 9's shape)*. ⚠ **Fairly stated it was LATENT, not live:** every caller that *stores* the struct did set `Class`; the two that skip it only read `Length`/`Width` for display and two more ask for `PersonalCar`, which is `0` and so right by accident. ⇒ ⭐ **the fix stamps it once inside `GetPreset` (normalising an undefined class to `PersonalCar` BEFORE the lookup, so `Class` always describes the data returned), and the three duplicated lines are DELETED.** ⭐ Rail is now a `[Theory]` over `Enum.GetValues<VehicleClass>()` so a new class cannot be added without an arm that stamps it |
