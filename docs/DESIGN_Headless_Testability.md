...
| 🔴🔴 **the worlds cannot be equalised** *(`HN-029`)* | `POST /scenario/load` ⇒ `NOT_SUPPORTED_HERE(editor.authoring)` in the cluster ⇒ ⛔ this design's own sequence *"load S in both"* is **not executable yet**, and world-CONTENT diffs are out of reach. ⭐ **FIX (user `2026-08-24`):** two cluster-wide endpoints **`scenario/load/live` + `scenario/load/edit`**, both publishing the host-agnostic `TransitionStateIntent` *(the editor is a one-node cluster that already uses this 2PC)* — 📄 [`Architect_Question_54` § Scenario load](blueprints/Architect_Question_54_Cluster_Mcp_Contract.md). `load/live` needs only endpoint plumbing *(all handlers exist)*; then "load S in both" runs and `entity-inspector` becomes a real content diff |
