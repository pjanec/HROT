...
| ⭐⭐⭐ **the worlds CAN now be equalised** *(`HN-029`, BUILT `2026-08-24`)* | ⭐ **This design's own sequence *"load S in BOTH, then diff"* is EXECUTABLE.** `scenario/load/live` + `scenario/load/edit` publish the host-agnostic `TransitionStateIntent` on the host's own orchestration bus; 📐 `--mode all` loads `hill-attack` live and materialises 8 entities. ⇒ `The_two_hosts_hold_the_same_loaded_world` compares the two hosts' loaded scenario content for real. 📄 the as-built, with six deviations: [`MCP_Integration.md` § Group U AS-BUILT](MCP_Integration.md). ⚠ **Two things it did NOT deliver:** `entity-inspector` stays a DECLARED divergence — the IG node also lists node-local entities *(networkId 0, unnamed)* — so it is now DIAGNOSED rather than SAME; and the two hosts' `networkId`s differ *(editor 1000+, cluster 2+: different allocator authorities)*, so entities are matched by NAME. *(Prior state, SUPERSEDED: "not executable yet — POST /scenario/load answers NOT_SUPPORTED_HERE(editor.authoring)".)* |
