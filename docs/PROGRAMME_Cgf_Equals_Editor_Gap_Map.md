...
| **The main toolbar as a readable `PanelKind`** | slice-2 design §6 ⑤ · §7 | ✅/⚠ | ⭐ **SLICE 2** — `main-toolbar` publishes on **both** hosts *(even when it does not draw)*. ⚠⚠ **CGF's is EMPTY**: 📐 `EditorSubsystem` is the ONLY caller of `MainToolbar.RegisterEntry` ⇒ a FEATURE gap *(`CE-016`)*, not an instrumentation one. ⛔ **§7 binds every later feature slice**: a toolbar-controlled feature must wire AND instrument its button on CGF |
