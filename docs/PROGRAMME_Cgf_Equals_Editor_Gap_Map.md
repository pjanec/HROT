...
| `AiDocumentManager` + `IPerspectiveSwitcher` + `PerspectiveWorkspaceRegistrar`×3 + `AssetCatalog` | UXI-37 §5b · slice-1 design §9.1 | ✅ | ⭐ **SLICE 1** — all four constructed at CGF's root, mirroring `EditorSubsystem` `:2545-2948`. ⚠ The catalog is **empty** *(CGF indexes no authoring assets — §9.4)*, which is the one measured consequence: `diagnostics` is declared divergent until asset indexing reaches CGF |
