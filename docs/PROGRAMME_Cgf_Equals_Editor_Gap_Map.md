...
| **The main toolbar as a readable `PanelKind`** | slice-2 design §6 ⑤ · §7 | ✅ **CE-016 CLOSED `2026-08-25`** | ⭐ **SLICE 2** — `main-toolbar` publishes on **both** hosts *(even when it does not draw)*. ⚠⚠ **The *"CGF's is EMPTY"* claim was already STALE**: slice 3 registered `SaveAllAiDocuments` + `QuickReloadAiAsset` *(`CE-022`)*. ⭐⭐ **`CE-034` closes the remaining gap, and it was a SILENT DEFAULT, not a missing feature**: the editor puts `MainToolbarTimeControlSection` on its toolbar *(`EditorSubsystem:4715`)* while CGF built `ClusterTimeTransportAdapter` — the very `ITimeTransportFacade` that section takes — and passed it only to the STATUS BAR, two lines away. 📌 *"A production caller that HAS a dependency must PASS it."* ⇒ same shared section, same id and sort order as the editor; ⛔ nothing invented. ✅✅ **NOW DONE — `CE-037`..`CE-040`, `2026-08-26` *(AQ58 A2)*:** CGF's toolbar routes through `ToolbarCommandAdapter`/`IEditorCommands`. ⭐⭐⭐ The layout is ONE shared table *(`CgfEditorShellToolbar`, AiShared)* and the per-host subset is **DERIVED** — an entry appears only for a command that host's shell can service ⇒ ⛔ no CGF list, no `if (host==…)`, and ruling 49 by construction. ⭐ CGF's two ad-hoc `ImGui.Button` entries and its dangling `ToolbarSep_TimeToPersp` are DELETED, replaced by `shell.save` + `blueprint.compileReload` at the editor's own ids/sortOrders with real icons; the `main-toolbar` known-divergence entry is deleted from the conformance rails. ⚠ **Three argued deviations** *(design §9)*, ⭐⭐ **all three now DISCHARGED or superseded:** no Save-All button *(the editor never had one — adding it would break the byte-identical gate; ⭐ still stands)*; ~~**the AI-debug group is OMITTED on CGF**~~ ⇒ ✅✅ **REVERSED `2026-08-27` by `CE-059`** *(see the row below)*; ~~Open/New omitted *(no picker launcher composed)*~~ ⇒ ✅ **composed by `CE-049`**. ⛔ **§7 still binds every later feature slice** |
| ⭐⭐⭐ **The AI-debug toolbar group on CGF** | this file's own argued deviation above · `CgfSubsystem` HISTORY note | ✅ **`CE-059` `2026-08-27`** | 🔒 **User, `--mode all` check:** *"Editor has also lots of toolbar buttons for debuggiing, none shown."* 📐 `AiDebugCommands.Register` had **exactly ONE caller repo-wide**, so the six `debug.*` ids did not exist on CGF. ⭐⭐ **The old omission argument had TWO claims and only ONE has rotted.** ⛔ **OVERTAKEN:** *"CGF has no debug session"* — 📐 this host already held **all three** `BlueprintDebugSession` ctor arguments *(`_blueprintRegistry`, the world, and `CgfClusterDebugTimeController : IEngineDebugTimeController`)* and simply never constructed it ⇒ 📌 **the silent-default shape, third instance in that file after `CE-052`**. ⭐⭐ **STILL BINDING:** `debug.*` is **AI-GRAPH** stepping and `CgfClusterDebugTimeController` is **CLUSTER-TIME** control — ⛔ binding the time controller to `debug.pause` would still make one id mean two things, so `CE-059` does **not** do that; it supplies the AI-graph session the ids already meant, and cluster-time keeps `MainToolbarTimeControlSection`. ⚠⚠ **Registering the group alone would have been WORSE than the omission** *(ruling 49)*: every `IsEnabled` reads `IDebugSessionRegistry.ActiveSession`, so the mirror is load-bearing — the editor's `SyncActiveDebugSession` **local function** is now the shared `ActiveDebugSessionMirror`. ⛔ **NOT wired:** the blueprint live-value provider *(`CE-062`)* — its stated blocker was the same now-false claim |
| ⭐⭐ **Perspective toolbar ICONS on CGF** | — | ✅ **`CE-058` `2026-08-27`** | 🔒 **User:** *"instead of graphical icons (as rendered in the editor) there are plain imgui buttons … cgf must be using some different toolbar code, not shared with editor"*. ⭐⭐⭐ **The diagnosis was half right and the wrong half is the instructive one:** 📐 both hosts build the **same** `PerspectiveToolbarSection` over the **same** `SilkIconProvider` — ⛔ there is no second toolbar. `RegisterPerspectiveIconKey` had **one caller repo-wide** *(five inline calls in `EditorSubsystem`, inside its `MainToolbar != null` block)*, so on CGF `GetPerspectiveIconKey` returned null and the section took its **documented text-label fallback**. ⇒ ⭐ the plain buttons were the shared code's own graceful degradation. ⭐ Now ONE shared table *(`PerspectiveIconKeys`, AiShared)*, registered at the **top of `RegisterWindows`** on both hosts — ⚠ moved out of that guard deliberately: inside it, **no window unit rail could reach it**, which is why nothing caught the gap |
