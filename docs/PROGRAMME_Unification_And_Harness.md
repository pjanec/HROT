...
| **2** | ⭐ **Keep building the harness** so it supports §2's four jobs — 📄 **DESIGNED**: [`DESIGN_Regression_Net.md`](DESIGN_Regression_Net.md) `N0`–`N2` | — |
| **3** | ⭐⭐ **Set up the baseline + tests, generate the goldens — and prove the harness actually works** — 📄 `DESIGN_Regression_Net.md` `N3`–`N5`, and ⭐⭐ **§8 is what *prove* means: a mutation table** | ⛔ **the net must be trusted before it is relied on**; a green suite that encodes nothing is worse than none |
