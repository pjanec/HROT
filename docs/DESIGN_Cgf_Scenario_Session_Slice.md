...
| **D6** | *(unstated)* | ⭐ **`scenariosRoot` is a `Func<string>`, not a string** | ⛔ The editor's `EditorBootstrap.ScenariosRoot` is a computed property over `ClusterConfiguration.Default.NasBasePath`; snapshotting it at construction would change **when** the value is read |
| ⭐⭐⭐ **D6a** *(`2026-08-27`, `CE-057`)* | ⛔⛔ **D6's second sentence was WRONG and is SUPERSEDED.** It said *"CGF passes `OrchestrationConstants.GetNodeScenariosRoot(nodeId)` — the same directory its own `HrotScenarioLoader` reads"* | ⭐⭐ **CGF passes `OrchestrationConstants.GetSharedScenariosRoot()`** — the same root the editor resolves | 📐 **Measured `2026-08-27`, from the user's `--mode all` check:** `{staging}/nodes/node-N/scenarios` **does not exist** *(the node directory holds only `recording_ledger`)*; the authored scenarios are in `{staging}/shared/scenarios` — i.e. `ClusterConfiguration.Default.NasBasePath` + `scenarios`, which IS `EditorBootstrap.ScenariosRoot`. ⇒ CGF was SAVING to a directory nothing lists and `CE-053` was LISTING from it, so the picker was empty. ⭐ `OrchestrationConstants` now owns the shared-root shape *(reachable from `Hrot.CGF`, unlike `Hrot.Orchestrator`)*, so there is one authority and no second answer. ⚠ **`Default` is a fresh `new()`**, so no loaded config makes the two hosts differ — the divergence was pure, not configurable |
