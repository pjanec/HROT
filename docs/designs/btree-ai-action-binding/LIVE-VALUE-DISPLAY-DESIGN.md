...
Decoding any slot into typed fields needs the managed `Type`. `RootParamsProjection` (the tier renderers' root-params section) already does this for the root params occurrence slot because `ManagedBlackboardVariable(Name, Type, ByteOffset)` carries `Type`. But the Slice-2 stateful manifest does **not**:
```
