...
- **Added hazard — aggregate inline ceiling.** Multiple distinct param DTOs rapidly consume the 100 B inline budget. The bin-packer must pad for alignment and enforce the ceiling; if the **aggregate** of all bound master DTOs exceeds 100 B → `FDP_001` hard error (overflow goes to the heavy tier, but **master** variables must always fit inline).
