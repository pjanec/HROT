...
2. **Assignment.** A generated/hand-written `ParseParamsDelegate(json, byte*)` runs once at behavior assignment, writing defaults into the root params slot ([BehaviorIngressSystem.cs:93](../../FDP/Toolkits/Fdp.Toolkits/Behavior/BehaviorIngressSystem.cs#L93)).
...
4. **Registry / runtime.** The blob is type-erased: `MethodNames[]` hold string keys `{Type}.{Method}@{offset}` (or bare name for 4-param). `Interpreter<byte,BTreeContext>.BindActions` resolves each key in the injected `ActionRegistry`; a miss installs a `=>Failure` fallback. The JSON bridge injects a populated registry (PREREQ-A, `8eb45e0c`). Tick site `BTreeTickSystem.cs:136` is generic over `byte` — the tree ticks against the root params slot's bytes (a type-parameter swap, not a behaviour-shaped generic-runtime change).
5. **Bin-packer.** `BlackboardBinPacker` computes sequential, C#-alignment-padded offsets against its own `MaxInlineBytes=100`/`MaxHeavyBytes=928` editor budgets — a legacy authoring-UI mirror of the analyzer's cap, pending retirement alongside it; it does not correspond to a runtime overflow region. Editor offsets are *advisory* for the budget UI; the **authoritative** offset is the compiled struct layout (`Marshal.OffsetOf`), and the real bound a build enforces is the tier ladder's structural ceiling.
...
1. **Per-asset blackboard struct** from the authored variables (reuse `BlackboardDtoEmitter`), `[StructLayout(Sequential)]`, bin-packed to fit the editor's inline budget (`BlackboardBinPacker.MaxInlineBytes`, a legacy 100 B mirror — the structural bound is the root params slot's tier); **offsets authoritative from the compiled struct** (`Marshal.OffsetOf`).
...
- **Added hazard — aggregate ceiling.** Multiple distinct param DTOs rapidly consume a tier's payload. The bin-packer must pad for alignment and enforce the ceiling; if the **aggregate** of all bound master DTOs exceeds the editor's legacy 100 B inline budget, that budget still gates the build as `FDP_001` today (pending retirement — the structural ceiling is the resolved tier's whole payload, up to 16 096 B, and the allocator promotes automatically).
