...
| <a id="uxr-90"></a>**UXR-90** | 🔒 **Map-interaction parity across Editor, SimHost and CGF.** All three have a map and all three must offer the **same common set**: rubber-band selection, selection highlight, centre-on-selected, delete selected, drag-move selected, distance measurement, and the shared entity context menu. Differences are the *exception*, declared per host — not the default | Perform each gesture in all three modes; behaviour matches except where a host has declared a difference | 🔴 **Far from met.** Editor has the full set; **SimHost has 3 tools** (Select/Drag/Rotate) and no rubber-band **visual**; **CGF has no `SelectionInteractionSystem` at all** — its "selection" is a context-menu item. Only the Editor passes `RubberBandState`. User requirement, 2026-08-10 | P0 |
