...
| <a id="uxr-90"></a>**UXR-90** | 🔒 **Map-interaction parity across Editor, SimHost and CGF.** All three have a map and all three must offer the **same common set**: rubber-band selection, selection highlight, centre-on-selected, delete selected, drag-move selected, distance measurement, and the shared entity context menu. Differences are the *exception*, declared per host — not the default | Perform each gesture in all three modes. The **gesture** is available everywhere; **which items or overlays appear** may legitimately differ because the entity lacks the components. ⚠ A capability may only be marked *not applicable* with **positive evidence the component is absent** — "nothing appeared" is equally consistent with a wiring mistake | 🔴 **Far from met.** Editor has the full set; **SimHost has 3 tools** (Select/Drag/Rotate) and no rubber-band **visual**; **CGF has no `SelectionInteractionSystem` at all**. ⭐ Mostly **one missing registrar call per host** — [baseline](UX_Map_Parity_Baseline.md). 🔒 **Ruled 2026-08-10: all three share the FULL set of gizmos and tools**; what differs is *data availability* (component presence, since replication is encoded in ECS component translators, not a config) or an explicit *host rule*. Never set membership. ⇒ parity is of the **gesture**; an empty overlay can be the correct answer | P0 |
| <a id="uxr-91"></a>**UXR-91** | **Multi-select semantics are uniform.** The context menu shows **only items applicable to *every* selected entity**, and the chosen action is **issued to each** of them | Select a mixed group → only universally-applicable items appear. Invoke one → it applies to every selected entity, not just the clicked one | 🔴 **No host supports it end to end** — but ⚠ **more of it is built than this row said** ([Correction 31](UX_Tasks_Detail.md#corrections)): the inspector list has ctrl/shift additive click **and** routes to the multi-entity overload at `selCount > 1`. That overload is a **default no-op nobody usefully overrides**, so the menu comes up blank; the map has no additive click at all; and multi-target behaviour is faked by closing over `_selectionState`, whose only mutator collapses to one. ⇒ [UXI-24](UX_Feature_Multi_Select.md), designed 2026-08-13. User requirement, 2026-08-10 | P0 |
