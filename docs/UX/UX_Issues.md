...
| | <a id="uxi-26"></a>**UXI-26** 🔴 | **Async handlers race the tick.** No `SynchronizationContext` exists (2 mentions repo-wide, both opting out) and the pick `TaskCompletionSource` is `RunContinuationsAsynchronously`, so `await` resumes on the **thread pool** — where the continuation calls `_world.Bus.Publish` into an `FdpEventBus` with **zero** synchronization. Affects **100 % of existing async handlers** (`EditorSubsystem.cs:1462`, `:1479`). ⭐ **Fix is one class**: a tick-bound `SynchronizationContext`, as WinForms/WPF/Unity do — no handler edited | `RW-L` | — | [UX_Interaction_API.md §6](UX_Interaction_API.md#6--thread-affinity--the-proven-fix-and-it-is-small) | ☐ |
| **C** | | **Tools** | | | | |
...
**Counts:** **26 issues** · **7 designed** · 9 🔴 · 1 unverified (UXI-19) · UXI-22 folded into UXI-23 · UXI-25 split out of UXI-04 (user, 2026-08-10).
