...
| **H** | | **Cross-host parity** | | | | |
| | <a id="uxi-23"></a>**UXI-23** 🔴 | **SimHost and CGF lack the common map-interaction set.** No rubber-band visual, no measure, no centre-on-selected, no delete-selected, and **CGF has no `SelectionInteractionSystem` at all**. Absorbs [UXI-22](#uxi-22). ⭐ **Mostly one missing registrar call per host.** Delivered by the mechanisms in B and C plus per-host wiring | `RW-M` | [UXR-90](UX_Requirements.md#uxr-90) | [UX_Map_Parity_Baseline.md](UX_Map_Parity_Baseline.md) *(inventory)* | ☐ |
