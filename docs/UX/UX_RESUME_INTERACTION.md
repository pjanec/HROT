...
have their own feature docs. **45 rulings · 35 corrections**, all contiguous and linked below.

✅ **[Q29 — entity commanding](Architect_Question_29_Entity_Commanding.md) is ANSWERED** (rulings 38-45).
A right-click order is an **immediate tactical intent**, carried by **one action id + a JSON argument**,
with the command set and default params coming from **TKB (new fields)**. [UXI-32](UX_Issues.md#uxi-32)
is ready to design.
...
| **3** | 🔴 **How does TKB data reach ExCon?** [Ruling 44](#) requires it, and **TKB content is replicated nowhere** — `EntityMasterTopic` carries only the `long` id; content moves as staged `.zip` files that **only SimHost loads**. **I1** new `AI.CommandSet` descriptor + a delivery path for ExCon · **I2** extend the compiled `BehaviorCatalog` (zero gap, but not data) · **I3** 🎯 I1 with I2 as fallback. ⚠ **A new DDS topic for TKB content is a much larger commitment than the descriptor field** — worth deciding deliberately | [Q29 §I](Architect_Question_29_Entity_Commanding.md) |
