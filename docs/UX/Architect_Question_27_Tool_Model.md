...
| **D** — `EditorTool` vs shared ids | ✅ **D2, with the relationship pinned** | *"A tool is not an action. **An action is what activates a tool.**"* ⇒ two vocabularies, one relationship; `GlobalActionIds.Measure` is the *action* that activates the *Measure tool*. ⭐ **This vindicates rejecting D3.** Also: *"tool handling should be defined by **registration-time flags** — no less flexibility than now"*. ⚠ **Refined the same day:** cancellation is *"driven by **focus changes only**; actions might need flagging if they steal focus"* ⇒ `CancelsModalTool` (renamed from `StealsFocus` the same day — it named a mechanism rather than the consequence) is an **action** flag, not a tool flag, and **re-activating a modal tool does not cancel it** unless `ToggleOnReactivate` is set |
| **F** — modal tool **stack** ⬅ *added 2026-08-10* | ✅ **required** | *"Sometimes in the middle of editing one thing we need to temporarily jump to another and then back. A tool stack. I bet it was implemented."* ⭐ **It was, and it was deleted** — `MapCanvas.PushTool`/`PopTool` survive only as a **broken `<see cref>`** (`EditorMapPickAdapter.cs:26`) and two comments; not recoverable from git (squashed import). What replaced it is flat: the pick adapters `Register` without LIFO, so the *caller's* control flow resumes via `TaskCompletionSource` but **the previous tool never does**. ⇒ `PushModal`/pop alongside `Activate`; **suspend = `SetFocus(false)` without the `Dispose()`** that both teardown paths currently pair it with. 🔒 Same mechanism as the B ruling's unfocused-subsystem suspension. ⭐ **And it settles the flag question: cancel is declarable, suspend is not** — suspension needs a resume point only the handler knows, so it is scoped (`using`/`await`) rather than declared |
| **E** — Escape | ✅ **E2 for modal, E1 where local** | *"Centralize for modal tools for sure, avoid code duplication; keep local where necessary."* ⚠ **Refined by F: Escape pops ONE stack level**, it does not clear the stack |
