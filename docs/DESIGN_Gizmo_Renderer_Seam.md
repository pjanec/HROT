...
| ⑤ | ⛔⛔ **A CAPABILITY THE MIGRATION DROPPED: a `Line` is UNPICKABLE.** The live hit-test serves **`Box2D` and `Sphere` only**; the retired rails were its only record | 🔒 **`R-137`** — filed as **`CE-259ac`**, ⛔ **not** re-implemented on a rail's say-so *(whether gizmo lines should be pickable is a UX decision and no design record asks for it)*. ⭐ `SC-GZ026-2b` **pins the loss** so it cannot be lost twice; the geometry coverage moved to `Box2D` |
