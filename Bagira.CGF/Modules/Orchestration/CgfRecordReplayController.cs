...
namespace Bagira.CGF.Modules.Orchestration
{
    ...
    /// replay DSM lifecycle (CGF-1-BATCH-23 A.1).
    ///
    /// <para>
    /// <b>Current behaviour — Phase 3 skeleton:</b>
    /// All lifecycle methods return <see langword="Task.CompletedTask"/> immediately.
    /// CGF participates in the DSM handshake (ACKs to the orchestrator) but does not
    ...
    public sealed class CgfRecordReplayController : IRecordReplayController
    {
        ...
        public Task PrepareRecordingAsync(Guid drillId, string storageDirectory)
        {
            FdpLog<CgfRecordReplayController>.Info(
                "[CgfRecordReplayController] PrepareRecording called (drillId={0}). " +
                "Phase 3 skeleton: no ECS recording — ACK only.", drillId);
        ...
        {
            FdpLog<CgfRecordReplayController>.Info(
                "[CgfRecordReplayController] PrepareReplay called (drillId={0}). " +
                "Phase 3 skeleton: no ECS replay — ACK only.", drillId);
            _replayActive = true;
            return Task.CompletedTask;
        }
