...
///
/// <para>
/// Entity creation requests are extracted from the scenario JSON via the staging
/// pipeline and enqueued for time-sliced processing by the genesis pipeline
/// (<c>CreateEntityRequestSystem</c> -> <c>NetworkSpawningSystem</c>).
/// This ensures the <c>EntityLifecycleModule</c> handshake is honoured and
/// that <c>AuthorityMask</c> is correctly stamped on every loaded entity.
/// </para>
/// <para>
...
public sealed class HrotEditLoadHandler : ITickableClusterStateHandler
{
    ...
    public void Commit(ExecuteNodeOpIntent intent, EntityRepository? repo)
    {
        ...
        try
        {
            // ⛔⛔ F1 — THE ZONE ARM IS GONE, and its absence is the whole point.
            //
            //   This used to parse a `Zones` section out of the envelope and hand it to an
            //   IZoneManagerService, warning when a host composed none (CE-102 / ruling 49). That warning
            //   was the honest face of a real split: zones travelled OUTSIDE the entity pipeline, so a
            //   host could load the entities and not the zones.
            //
            //   ⭐ A zone is now an ordinary authored ENTITY, so it rides `_pendingRequests` through the
            //   genesis pipeline below with everything else. There is no half to miss, no service to
            //   compose, and no host-specific absence to explain — which is why nothing replaces this.
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Called from the main ECS thread each frame.  Waits until all pending entity
    /// creation requests have been consumed by <c>CreateEntityRequestSystem</c> and
    /// all ECS entities have left the <c>Constructing</c> lifecycle phase.
    /// </remarks>
    public void DrainDeferredAcks()
    {
        if (_operatingEditTcs == null) return;

        // Condition 1: all entity creation requests must be consumed first.
