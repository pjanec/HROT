...
public sealed class TheRetiredSurfacesAreGoneTests
{
    ...
    private static Type[] EditorTypes() => new[]
    {
        ...
        typeof(Hrot.Blueprints.Editor.Windows.BlueprintDetailsWindow),
    ...
    /// <c>BlueprintDetailsWindow</c>'s node arm, live since Batch 87.
    /// </summary>
    [Fact]
    public void TheBlueprintsInspectorStub_IsRetired()
        => Assert.DoesNotContain(
            typeof(Hrot.Blueprints.Editor.Windows.BlueprintDetailsWindow).Assembly.GetTypes(),
            t => t.Name == "InspectorWindow");

    /// <summary>⛔ <b>The two Blueprint variables windows are gone</b> — their replacement
    /// *(<c>BlueprintDetailsWindow</c> hosting the SHARED <c>VariableDetailsSection</c>, <c>U-6</c> /
    /// Batch 82)* is live, which is §6 <c>L5</c>'s precondition.</summary>
    [Fact]
