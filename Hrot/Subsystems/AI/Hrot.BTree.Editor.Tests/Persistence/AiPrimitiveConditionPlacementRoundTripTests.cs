...
/// Condition.WorkingStateTargetField, mirroring the action-side sink change.
/// </summary>
public sealed class AiPrimitiveConditionPlacementRoundTripTests
{
    ...
    public void PlacedAiPrimitiveConditionNode_MapsToDto_MatchingComposedActionShape()
    {
        ...
        conditionNodeDto.Condition.WorkingStateTargetField.Should().NotBeNullOrEmpty(
            "Slice 1: the sink must auto-create and bind a WorkingState host variable for composed conditions too");
        conditionNodeDto.Condition.WorkingStateTargetField.Should().NotBe(
            conditionNodeDto.Condition.ExpressionTargetField,
        ...
        dto.Blackboard.Variables.Should().HaveCount(2,
            "Slice 1: placing a composed AiPrimitive condition now creates both the bpParams (Input) and bpWorkingState (State) variables");

        var paramsVarDto = dto.Blackboard.Variables.Should().ContainSingle(
            v => v.Name == conditionNodeDto.Condition.ExpressionTargetField).Which;
        paramsVarDto.Type.TypeId.Should().Be(typeof(FakeBpGeneratedCondition.Params).FullName,
        ...
        var wsVarDto = dto.Blackboard.Variables.Should().ContainSingle(
            v => v.Name == conditionNodeDto.Condition.WorkingStateTargetField).Which;
        wsVarDto.Type.TypeId.Should().Be(typeof(FakeBpGeneratedCondition.WorkingState).FullName,
