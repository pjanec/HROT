...
/// <summary>Outcome of a <see cref="BlueprintAttachService.AttachToEntity"/> call.</summary>
public enum BlueprintAttachStatus
{
    ...
    /// <summary>The asset's blueprint id is not present in the registry.</summary>
    NotRegistered,
...
/// <summary>
/// Result of <see cref="BlueprintAttachService.AttachToEntity"/>.
/// </summary>
...
/// <summary>
/// Production-side helper that attaches an Instance Blueprint to a live entity so the
/// <c>BlueprintTickSystem</c> will tick it. This is the single seam shared by the headless
/// real-kernel run test and the future MVE-03 "Run Opened Blueprint on a Test Entity"
/// toolbar button — both must go through exactly the same attach sequence.
/// </summary>
/// <remarks>
/// <para>
/// The sequence mirrors <c>BlueprintTestFixture.AttachBlueprint</c> (the proven path):
/// <list type="number">
///   <item><c>BlueprintIdHash.Compute(asset.AssetId)</c> → runtime id.</item>
///   <item><c>registry.TryGetById</c> → require it is registered and <c>Kind == Instance</c>.</item>
...
/// </para>
/// </remarks>
public static unsafe class BlueprintAttachService
{
    /// <summary>
        ...
        Entity entity)
    {
        ...
        if (!registry.TryGetById(blueprintId, out var def) || def is null)
            return new BlueprintAttachResult(
                ...
                $"Blueprint '{asset.Name}' (id 0x{blueprintId:X8}) is not registered. " +
                "Compile/register it before attaching.");

        if (def.Kind != Fdp.Toolkit.Blueprints.BlueprintDispatchKind.Instance)
            return new BlueprintAttachResult(
                BlueprintAttachStatus.NotInstanceKind, default,
                $"Blueprint '{asset.Name}' is {def.Kind}, not Instance; only Instance " +
        ...
        if (TryFindExistingTier(world, entity, blueprintId, out var existingTier))
            return new BlueprintAttachResult(
                ...
                $"Blueprint '{asset.Name}' is already attached to entity {entity} " +
                $"(tier {existingTier}).");
        ...
        if (!BlueprintBlackboardPartitions.TryAttach(
            ...
            return new BlueprintAttachResult(
                ...
                $"No free slot/payload for blueprint '{asset.Name}' on entity {entity} " +
                $"in tier {tier}.");
        ...
        return new BlueprintAttachResult(
            ...
            $"Attached blueprint '{asset.Name}' to entity {entity} (tier {tier}).");
    }

    /// <summary>
    /// Selects the smallest blackboard tier whose payload can hold <paramref name="stateSize"/>.
    /// Bounds match <c>BlueprintBlackboard{1024,4096}.PayloadSize</c>.
    /// </summary>
