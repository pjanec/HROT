...
namespace Hrot.Map.Common.Replication.Ingress
{
    ...
    /// (checked via <see cref="AuthorityExtensions.HasAuthority(Fdp.ModuleHost.Core.Abstractions.ISimulationView, Entity, long)"/>),
    ...
    ///
    /// <para>Currently handles <see cref="EDescriptorType.dtWorldPos"/>:
    ...
    public sealed class UpdateEntityDescriptorRequestSystem : ComponentSystem
    {
        ...
        private void ProcessGeoSpatialUpdate(UpdateEntityDescriptorRequest req, Entity entity)
        {
            ...
            long packedKey = Fdp.ModuleHost.Core.Network.OwnershipExtensions.PackKey((long)req.DescriptorType, req.PartId);
        ...
        private void WriteAck(Guid requestId, int entityId, NedStatusCode errorCode)
        {
