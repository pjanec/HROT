...
{
    /// <summary>
    /// Unit tests for <see cref="SstRequestFinalizationSystem"/>.
    ///
    ...
    /// </summary>
    public class SstRequestFinalizationSystemTests
    {
        // ── Factory helpers ───────────────────────────────────────────────────
        ...
        private static (SstRequestFinalizationSystem system, StubAckSink ackSink, NetworkEntityMap map)
            BuildSystem()
        {
            var ackSink = new StubAckSink();
            var map     = new NetworkEntityMap();
            var system  = new SstRequestFinalizationSystem(ackSink, map);
        ...
        public void Execute_Create_SendsSuccessAck_WhenEntityBecomesActive()
        {
            ...
            Assert.Equal((int)SstStatusCode.Success,   ack.StatusCode);
        }
