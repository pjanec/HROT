...
///
/// Verifies that <see cref="BdcTkbCatalog.RegisterAll"/> correctly registers the
...
public class TacGraphicRouteBlueprintTests
{
    ...
    public void TacGraphicRoute_IsRegisteredInTkbCatalog()
    {
        ...
        Assert.True(db.TryGetByType(TkbEntityTypes.TacGraphic_Route, out _),
            "TacGraphic_Route (8802) must be registered in BdcTkbCatalog.");
    }
    ...
    public void TacGraphicRoute_Blueprint_DoesNotAddEditablePolyline()
    {
        ...
        Assert.False(world.HasManagedComponent<Bagira.IG.Components.EditablePolyline>(entity),
    ...
    private static EntityRepository CreateWorld()
    {
        ...
        // Additional components used by other blueprints in BdcTkbCatalog
        // (ApplyTo silently skips unregistered component types, so we only
        // need to register what we actually assert on).
        world.RegisterManagedComponent<Bagira.IG.Components.EditablePolyline>();
        return world;
    }
}
