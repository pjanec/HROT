...
public sealed class ReferenceArchiveHandlerTests
{
    ...
    public void Commit_ProducesManifestJson_WhenFdpExists()
    {
        ...
        const string drillId = "test_drill_01";
        const int    nodeId  = 5;

        var drillDir  = Path.Combine(tempRoot, drillId);
        var fdpFile   = Path.Combine(drillDir, $"node_{nodeId}.fdp");
        ...
        try
        {
            ...
            var cmd       = MakeCmd($"{{\"DrillId\":\"{drillId}\"}}", txId);
            ...
            Assert.Equal(Path.Combine(drillId, $"node_{nodeId}.fdp"), entry.RelativeDest);
        }
        finally
        {
    ...
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
        const string drillId = "abort_drill";
        const int    nodeId  = 3;
        ...
        try
        {
            ...
            var cmd     = MakeCmd($"{{\"DrillId\":\"{drillId}\"}}");
    ...
    // ── Guard: no DrillId in payload ──────────────────────────────────────────

    /// <summary>
    /// When the payload has no "DrillId" key, <see cref="ReferenceArchiveHandler.Commit"/>
    /// must exit silently without publishing any status and without throwing.
    /// </summary>
    [Fact]
    public void Commit_SkipsGracefully_WhenNoDrillId()
    {
        var transport = new StubTransport();
