...
using ClusterRunMode = Hrot.ClusterRunner.Configuration.RunMode;
...
public sealed class CgfSubsystemHeadlessTests
{
    ...
    {
        int domainId = Interlocked.Increment(ref _domainCounter);
        using var harness = new HrotRunnerHarness(ClusterRunMode.SimHost | ClusterRunMode.IG, domainId);

        long tkbType  = TkbEntityTypes.Tank_M1Abrams;
        var  spawnPos = new GeoPoint { Latitude = 52.521, Longitude = 13.406, Altitude = 0 };
    ...
    {
        int domainId = Interlocked.Increment(ref _domainCounter);
        using var harness = new HrotRunnerHarness(ClusterRunMode.SimHost, domainId);

        long tkbType  = TkbEntityTypes.Tank_M1Abrams;
        var  spawnPos = new GeoPoint { Latitude = 52.520, Longitude = 13.405, Altitude = 0 };
        long networkId = harness.SimHost.TestHook_SpawnEntity(tkbType, spawnPos);
        ...
        Assert.True(missionSeeded,
            $"SimHost entity {networkId} did not receive a MissionPlanQueue with PhaseCount≥1 within {MissionTimeoutMs} ms. " +
    ...
    public void SimHost_WanderMission_EntityMovesAfterDoctrineActivation()
    {
        ...
        long tkbType  = TkbEntityTypes.Tank_M1Abrams;
        var  spawnPos = new GeoPoint { Latitude = 52.524, Longitude = 13.406, Altitude = 0 };
        long networkId = harness.SimHost.TestHook_SpawnEntity(tkbType, spawnPos);
