...
namespace Bagira.SimHost.Integration.Tests
{
    ...
    /// orchestrates <see cref="RecordingModule"/>, <see cref="StoryRecorderModule"/>,
    ...
    public class RecordReplayIntegrationTests : IDisposable
    {
        ...
        public async Task RecordingLifecycle_InstallUninstall_ModuleInstalledThenGone()
        {
            ...
            var expectedFile = Path.Combine(_tempDir, drillId.ToString(), "node_1.fdp");
        ...
        [Fact(Timeout = 20_000)]
        public async Task StoryRecording_WithConcurrentGlobalRecorder_BothFilesProduced()
        {
            // Arrange: global recording + story recording start concurrently.
            var drillId    = Guid.NewGuid();
            ...
            var globalFile = Path.Combine(_tempDir, drillId.ToString(), "node_1.fdp");
            var storyFile  = Path.Combine(_tempDir, "stories", $"{storyId}_node1.fdp");

            Assert.True(File.Exists(globalFile),
                $"Global recording file not found at {globalFile}.");
            Assert.True(File.Exists(storyFile),
                $"Story recording file not found at {storyFile}.");
        }
