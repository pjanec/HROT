...
public sealed class ClusterUiCacheTests : IDisposable
{
    ...
    public void ClusterUiCache_ReflectsSystemStateTopic()
    {
        ...
        writer.Write(new SystemStateTopic { CurrentState = DSMState.LoadingLive });

        Thread.Sleep(150); // DDS propagation

        _uiCache.Update();

        Assert.Equal(DSMState.LoadingLive, _uiCache.CurrentState);
    ...
    public void ClusterUiCache_ClosesInFlightTxOnSysOpStatusSuccess()
    {
        ...
        using var statusWriter = new DdsWriter<SysOpStatus>(_participant);
        ...
        statusWriter.Write(new SysOpStatus
        {
