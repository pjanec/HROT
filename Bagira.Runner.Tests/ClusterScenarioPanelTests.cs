...
///
/// The panel now takes a <see cref="ClusterUiCache"/> instead of <see cref="DrillMaster"/>.
...
public sealed class ClusterScenarioPanelTests : IDisposable
{
    ...
    {
        using var reader = new DdsReader<SysOpRequest>(_participant);
    ...
    public void SeekDebounce_WritesAfter500ms()
    {
        ...
        Assert.True(found, "1 SysOpRequest{ReplaySeek} should be published after debounce expires.");
    ...
    public void LoadScenario_WithNoSelection_NoWriteOccurs()
    {
        ...
        Assert.False(anyWritten, "No SysOpRequest should be written when selection index is -1.");
    }
}
