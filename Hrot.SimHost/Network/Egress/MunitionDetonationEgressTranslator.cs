...
namespace Hrot.SimHost.Network.Egress
{
    ...
    /// logged and skipped — no exception is thrown.
    /// </para>
    /// </summary>
            ...
            foreach (ref readonly var evt in events)
            {
                if (!_entityMap.TryGetNetworkId(evt.Shooter, out long shooterNetId))
                {
                    FdpLog<MunitionDetonationEgressTranslator>.Warn(
                        "[MunitionDetonationEgress] Shooter entity #{0} not in NetworkEntityMap — skipping detonation.",
                        evt.Shooter.Index);
                    continue;
                }
                ...
                {
                    FdpLog<MunitionDetonationEgressTranslator>.Warn(
                        "[MunitionDetonationEgress] Target entity #{0} not in NetworkEntityMap — skipping detonation.",
                        evt.Target.Index);
                    continue;
                }
