...
/// <summary>
/// End-to-end DSM test scripts (CGF1-S0310).
...
/// </summary>
public sealed class DsmE2eScriptTests
{
    ...
    private static async Task<int> RunScriptAsync(string scriptFileName)
    {
        ...
        using var statusReader    = new DdsReader<SysOpStatus>(testParticipant);

        // Wire everything that requires a live world/DrillMaster inside AfterInitialize,
        ...
        executor.AfterInitialize = () =>
        {
            ...
            var drillMaster = orchestratorSvc.TestHook_DrillMaster!;
            ...
            executor.RegisterHandler(new SysopActionHandler(drillMaster, statusReader, logger));
        };
    ...
    /// <summary>
    /// Transitions to RunningEdit, spawns an entity, enters RunningDryRun,
    /// moves and re-spawns, then returns to RunningEdit and asserts the state reverts.
    /// </summary>
    [Fact(Timeout = 60000)]
    public async Task DryRunStateRestore_Passes()
    {
    ...
    /// branches to a new RunningLive drill, spawns a post-branch entity, and
