...
/// <c>SelectionChangedEvent</c> when the operator clicked on an entity; the IOS
/// "Selection &amp; Mission" panel always showed "No selection" because
/// <c>IosLogic.SelectedEntityId</c> was never updated.
...
public class SelectionAndMissionIntegrationTests
{
    ...
    private static readonly GeoPosition BerlinGeo = new GeoPosition
    {
        Latitude  = 52.521,
        Longitude = 13.406,
        Altitude  = 0,
    ...
    /// publishes a <c>SelectionChangedEvent</c> via DDS, which IOS receives and uses to
    /// update <see cref="Bagira.IOS.IosLogic.SelectedEntityId"/>.
    ///
    /// <para>
    ...
    public void IG_EntityClick_IosReceivesSelectionAndUpdatesSelectedEntityId()
    {
        ...
        // IOS must receive the DDS event and update SelectedEntityId within a few frames.
        bool selected = harness.PumpUntil(
            () => harness.Ios.Logic.SelectedEntityId == (int)networkId,
            SelectionTimeoutFrames);

        Assert.True(selected,
            $"IOS SelectedEntityId was not updated to {networkId} within {SelectionTimeoutFrames} frames. " +
            $"Current value: {harness.Ios.Logic.SelectedEntityId}. " +
    ...
    /// <summary>
    /// Verifies that <see cref="Bagira.IOS.Services.IMissionEditorService.CommitMissionAsync"/>
    ...
    /// </summary>
    [Fact]
    public async Task IOS_CommitMissionAsync_ResolvesWithAck_NotTimeout()
    {
        using var harness = new BagiraRunnerHarness();
        ...
        // MiniIosPanelState.SubmitWithWanderMissionViaGateway.
        ...
        Task<Bagira.IOS.Services.MissionCommitResult> commitTask =
            harness.Ios.Logic.MissionEditorService.CommitMissionAsync(networkId, plan, 0);

        // Pump frames: each frame runs IosSubsystem.Update() →  IosLogic.Update() →
    ...
    private static bool IgHasNetworkEntity(BagiraRunnerHarness harness, long networkId)
    {
    ...
    private static bool SimHostHasEntity(BagiraRunnerHarness harness, long networkId)
    {
