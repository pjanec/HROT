...
namespace Hrot.Common.Orchestration
{
    ...
    /// <see cref="OrchestrationStatus"/>) and the Hrot DDS message types
    ...
    /// thread inside <see cref="FDP.Toolkit.Orchestration.ClusterSlave.Tick"/>.</para>
    /// </summary>
    public sealed class DdsOrchestrationTransport : IOrchestrationTransport
    {
        ...
        /// <summary>
        /// Raw DDS status writer — exposed so legacy Hrot handlers that still accept
        ...
        public DdsOrchestrationTransport(DdsParticipant participant, int nodeId)
        {
            ...
            {
                IsBackground = true,
                Name = $"Node{nodeId}-ClusterSlave-Transport",
            };
