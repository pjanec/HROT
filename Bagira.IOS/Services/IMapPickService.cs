...
/// <c>MapClickEvent</c> round-trip, allowing any IOS panel to request a
...
public interface IMapPickService
{
    ...
    ///
    /// <para>
    /// Sends a <c>CMD_PICK_LOCATION</c> <see cref="Bagira.BDC.SSTM.MapCommandRequest"/>
    /// to the target IG and returns a <see cref="Task{GeoPosition}"/> that resolves
    /// when the operator clicks the map (or is cancelled via
    /// <paramref name="ct"/>).
    /// </para>
    /// </summary>
    /// <param name="ct">
    ...
    /// </param>
    Task<GeoPosition> PickLocationAsync(CancellationToken ct = default);
    ...
    ///
    /// <para>
    /// Sends a <c>CMD_PICK_ENTITY</c> <see cref="Bagira.BDC.SSTM.MapCommandRequest"/>
    /// to the target IG and returns a <see cref="Task{Int32}"/> that resolves with
    /// the network entity ID when the operator clicks a valid entity (or is
    /// cancelled via <paramref name="ct"/>).
    /// </para>
    /// </summary>
